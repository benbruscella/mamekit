// GENERATED executable machine composition from src/mame/universal/ladybug.cpp; do not edit.
import { decodeBoardIr } from '../../../../runtime/ir/decode.js';
import type { BoardConfig, BoardSinks, InputPorts, Regions } from '../../../../runtime/core/types.js';
import type { GeneratedCompiledHandler } from '../../../../runtime/ir/board.js';
import { createGeneratedBoard } from '../../../../runtime/core/generated-board.js';
import boardData from './board.json' with { type: 'json' };

// Decoded, not asserted: a stale or hand-edited artifact fails here, naming the
// field and its MAME source line, instead of crashing deep inside execution.
const defined = decodeBoardIr(boardData, 'ladybug');

// Direct JavaScript for handlers whose IR shape shows nested hot loops. The
// interpreter remains the semantic reference; these are checked against it by
// src/gen/emit-handler-codegen.spec.ts.
defined.compiledHandlers = {
  ...(() => {
    const methods = (() => {

  function method_spr_w(runtime: any, offset: any, data: any): any {
    const members = runtime.members;

    runtime.writeIndex(runtime.writableMember("m_spr_ram"), ((offset) & (1023)), data);
  }

  function method_bg_r(runtime: any, offset: any): any {
    const members = runtime.members;

    return runtime.readIndex((members.m_bg_ram ?? runtime.member("m_bg_ram")), ((offset) & (2047)));
  }

  function method_bg_w(runtime: any, offset: any, data: any): any {
    const members = runtime.members;
    const __l = runtime.links ?? runtime.calls;

    runtime.writeIndex(runtime.writableMember("m_bg_ram"), ((offset) & (2047)), data);
    (__l["m_bg_tilemap.mark_tile_dirty"] ? __l["m_bg_tilemap.mark_tile_dirty"](((offset) & (1023))) : (members.m_bg_tilemap) != null ? ((runtime.dereference(members.m_bg_tilemap)).mark_tile_dirty?.(((offset) & (1023))) ?? 0) : (__l["mark_tile_dirty"]?.(((offset) & (1023))) ?? 0));
  }

  function method_screen_update_ladybug(runtime: any, screen: any, bitmap: any, cliprect: any): any {
    const members = runtime.members;
    const __l = runtime.links ?? runtime.calls;

    (__l["bitmap.fill"] ? __l["bitmap.fill"](0, cliprect) : (bitmap) != null ? ((runtime.dereference(bitmap)).fill?.(0, cliprect) ?? 0) : (__l["fill"]?.(0, cliprect) ?? 0));
    (__l["m_video.draw"] ? __l["m_video.draw"](screen, bitmap, cliprect, (__l["flip_screen"] ? __l["flip_screen"]() : runtime.macro("flip_screen"))) : (members.m_video) != null ? ((runtime.dereference(members.m_video)).draw?.(screen, bitmap, cliprect, (__l["flip_screen"] ? __l["flip_screen"]() : runtime.macro("flip_screen"))) ?? 0) : 0);
    return 0;
  }
  return {
    "spr_w": method_spr_w,
    "bg_r": method_bg_r,
    "bg_w": method_bg_w,
    "screen_update_ladybug": method_screen_update_ladybug
  };
})();
    return {
      "ladybug_state.screen_update_ladybug": methods["screen_update_ladybug"],
    };
  })(),
} as Record<string, GeneratedCompiledHandler>;
// The host call names those handlers reach, resolved once into a fast table.
defined.compiledHandlerLinks = ["bitmap.fill","fill","flip_screen","m_bg_tilemap.mark_tile_dirty","m_video.draw","mark_tile_dirty"];
export default {
  machine: defined,
  createBoard: (
    config: BoardConfig,
    regions: Regions,
    inputs: InputPorts,
    sinks: BoardSinks,
  ) => createGeneratedBoard(defined, config, regions, inputs, sinks),
};
