...
defined.compiledHandlers = {
  ...
  function method_common_sreset(runtime: any, state: any): any {
    ...
    if ((((((Number(state) !== Number(0)) ? 1 : 0)) && ((((members.m_sreset_old ?? runtime.member("m_sreset_old"))) ? 0 : 1))) ? 1 : 0)) {
      ...
      (__l["m_sound_to_main.acknowledge_w"] ? __l["m_sound_to_main.acknowledge_w"]() : (members.m_sound_to_main) != null ? (typeof (runtime.dereference(members.m_sound_to_main)).acknowledge_w === 'function' ? (runtime.dereference(members.m_sound_to_main)).acknowledge_w() : typeof (runtime.dereference(members.m_sound_to_main)).acknowledge_w === 'number' || typeof (runtime.dereference(members.m_sound_to_main)).acknowledge_w === 'boolean' ? (runtime.dereference(members.m_sound_to_main)).acknowledge_w : runtime.container(members.m_sound_to_main, "acknowledge_w")) : 0);
      (__l["m_soundnmi.in_w_0"] ? __l["m_soundnmi.in_w_0"](0) : (members.m_soundnmi) != null ? ((runtime.dereference(members.m_soundnmi)).in_w_0?.(0) ?? 0) : 0);
    }
    (__l["m_audiocpu.set_input_line"] ? __l["m_audiocpu.set_input_line"](-2, state) : (members.m_audiocpu) != null ? ((runtime.dereference(members.m_audiocpu)).set_input_line?.(-2, state) ?? 0) : 0);
    members.m_sreset_old = ((((Number(1) === Number(state)) ? 1 : 0)) | 0);
  }

  function method_bublbobl_nmitrigger_w(runtime: any, data: any): any {
    const members = runtime.members;
    const __l = runtime.links ?? runtime.calls;

    (__l["m_subcpu.pulse_input_line"] ? __l["m_subcpu.pulse_input_line"](-1, 0) : (members.m_subcpu) != null ? ((runtime.dereference(members.m_subcpu)).pulse_input_line?.(-1, 0) ?? 0) : 0);
  }

  function method_mcram_vect_r(runtime: any, irqline: any): any {
    const members = runtime.members;
    const __l = runtime.links ?? runtime.calls;
  ...
  function method_bublbobl_mcu_port3_w(runtime: any, data: any): any {
    const members = runtime.members;

    members.m_port3_out = ((data) & 0xff);
  }

  function method_bublbobl_mcu_port3_r(runtime: any): any {
    const members = runtime.members;

    return (members.m_port3_in ?? runtime.member("m_port3_in"));
  }

  function method_bublbobl_mcu_port4_w(runtime: any, data: any): any {
    const members = runtime.members;
...
defined.compiledHandlerLinks = ["m_audiocpu.set_input_line","m_main_to_sound.pending_r","m_maincpu.set_input_line","m_sound_to_main.acknowledge_w","m_sound_to_main.pending_r","m_soundnmi.in_w_0","m_subcpu.pulse_input_line","m_ym2203.reset","m_ym3526.reset"];
export default {
  machine: defined,
  createBoard: (
    config: BoardConfig,
