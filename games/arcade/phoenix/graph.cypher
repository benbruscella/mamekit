...
// generated 2026-09-24T02:29:18.425Z
...
MERGE (n:KG {id: 'handler:phoenix_state.screen_update_phoenix'}) SET n:Handler SET n += {method: 'screen_update_phoenix', ownerClass: 'phoenix_state', sourceFile: 'src/mame/phoenix/phoenix_v.cpp', sourceLine: 434, sourceColumn: 1, sourceEndLine: 439, sourceParameters: 'screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect', sourceBody: 'm_bg_tilemap->draw(screen, bitmap, cliprect, 0,0);
	...
	return 0;'};
MERGE (n:KG {id: 'device:phoenix_state.phoenix/gfxdecode'}) SET n:Device SET n += {type: 'GFXDECODE', tag: 'gfxdecode', clock: null, config: ['GFXDECODE(config, m_gfxdecode, m_palette, gfx_phoenix)'], member: 'm_gfxdecode', sourceFile: 'src/mame/phoenix/phoenix.cpp', sourceLine: 467, sourceColumn: 2, sourceEndLine: 467, clockExpr: 'm_palette'};
MERGE (n:KG {id: 'device:phoenix_state.phoenix/palette'}) SET n:Device SET n += {type: 'PALETTE', tag: 'palette', clock: null, config: ['PALETTE(config, m_palette, FUNC(phoenix_state::phoenix_palette), 256)'], member: 'm_palette', sourceFile: 'src/mame/phoenix/phoenix.cpp', sourceLine: 468, sourceColumn: 2, sourceEndLine: 468, clockExpr: 'FUNC(phoenix_state::phoenix_palette)', paletteEntries: 256};
...
MERGE (n:KG {id: 'device:phoenix_state.phoenix/cust'}) SET n:Device SET n += {type: 'PHOENIX_SOUND', tag: 'cust', clock: 0, config: ['PHOENIX_SOUND(config, "cust").add_route(ALL_OUTPUTS, "mono", 0.4)'], cls: 'phoenix_sound_device', clsHierarchy: ['phoenix_sound_device'], sourceFile: 'src/mame/phoenix/phoenix.cpp', sourceLine: 479, sourceColumn: 2, sourceEndLine: 479, startHandler: 'phoenix_sound_device.device_start'};
...
MERGE (n:KG {id: 'handler:phoenix_sound_device.device_start'}) SET n:Handler SET n += {method: 'device_start', ownerClass: 'phoenix_sound_device', sourceFile: 'src/mame/phoenix/phoenix_a.cpp', sourceLine: 65, sourceColumn: 1, sourceEndLine: 104, sourceParameters: '', sourceBody: 'int i, j;
	uint32_t shiftreg;
...
MATCH (a:KG {id: 'device:phoenix_state.phoenix/cust'}), (b:KG {id: 'handler:phoenix_sound_device.device_start'}) MERGE (a)-[r:CALLS_HANDLER]->(b);
