# Space Panic (version E)

**Universal · 1980** — transpiled from the MAME driver `src/mame/universal/cosmic.cpp` by mamekit.

![marquee](/artwork/media/marquees/panic.webp)

| Cover | Cabinet |
| --- | --- |
| ![flyer](/artwork/covers/panic.webp) | ![cabinet](/artwork/media/cabinets/panic.webp) |

## The machine

| CPU | Type | Clock | Mapped ranges |
| --- | --- | --- | --- |
| `maincpu` | Z80 | 1.803 MHz | 11 |

- **Sound:** samples × 1 @ 1.803 MHz
- **Screen:** 256×192 @ 58.64 Hz · rotated 270°

### ROM chips

| Region | Chip | Offset | Size | CRC |
| --- | --- | --- | --- | --- |
| `maincpu` | `spe1` | 0x0 | 0x800 | `70ac0888` |
| `maincpu` | `spe2` | 0x800 | 0x800 | `2b910c48` |
| `maincpu` | `spe3` | 0x1000 | 0x800 | `03810148` |
| `maincpu` | `spe4` | 0x1800 | 0x800 | `119bbbfd` |
| `maincpu` | `spcpanic.5` | 0x2000 | 0x800 | `5b80f277` |
| `maincpu` | `spcpanic.6` | 0x2800 | 0x800 | `b73babf0` |
| `maincpu` | `spe7` | 0x3000 | 0x800 | `2894106e` |
| `gfx1` | `spcpanic.11` | 0x0 | 0x800 | `acea9df4` |
| `gfx1` | `spcpanic.12` | 0x800 | 0x800 | `e83423d0` |
| `gfx1` | `spcpanic.10` | 0x1000 | 0x800 | `c9631c2d` |
| `gfx1` | `spcpanic.9` | 0x1800 | 0x800 | `eec78b4c` |
| `proms` | `82s123.sp` | 0x0 | 0x20 | `35d43d2f` |
| `user1` | `spcpanic.8` | 0x0 | 0x800 | `7da0b321` |

## Controls

| Key | Function | Port | Bit |
| --- | --- | --- | --- |
| Space / X | button1 | `P1` | 0x1 |
| Right | joystick right | `P1` | 0x2 |
| Left | joystick left | `P1` | 0x4 |
| Down | joystick down | `P1` | 0x8 |
| Up | joystick up | `P1` | 0x10 |
| Z | button2 | `P1` | 0x80 |
| 1 | start1 | `SYSTEM` | 0x1 |
| 2 | start2 | `SYSTEM` | 0x2 |
| 5 | coin1 | `SYSTEM` | 0x40 |
| 6 | coin2 | `SYSTEM` | 0x80 |

## DIP switches (factory defaults)

| Setting | Port | Mask | Default |
| --- | --- | --- | --- |
| Coin A | `DSW` | 0x7 | 0x0 |
| Cabinet | `DSW` | 0x8 | 0x0 |
| Bonus Life | `DSW` | 0x10 | 0x0 |
| Lives | `DSW` | 0x20 | 0x0 |
| Coin B | `DSW` | 0xc0 | 0x40 |

## The MAME driver — the people who reverse-engineered it

- **Driver source:** `src/mame/universal/cosmic.cpp`
- **Written by:** Nicola Salmoria, Lee Taylor
- **License:** BSD-3-Clause
- **Credited by MAMEDEV:** Lee Taylor (driver header); Nicola Salmoria (driver header); ClawGrip (2 release notes); David Bosque (1 release note); Gerg Kennedy (1 release note); hap (1 release note); jkburks (1 release note); Peter Ferrie (1 release note); Retromaniacs (1 release note)
- **Credit source:** MAME driver header (copyright-holders) and MAMEDEV release notes, release notes 0.100–0.289
- **Commit activity:** 196 commits by 33 commit authors, 2007–2026 (git log --follow over the driver file at MAME f34f02505e32; not a statement of authorship)

## The story

Arcade Video game published 46 years ago:

Space Panic (c) 1980 Universal Company, Limited.

Influential computer game designer and writer Chris Crawford calls Space Panic "the granddaddy of all platform games", as it predates Nintendo's Donkey Kong (from 1981) which is often cited as the original platform game.

Space Panic lacks Donkey Kong's jump mechanic - the main character instead digs holes in the platforms into which he must lure the aliens. He must then hit them to knock them out of the hole and off the screen. In later levels, two or more holes must be lined up vertically in order to dispose of the aliens. There is also a limited supply of oxygen, which functions as a timer.

### Technical
Main CPU : Zilog Z80 (@ 1.802666 Mhz)
Sound Chips : DAC

Players : 2
Control : 4-way joystick
Buttons : 2

### Trivia
Space Panic was released in November 1980 in Japan and in February 1981 in the USA. It was the first platform game and like the title Heiyanko Alien, employs a digging mechanic. This digging into bricks ability would be borrowed my the computer and arcade game Lode Runner a couple of years later.

As was typical of Universal at the time, the cabinet came beautifully detailed in colorful artwork. This included art around the coin door and extended above the monitor.

Licensed to ADP Automaten for the German market.
Licensed to Inder for the Spanish market.

### Ports
* CONSOLES:
[US] Colecovision (1983) "Space Panic [Model 2447]"
[JP] Casio PV-1000

* COMPUTERS:
[US] Apple II (1981) "Apple Panic"
Microbee (198?) "Apple Panic"
PC-DOS (1982) "Apple Panic"
Vic-20 (1982) "Apple Panic [Model VI-AP-C]"
[US] TRS-80 (1982) "Apple Panic"
Atari XE (1982) "Apple Panic"
[US] Apple II (1983) "Apple Panic"
[US] VTech Laser-VZ "Vz Panik"

### Series
Cosmic Series

1. Cosmic Monsters [Upright model] (1979)
2. Cosmic Monsters 2 (1979)
3. Cosmic Guerilla (1979)
4. Cosmic Alien (1980)
5. Devil Zone (1980)
6. Zero Hour (1980)
7. Space Panic (1980)
8. Cosmic Avenger (1981)

### Contribute
Edit this entry: https://www.arcade-history.com/game/2551/?o=2

*Story courtesy of Gaming History (arcade-history.com).*

---

*Generated by [mamekit](https://github.com/benbruscella/mamekit) from the knowledge graph of MAME driver `cosmic`. Play it at [../../../app/g/panic/](../../../app/g/panic/) or [explore the knowledge graph](viewer.html).*
