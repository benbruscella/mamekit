...
// generated 2026-09-24T02:30:45.703Z
...
MERGE (n:KG {id: 'handler:travrusa_state.get_tile_info'}) SET n:Handler SET n += {method: 'get_tile_info', ownerClass: 'travrusa_state', sourceFile: 'src/mame/irem/travrusa.cpp', sourceLine: 295, sourceColumn: 1, sourceEndLine: 306, sourceParameters: 'tilemap_t &tilemap, tile_data &tileinfo, tilemap_memory_index tile_index', sourceBody: 'uint8_t const attr = m_videoram[2 * tile_index + 1];
	...
	tileinfo.set(0,
			...
			attr & 0x0f,
			flags);'};
MERGE (n:KG {id: 'device:travrusa_state.travrusa/maincpu'}) SET n:Device SET n += {type: 'Z80', tag: 'maincpu', clock: 3072000, config: ['Z80(config, m_maincpu, 18.432_MHz_XTAL / 6)', 'm_maincpu->set_addrmap(AS_PROGRAM, &travrusa_state::program_map)'], member: 'm_maincpu', sourceFile: 'src/mame/irem/travrusa.cpp', sourceLine: 704, sourceColumn: 2, sourceEndLine: 704};
MERGE (n:KG {id: 'device:travrusa_state.travrusa/screen'}) SET n:Device SET n += {type: 'SCREEN', tag: 'screen', clock: null, config: ['screen_device &screen(SCREEN(config, "screen", SCREEN_TYPE_RASTER))', 'screen.set_raw(18.432_MHz_XTAL / 3, 384, 8, 248, 282, 0, 256)', 'screen.set_screen_update(FUNC(travrusa_state::screen_update))', 'screen.set_palette(m_palette)', 'screen.screen_vblank().set_inputline(m_maincpu, INPUT_LINE_IRQ0)'], sourceFile: 'src/mame/irem/travrusa.cpp', sourceLine: 708, sourceColumn: 2, sourceEndLine: 708, configCalls: ['set_raw(6144000,384,8,248,282,0,256)', 'set_palette("palette")'], clockExpr: 'SCREEN_TYPE_RASTER', screenRaw: [6144000, 384, 8, 248, 282, 0, 256], screenRawExpr: ['18.432_MHz_XTAL / 3', '384', '8', '248', '282', '0', '256']};
...
MERGE (n:KG {id: 'device:travrusa_state.travrusa/gfxdecode'}) SET n:Device SET n += {type: 'GFXDECODE', tag: 'gfxdecode', clock: null, config: ['GFXDECODE(config, m_gfxdecode, m_palette, gfx_travrusa)'], member: 'm_gfxdecode', sourceFile: 'src/mame/irem/travrusa.cpp', sourceLine: 717, sourceColumn: 2, sourceEndLine: 717, clockExpr: 'm_palette'};
MERGE (n:KG {id: 'device:travrusa_state.travrusa/palette'}) SET n:Device SET n += {type: 'PALETTE', tag: 'palette', clock: null, config: ['PALETTE(config, m_palette, FUNC(travrusa_state::travrusa_palette), 16*8+16*8, 128+16)'], member: 'm_palette', sourceFile: 'src/mame/irem/travrusa.cpp', sourceLine: 718, sourceColumn: 2, sourceEndLine: 718, clockExpr: 'FUNC(travrusa_state::travrusa_palette)', paletteEntries: 144};
MERGE (n:KG {id: 'device:travrusa_state.travrusa/irem_audio'}) SET n:Device SET n += {type: 'IREM_M52_SOUNDC_AUDIO', tag: 'irem_audio', clock: 0, config: ['IREM_M52_SOUNDC_AUDIO(config, "irem_audio")'], cls: 'm52_soundc_audio_device', clsHierarchy: ['m52_soundc_audio_device', 'irem_audio_device'], sourceFile: 'src/mame/irem/travrusa.cpp', sourceLine: 721, sourceColumn: 2, sourceEndLine: 721, startHandler: 'irem_audio_device.device_start'};
...
MATCH (a:KG {id: 'device:travrusa_state.travrusa/irem_audio'}), (b:KG {id: 'handler:irem_audio_device.device_start'}) MERGE (a)-[r:CALLS_HANDLER]->(b);
