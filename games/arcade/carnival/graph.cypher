...
// generated 2026-09-24T02:26:22.384Z
...
MERGE (n:KG {id: 'device:carnival_state.carnivala_audio/psg'}) SET n:Device SET n += {type: 'AY8912', tag: 'psg', clock: 1193181.6666666667, config: ['AY8912(config, m_psg, XTAL(3\'579\'545)/3).add_route(ALL_OUTPUTS, "mono", 0.25)'], member: 'm_psg', sourceFile: 'src/mame/sega/vicdual.cpp', sourceLine: 291, sourceColumn: 2, sourceEndLine: 291};
...
MERGE (n:KG {id: 'device:vicdual_state.vicdual_root/maincpu'}) SET n:Device SET n += {type: 'Z80', tag: 'maincpu', clock: 1933560, config: ['Z80(config, m_maincpu, VICDUAL_MAIN_CPU_CLOCK)'], member: 'm_maincpu', sourceFile: 'src/mame/sega/vicdual.cpp', sourceLine: 921, sourceColumn: 2, sourceEndLine: 921};
MERGE (n:KG {id: 'device:vicdual_state.vicdual_root/coinstate'}) SET n:Device SET n += {type: 'TIMER', tag: 'coinstate', clock: null, config: ['TIMER(config, m_coinstate_timer).configure_generic(FUNC(vicdual_state::clear_coin_status))'], member: 'm_coinstate_timer', sourceFile: 'src/mame/sega/vicdual.cpp', sourceLine: 923, sourceColumn: 2, sourceEndLine: 923};
MERGE (n:KG {id: 'device:vicdual_state.vicdual_root/coinstate/callback:coinstate:0'}) SET n:Callback SET n += {signal: 'configure_generic', operation: 'configure_generic', raw: 'TIMER(config, m_coinstate_timer).configure_generic(FUNC(vicdual_state::clear_coin_status))', ownerTag: 'coinstate', sourceFile: 'src/mame/sega/vicdual.cpp', sourceLine: 923, sourceColumn: 2, sourceEndLine: 923, targetClass: 'vicdual_state', targetMethod: 'clear_coin_status'};
MERGE (n:KG {id: 'handler:vicdual_state.clear_coin_status'}) SET n:Handler SET n += {method: 'clear_coin_status', ownerClass: 'vicdual_state', sourceFile: 'src/mame/sega/vicdual.cpp', sourceLine: 709, sourceColumn: 1, sourceEndLine: 712, sourceParameters: 'int param', sourceBody: 'm_coin_status = 0;'};
MERGE (n:KG {id: 'device:vicdual_state.vicdual_root/screen'}) SET n:Device SET n += {type: 'SCREEN', tag: 'screen', clock: null, config: ['SCREEN(config, m_screen, SCREEN_TYPE_RASTER)', 'm_screen->set_raw(VICDUAL_PIXEL_CLOCK, VICDUAL_HTOTAL, VICDUAL_HBEND, VICDUAL_HBSTART, VICDUAL_VTOTAL, VICDUAL_VBEND, VICDUAL_VBSTART)', 'm_screen->set_screen_update(FUNC(vicdual_state::screen_update_color))'], member: 'm_screen', sourceFile: 'src/mame/sega/vicdual.cpp', sourceLine: 926, sourceColumn: 2, sourceEndLine: 926, configCalls: ['set_raw(5156160,328,0,256,262,0,224)'], clockExpr: 'SCREEN_TYPE_RASTER', screenRaw: [5156160, 328, 0, 256, 262, 0, 224], screenRawExpr: ['VICDUAL_PIXEL_CLOCK', 'VICDUAL_HTOTAL', 'VICDUAL_HBEND', 'VICDUAL_HBSTART', 'VICDUAL_VTOTAL', 'VICDUAL_VBEND', 'VICDUAL_VBSTART']};
...
MATCH (a:KG {id: 'device:vicdual_state.vicdual_root/coinstate'}), (b:KG {id: 'device:vicdual_state.vicdual_root/coinstate/callback:coinstate:0'}) MERGE (a)-[r:HAS_CALLBACK]->(b);
...
MATCH (a:KG {id: 'device:vicdual_state.vicdual_root/coinstate/callback:coinstate:0'}), (b:KG {id: 'handler:vicdual_state.clear_coin_status'}) MERGE (a)-[r:CALLS_HANDLER]->(b);
