...
export class Namco06 {
    ...
    cycleAccum = 0;
    readStretch = false;
    constructor(clock, cpuClock, nmi, slots) {
        this.clock = clock;
    ...
    dataRead() {
        if (!(this.control & 0x10))
            return 0; // read in write mode
        let result = 0xff;
        if (this.control & 1)
            result &= this.slots[0]?.read?.() ?? 0xff;
        if (this.control & 2)
            result &= this.slots[1]?.read?.() ?? 0xff;
        if (this.control & 4)
            result &= this.slots[2]?.read?.() ?? 0xff;
        if (this.control & 8)
            result &= this.slots[3]?.read?.() ?? 0xff;
        return result;
    }
    dataWrite(data) {
        if (this.control & 0x10)
            return; // write in read mode
        if (this.control & 1)
            this.slots[0]?.write?.(data);
        if (this.control & 2)
            this.slots[1]?.write?.(data);
        if (this.control & 4)
            this.slots[2]?.write?.(data);
        if (this.control & 8)
            this.slots[3]?.write?.(data);
    }
    /** advance by CPU cycles; pulses NMI to the main CPU at the divided rate */
    tick(cpuCycles) {
        if (!this.cpuCyclesPerNmi)
            return;
