...
export function absSinAttenuation(input) {
 ...
 */
export function attenuationToVolume(input) {
...
export function attenuationIncrement(rate, index) {
...
export function detuneAdjustment(detune, keycode) {
 ...
 */
export function opnKeycode(blockFreq) {
 ...
 */
export function opnPhaseStep(blockFreq, detune, multipleX1) {
 ...
 */
export function roundtripFp(value) {
    if (value < -32768)
        return -32768;
    if (value > 32767)
        return 32767;
...
const WAVEFORM = (() => {
    ...
    for (let i = 0; i < 1024; i++)
        t[i] = absSinAttenuation(i) | (((i >> 9) & 1) << 15);
    return t;
})();
/** effective_rate (ymfm_fm.h): apply KSR to a raw ADSR rate. */
function effectiveRate(rawrate, ksr) {
...
export class YM2203 {
    ...
    writeAddress(data) {
        ...
        if (this.address === 0x2d)
            this.updatePrescale(6);
        else if (this.address === 0x2e && this.prescaleValue === 6)
            this.updatePrescale(3);
        else if (this.address === 0x2f)
            this.updatePrescale(2);
    }
    ...
    prepareOperator(op) {
        ...
        if ((keystate ^ op.keyState) !== 0) {
            ...
            if (keystate !== 0)
                ...
                this.startRelease(op);
        }
    }
    /** opn_registers_base::cache_operator_data */
    cacheOperatorData(op) {
        ...
        if (multiFreq && op.choffs === 2) {
            if (opoffs === 2)
                blockFreq = ((regs[0xad] & 0x3f) << 8) | regs[0xa9];
            else if (opoffs === 10)
                blockFreq = ((regs[0xae] & 0x3f) << 8) | regs[0xaa];
            else if (opoffs === 6)
                blockFreq = ((regs[0xac] & 0x3f) << 8) | regs[0xa8];
        }
    ...
    startAttack(op, isRestart = false) {
        ...
        if (op.cacheEgRate[EG_ATTACK] >= 62)
            op.envAttenuation = 0;
    }
    /** fm_operator::start_release */
    startRelease(op) {
        if (op.envState >= EG_RELEASE)
            return;
    ...
    /** fm_operator::clock */
    clockOperator(op) {
        if ((this.regs[0x90 + op.opoffs] >> 3) & 1)
            this.clockSsgEgState(op);
        else
            op.ssgInverted = false;
        // envelope clocks when the low 2 bits of the counter are 0
        if ((this.envCounter & 3) === 0)
            this.clockEnvelope(op, this.envCounter >>> 2);
    ...
    clockSsgEgState(op) {
        ...
        if (mode & 1) {
            ...
            if (op.envState !== EG_ATTACK)
                op.envAttenuation = op.ssgInverted ? 0x200 : 0x3ff;
        }
        else {
            // continuous modes (0/2/4/6)
            if ((mode >> 1) & 1)
                op.ssgInverted = !op.ssgInverted;
            if (op.envState === EG_DECAY || op.envState === EG_SUSTAIN)
                this.startAttack(op, true);
            if (((mode >> 1) & 1) === 0)
                op.phase = 0;
        }
        if (op.envState === EG_RELEASE)
            op.envAttenuation = 0x3ff;
    }
    /** fm_operator::clock_envelope */
    clockEnvelope(op, envCounter) {
        if (op.envState === EG_ATTACK && op.envAttenuation === 0)
            op.envState = EG_DECAY;
        if (op.envState === EG_DECAY && op.envAttenuation >= op.cacheEgSustain)
            op.envState = EG_SUSTAIN;
        ...
        if (op.envState === EG_ATTACK) {
            ...
            if (rate < 62)
                op.envAttenuation += (~op.envAttenuation * increment) >> 4;
        }
        else {
            const ssgEg = ((this.regs[0x90 + op.opoffs] >> 3) & 1) !== 0;
            if (!ssgEg)
                op.envAttenuation += increment;
            else if (op.envAttenuation < 0x200)
                op.envAttenuation += 4 * increment;
            if (op.envAttenuation >= 0x400)
                op.envAttenuation = 0x3ff;
        }
    }
    /** fm_operator::envelope_attenuation (no KSL shift or LFO AM on OPN). */
    envelopeAttenuation(op) {
    ...
    computeVolume(op, phase) {
        if (op.envAttenuation > EG_QUIET)
            return 0;
    ...
    outputFm() {
        ...
        for (const ch of this.channels) {
            ...
            let opmod = 0;
            if (feedback !== 0)
                opmod = (ch.feedback0 + ch.feedback1) >> (10 - feedback);
            ...
            if (algorithmOps & 0x080)
                result = clamp(result + opout[1], -32768, 32767);
            if (algorithmOps & 0x100)
                result = clamp(result + opout[2], -32768, 32767);
            if (algorithmOps & 0x200)
                result = clamp(result + opout[3], -32768, 32767);
            sum += result;
        }
