...
export class Z80 {
    ...
    setIrqLine(active, dataBus = 0xff) {
        this.irqLine = active;
        if (active)
    ...
    getRX(idx, useIX) {
        if (idx === 4)
            return (useIX ? this.ix : this.iy) >> 8;
        if (idx === 5)
            return (useIX ? this.ix : this.iy) & 0xff;
    ...
    setRX(idx, v, useIX) {
        ...
        if (idx === 4) {
            if (useIX)
                this.ix = (this.ix & 0x00ff) | (v << 8);
            else
                this.iy = (this.iy & 0x00ff) | (v << 8);
        }
        else if (idx === 5) {
            if (useIX)
                this.ix = (this.ix & 0xff00) | v;
            else
                this.iy = (this.iy & 0xff00) | v;
        }
        else {
            this.setR8(idx, v);
        }
    ...
    daa() {
        ...
        let res = a;
        if (this.f & NF) {
            if ((this.f & HF) !== 0 || (a & 0x0f) > 9)
                res -= 6;
            if ((this.f & CF) !== 0 || a > 0x99)
                res -= 0x60;
        }
        else {
            if ((this.f & HF) !== 0 || (a & 0x0f) > 9)
        ...
        if (op >= 0x40 && op < 0x80) {
            ...
            switch (op & 7) {
                ...
                case 3: { // LD (nn),rr / LD rr,(nn)
                    ...
                    this.wz = (addr + 1) & 0xffff;
                    if (y & 1)
                        this.setRP(y >> 1, this.rd16(addr));
                    else
                        this.wr16(addr, this.rp(y >> 1));
                    return 20;
                }
