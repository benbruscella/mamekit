 ...
 */
export const AY8910_VOL_TABLE = (() => {
    const t = new Float32Array(16);
    for (let v = 1; v < 16; v++)
        t[v] = Math.pow(1.188502227, 2 * (v - 15));
    return t;
})();
export class AY8910 {
    ...
    envPeriod = 0; // 16-bit (reg 11 | reg 12 << 8)
    envCount = 0;
    envStep = 0; // counts 15 -> 0 each ramp
    envAttack = 0; // 0x00 or 0x0f, XORed into the step
    envHold = 0;
    envAlternate = 0;
    envHolding = 0;
    constructor(clock) {
    ...
    writeReg(reg, value) {
        ...
        switch (reg) {
            ...
            case AY_PORTA: // write reaches the pins only in output mode
                if (this.regs[AY_ENABLE] & 0x40)
                    this.portAWrite?.(value);
                break;
            case AY_PORTB:
                if (this.regs[AY_ENABLE] & 0x80)
                    this.portBWrite?.(value);
                break;
        }
    }
    /**
    ...
    tick() {
            ...
            if (++this.envCount >= this.envPeriod * 2) {
                ...
                if (this.envStep < 0) {
                    if (this.envHold) {
                        ...
                        if (this.envAlternate && (this.envStep & 0x10))
                            this.envAttack ^= 0x0f;
                        this.envStep &= 0x0f;
                    }
                }
        ...
        for (let ch = 0; ch < 3; ch++) {
            const tone = tones[ch];
            const gate = (tone.output | ((enable >> ch) & 1)) &
 ...
 */
export function konamiFilterCaps(data) {
    const caps = [0, 0, 0];
    for (let i = 0; i < 3; i++) {
        let c = 0;
        if (data & 1)
            c += 47000; // 47000 pF = 0.047 uF
        if (data & 2)
            c += 220000; // 220000 pF = 0.22 uF
        data >>= 2;
        caps[i] = c;
    }
    return caps;
 ...
 */
export function lowpass3RCoeff(r1, r2, r3, capPf, sampleRate) {
    if (capPf === 0)
        return 1;
 ...
 */
export function rcLowPass(buf, k, memory) {
 ...
 */
export function renderBank(chips, out, scratch) {
    out.fill(0);
    if (chips.length === 0)
        return;
