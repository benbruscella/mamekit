...
const PRESCALER_BY_SEL = [96, 48, 64, 0, 96, 48, 64, 0];
export class MSM5205 {
    clock;
    sampleRate; // stream runs at master-clock rate, like MAME
    /** Fires with the new VCK state (1=rise, 0=fall) from tick(). */
    vckCallback = null;
    diff = new Int16Array(49 * 16); // diff_lookup
    signalV = 0; // -2048..2047
    stepV = 0; // 0..48
    data = 0; // latched nibble (already width-masked)
    resetPin = false;
    vck = 0;
    prescaler = 96;
    bitwidth = 4;
    phase = 0; // master clocks since last VCK toggle
    ...
    vclkW(state) {
        if (this.prescaler !== 0)
            return; // master mode: pin ignored (MAME logs an error)
    ...
    tick(masterClocks) {
        ...
        if (this.prescaler === 0)
            return; // slave mode: clocked via vclkW
        ...
        while (this.phase >= half) {
            ...
            this.vck ^= 1;
            if (this.vck === 0)
                this.decode(this.data); // capture before the CPU can react
            if (this.vckCallback !== null)
                this.vckCallback(this.vck);
        }
    }
    ...
    fifoPush(v) {
        if (this.fifo.length - this.fifoHead >= 4096)
            return; // safety cap
        this.fifo.push(v);
    }
    fifoPop() {
