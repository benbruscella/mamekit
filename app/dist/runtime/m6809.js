...
export class M6809 {
    ...
    firqLine = false;
    nmiPending = false;
    ldsEncountered = false; // NMI inhibited until S loaded after reset
    syncWait = false;
    cwaiWait = false;
    cy = 0; // cycle accumulator for the current step
    ea = 0;
    mode = MODE_IMM;
    ...
    step() {
        ...
        if (this.syncWait) {
            ...
            if (!this.nmiPending && !this.firqLine && !this.irqLine)
                return 1;
            this.syncWait = false;
            this.halted = false;
    ...
    fetchOp() {
        ...
        if (this.opcodeXform !== null)
            v = this.opcodeXform(this.pc, v) & 0xff;
    ...
    flags8(mask, a, b, r) {
        let cc = this.cc & ~mask;
        if ((mask & CC_H) !== 0 && ((a ^ b ^ r) & 0x10) !== 0)
            cc |= CC_H;
        if ((mask & CC_N) !== 0 && (r & 0x80) !== 0)
            cc |= CC_N;
        if ((mask & CC_Z) !== 0 && (r & 0xff) === 0)
            cc |= CC_Z;
        if ((mask & CC_V) !== 0 && ((a ^ b ^ r ^ (r >> 1)) & 0x80) !== 0)
            cc |= CC_V;
        if ((mask & CC_C) !== 0 && (r & 0x100) !== 0)
            cc |= CC_C;
        this.cc = cc;
        return r & 0xff;
    }
    ...
    flags16(mask, a, b, r) {
        let cc = this.cc & ~mask;
        if ((mask & CC_N) !== 0 && (r & 0x8000) !== 0)
            cc |= CC_N;
        if ((mask & CC_Z) !== 0 && (r & 0xffff) === 0)
            cc |= CC_Z;
        if ((mask & CC_V) !== 0 && ((a ^ b ^ r ^ (r >> 1)) & 0x8000) !== 0)
            cc |= CC_V;
        if ((mask & CC_C) !== 0 && (r & 0x10000) !== 0)
            cc |= CC_C;
        this.cc = cc;
        return r & 0xffff;
    }
    ...
    readOperand16() {
        if (this.mode === MODE_EA)
            return (this.rd(this.ea) << 8) | this.rd(this.ea + 1);
    ...
    pushRegs(sp, mask, other) {
        if ((mask & 0x80) !== 0) {
            sp = (sp - 1) & 0xffff;
            this.wr(sp, this.pc & 0xff);
            sp = (sp - 1) & 0xffff;
            this.wr(sp, this.pc >> 8);
        }
        if ((mask & 0x40) !== 0) {
            sp = (sp - 1) & 0xffff;
            this.wr(sp, other & 0xff);
            sp = (sp - 1) & 0xffff;
            this.wr(sp, other >> 8);
        }
        if ((mask & 0x20) !== 0) {
            sp = (sp - 1) & 0xffff;
            this.wr(sp, this.y & 0xff);
            sp = (sp - 1) & 0xffff;
            this.wr(sp, this.y >> 8);
        }
        if ((mask & 0x10) !== 0) {
            sp = (sp - 1) & 0xffff;
            this.wr(sp, this.x & 0xff);
            sp = (sp - 1) & 0xffff;
            this.wr(sp, this.x >> 8);
        }
        if ((mask & 0x08) !== 0) {
    ...
    pendingVector() {
        if (this.nmiPending)
            return VEC_NMI;
        if ((this.cc & CC_F) === 0 && this.firqLine)
            return VEC_FIRQ;
        if ((this.cc & CC_I) === 0 && this.irqLine)
            return VEC_IRQ;
        return 0;
    }
    ...
    exec3x(op) {
        switch (op) {
            ...
            case 0x3c: { // CWAI: CC &= imm, push entire state, wait
                ...
                const vec = this.pendingVector();
                if (vec !== 0)
                    this.cwaiDispatch(vec); // fast dispatch, no wait
                else {
                    this.cwaiWait = true;
                    this.halted = true;
                }
    ...
    execPage2() {
        ...
        while (op === 0x10 || op === 0x11)
            op = this.fetchOp(); // stay in page 2 (MAME DISPATCH10)
        if (op >= 0x20 && op <= 0x2f) {
    ...
    execPage3() {
        let op = this.fetchOp();
        while (op === 0x10 || op === 0x11)
            op = this.fetchOp(); // stay in page 3 (MAME DISPATCH11)
        switch (op) {
    ...
    rmw(sub) {
        switch (sub) {
            ...
            case 0x4:
            case 0x5: { // LSR: C = bit0, N cleared, V untouched
                const m = this.readOperand();
    ...
    colOp(op) {
        ...
        if (op === 0x87 || op === 0x8f || op === 0xc7 || op === 0xcd || op === 0xcf)
            return;
        if (op === 0x8d) { // BSR
        ...
        switch (sub) {
            ...
            case 0xc:
                this.amCol(op);
                if (rowB)
                    this.setD(this.nz16v0(this.readOperand16())); // LDD
                else
                    this.cmp16(this.x); // CMPX
                return;
            case 0xd:
                this.amCol(op);
                if (rowB)
                    this.st16(this.getD()); // STD
                else { // JSR
                    this.idle(2);
                    this.gosub(this.ea);
                }
            ...
            default: { // 8-bit ALU on A (rows 8-B) or B (rows C-F)
                ...
                switch (sub) {
                    case 0x0:
                        v = this.flags8(CC_NZVC, reg, m, reg - m);
                        break; // SUB
                    case 0x1:
                        this.flags8(CC_NZVC, reg, m, reg - m);
                        return; // CMP
                    case 0x2:
                        v = this.flags8(CC_NZVC, reg, m, reg - m - (this.cc & CC_C));
                        break; // SBC
                    case 0x4: // AND
                        this.cc &= ~CC_V;
                    ...
                    case 0x8: // EOR
                        ...
                        break;
                    case 0x9:
                        v = this.flags8(CC_HNZVC, reg, m, reg + m + (this.cc & CC_C));
                        break; // ADC
                    case 0xa: // OR
                        this.cc &= ~CC_V;
                        v = this.flags8(CC_NZ, 0, 0, reg | m);
                        break;
                    default:
                        v = this.flags8(CC_HNZVC, reg, m, reg + m);
                        break; // 0xb ADD
                }
                if (rowB)
                    this.b = v;
    ...
    daa() {
        ...
        const t = this.a + cf;
        if ((t & 0x100) !== 0)
            this.cc |= CC_C; // C is set, never cleared (MAME/hardware)
    ...
    tfr() {
        ...
        if ((param & 0x0f) === 4)
            this.ldsEncountered = true; // TFR ..,S arms NMI
        this.idle(4);
    }
    exg() {
        const param = this.fetchArg();
