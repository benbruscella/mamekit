...
export class M6803 {
    ...
    bus;
    ports;
    ram = new Uint8Array(0x80); // internal RAM $0080-$00FF
    irqLine = false;
    nmiPending = false;
    waiState = false;
    irqShadow = false; // TAP/CLI one-instruction interrupt delay
    ea = 0;
    cy = 0;
    ...
    pendingVector() {
        ...
        if (this.irqLine)
            return VEC_IRQ;
        const t = this.tcsr;
        if ((t & (TCSR_EICI | TCSR_ICF)) === (TCSR_EICI | TCSR_ICF))
            return VEC_ICI;
        if ((t & (TCSR_EOCI | TCSR_OCF)) === (TCSR_EOCI | TCSR_OCF))
            return VEC_OCI;
        if ((t & (TCSR_ETOI | TCSR_TOF)) === (TCSR_ETOI | TCSR_TOF))
            return VEC_TOI;
        const s = this.trcsr;
        if ((s & (TRCSR_RIE | TRCSR_RDRF)) === (TRCSR_RIE | TRCSR_RDRF) ||
    ...
    calcEA(mode) {
        if (mode === 1)
            ...
            this.ea = (this.x + this.fetch()) & 0xffff;
        else
            this.ea = this.fetch16();
    }
    rd8m(mode) {
        if (mode === 0)
            return this.fetch();
    ...
    exec(op) {
        switch (op >> 4) {
            ...
            case 0x3:
                ...
                return;
            case 0x4:
                this.rmwReg(op & 0x0f, false);
                return;
            case 0x5:
                this.rmwReg(op & 0x0f, true);
                return;
            case 0x6:
                this.rmwMem(op & 0x0f, true);
                return;
            case 0x7:
                this.rmwMem(op & 0x0f, false);
                return;
            default:
                this.colOp(op);
                return;
    ...
    exec1x(op) {
        switch (op) {
            case 0x10:
                this.a = this.flags8(this.a, this.b, this.a - this.b);
                return; // SBA
            case 0x11:
                this.flags8(this.a, this.b, this.a - this.b);
                return; // CBA
            case 0x16:
                this.b = this.nz8(this.a);
                return; // TAB
            case 0x17:
                this.a = this.nz8(this.b);
                return; // TBA
            case 0x19:
                this.daa();
                return;
            case 0x1b:
                this.a = this.flags8h(this.a, this.b, this.a + this.b);
                return; // ABA
    ...
    rmwReg(sub, isB) {
        if (!M6803.rmwLegal(sub) || sub === 0xe)
            return; // illegal (incl. $4E/$5E)
    ...
    rmwMem(sub, indexed) {
        if (!M6803.rmwLegal(sub))
            return; // illegal slots are 1-byte no-ops (MAME illegl1)
    ...
    colOp(op) {
        ...
        switch (sub) {
            ...
            case 0xc:
                if (rowB)
                    this.setD(this.nz16(this.rd16m(mode))); // LDD
            ...
            default: { // two-operand 8-bit ALU on A (rows 8-B) or B (rows C-F)
                ...
                switch (sub) {
                    case 0x0:
                        v = this.flags8(reg, m, reg - m);
                        break; // SUB
                    case 0x1:
                        this.flags8(reg, m, reg - m);
                        return; // CMP
                    case 0x2:
                        v = this.flags8(reg, m, reg - m - (this.cc & CC_C));
                        break; // SBC
                    case 0x4:
                        v = this.nz8(reg & m);
                        break; // AND
                    ...
                    case 0x8:
                        ...
                        break; // EOR
                    case 0x9:
                        v = this.flags8h(reg, m, reg + m + (this.cc & CC_C));
                        break; // ADC
                    case 0xa:
                        v = this.nz8(reg | m);
                        break; // OR
                    default:
                        v = this.flags8h(reg, m, reg + m);
                        break; // 0xb ADD
                }
                if (rowB)
                    this.b = v;
    ...
    daa() {
        ...
        let cf = 0;
        if (lsn > 0x09 || (this.cc & CC_H) !== 0)
            cf |= 0x06;
        if (msn > 0x80 && lsn > 0x09)
            cf |= 0x60;
        if (msn > 0x90 || (this.cc & CC_C) !== 0)
            cf |= 0x60;
        const t = this.a + cf;
    ...
    tickCounter(cycles) {
        ...
        if (this.counter >= this.timerNext)
            this.timerEvent();
    }
    timerEvent() {
    ...
    cleanupCounters() {
        ...
        if (cth === 0)
            return;
        const rebase = (v) => (((((v >>> 16) - cth) & 0xffff) * 0x10000) + (v & 0xffff)) >>> 0;
    ...
    ioRead(addr) {
        switch (addr) {
            ...
            case 0x09: // counter high; clears TOF if TCSR was read since it was set
                if ((this.pendingTcsr & TCSR_TOF) === 0)
                    this.tcsr &= ~TCSR_TOF;
            ...
            case 0x0d: // input capture high; clears ICF per protocol
                if ((this.pendingTcsr & TCSR_ICF) === 0)
                    this.tcsr &= ~TCSR_ICF;
            ...
            case 0x11: // TRCSR read arms RDR-read clearing of ORFE/RDRF
                if ((this.trcsr & TRCSR_ORFE) !== 0)
                    this.trcsrReadOrfe = true;
                if ((this.trcsr & TRCSR_RDRF) !== 0)
                    this.trcsrReadRdrf = true;
                return this.trcsr;
            case 0x12: // RDR
    ...
    ioWrite(addr, data) {
        switch (addr) {
            ...
            case 0x0b: { // OCR high; clears OCF if TCSR was read since it was set
                if ((this.pendingTcsr & TCSR_OCF) === 0)
                    this.tcsr &= ~TCSR_OCF;
                const oc = this.ocd & 0xffff;
                if ((oc >> 8) !== data) {
            ...
            case 0x10:
                this.rmcr = data;
                return; // SCI rate/mode: stored only (SCI stubbed)
    ...
    writePort2() {
        if (!this.port2Written)
            return;
        let data = this.p2;
        const ddr = this.ddr2 & 0x1f;
        if (ddr !== 0x1f && ddr !== 0)
            data = (this.p2 & ddr) | (ddr ^ 0xff);
        if ((this.trcsr & TRCSR_TE) !== 0)
            data = (data & 0xef) | 0x10; // TX idles high
        data &= 0x1f;
        if (this.ports.p2Write !== undefined)
            this.ports.p2Write(data);
    }
}
