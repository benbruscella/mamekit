 ...
 */
async function fetchRomSet(game) {
 ...
 */
export async function runShell(cfg, preloaded) {
    ...
    void loadArtwork(cfg.game, 'bezel').then(art => {
        if (art?.window)
            ui.setBezel(art.bmp, art.window);
    });
    ...
    window.mamekit = { board, input, config: cfg, audio };
    ...
    const tick = (now) => {
        ...
        if (now - fpsWindowStart >= 1000) {
            ...
            if (snap.cpus.length > 1)
                parts.push(`sub=${snap.cpus[1].held ? 'held' : hex4(snap.cpus[1].pc)}`);
            if (snap.credits !== undefined)
                parts.push(`credits=${snap.credits}`);
            if (input.debug)
                parts.push(input.dump());
...
// ---------------------------------------------------------------------------
function assembleRegions(specs, files, status, critical = new Set()) {
    ...
    for (const spec of specs) {
        ...
        for (const load of spec.loads) {
            ...
            for (const ro of load.reloadOffsets ?? [])
                bytes.set(f.subarray(0, load.size), ro);
        }
...
function buildDom(cfg) {
    ...
    return {
        overlay,
        status: (text) => { statusEl.textContent = text; if (overlay.style.display !== 'none' && !overlay.querySelector('[data-dropzone]'))
...
function waitForZip(ui, zone, specs, critical, game) {
    return new Promise(resolve => {
        ...
        pick.addEventListener('change', () => { if (pick.files?.[0])
            void handle(pick.files[0]); });
        ...
        zone.search.button.addEventListener('click', () => {
            ...
            fetchRomSet(game).then(raw => finish(() => {
