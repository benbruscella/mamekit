...
export class InvadersSound {
    ...
    ampOn = true;
    ampGain = 1; // smoothed gate
    ampK = lpK(80); // ~2 ms gate ramp
    // --- shared LFSR noise (midway_lfsr @ 7515 Hz, sample-and-hold) ------------
    lfsr = 0x1ffff;
    noiseOut = 0; // +/-1
    noiseFrac = 0;
    noiseStep = NOISE_CLOCK / RATE;
    // --- UFO (continuous) -------------------------------------------------------
    ufoOn = false;
    ufoEnv = 0; // click-avoidance ramp
    ufoLfoPhase = 0; // SLF triangle, cycles [0,1)
    ufoPhase = 0;
    // --- shot (one-shot) --------------------------------------------------------
    shotEnv = 0;
    shotT = 0; // seconds since trigger
    shotPhase = 0;
    shotDecay = decayK(0.12);
    // --- explosion (one-shot noise rumble) --------------------------------------
    explEnv = 0;
    explLp1 = 0;
    explLp2 = 0;
    explDecay = decayK(0.30);
    explLp1K = lpK(284);
    explLp2K = lpK(128);
    ...
    invhitT = 0;
    invhitPhase = 0;
    invhitDecay = decayK(0.08);
    // --- bonus base (gated) -------------------------------------------------------
    bonusOn = false;
    bonus555Phase = 0;
    bonusTonePhase = 0;
    // --- fleet thump (gated, pitch from data bits) --------------------------------
    fleetHz = 0; // 0 = no bits set
    fleetEnv = 0;
    fleetPhase = 0;
    fleetLp1 = 0;
    fleetLp2 = 0;
    fleetAttack = lpK(160); // ~1 ms-scale thump attack
    fleetRelease = decayK(0.035);
    fleetLp1K = lpK(FLEET_LP1_HZ);
    fleetLp2K = lpK(FLEET_LP2_HZ);
    // --- saucer hit (gated descending warble) ---------------------------------------
    saucerHitOn = false;
    saucerHitEnv = 0;
    saucerHitCenter = 1200;
    saucerHitLfoPhase = 0;
    saucerHitPhase = 0;
    saucerHitDrift = decayK(0.5); // 1200 -> 500 Hz drift
    saucerHitRelease = decayK(0.03);
    /** `clock` is accepted for the shared shell contract and ignored (RC board). */
    constructor(_clock = 1996800) { }
    /**
    ...
    write(offset, data) {
        ...
        if (offset === 0x51) {
            ...
            if (this.ufoOn && !(prev & 0x01))
                this.ufoLfoPhase = 0; // SLF cap starts low
    ...
    render(out) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (this.bonusOn) {
                ...
                if (this.bonus555Phase >= 1)
                    this.bonus555Phase -= 1;
                this.bonusTonePhase += BONUS_TONE_HZ * dt;
                if (this.bonusTonePhase >= 1)
                    this.bonusTonePhase -= 1;
            ...
            if (this.fleetHz > 0) {
                ...
                if (this.fleetPhase >= 1)
                    this.fleetPhase -= 1;
            }
            else {
                this.fleetEnv *= this.fleetRelease;
                if (this.fleetEnv < 1e-4)
                    this.fleetEnv = 0;
            }
            ...
            if (this.saucerHitEnv > 0) {
                ...
                if (this.saucerHitPhase >= 1)
                    this.saucerHitPhase -= 1;
