...
export class TimepltAudio {
    ...
    ays = [new AY8910(1_789_772), new AY8910(1_789_772)];
    ayAddr = [0, 0];
    /**
     ...
     */
    constructor(spec, rom, registry, shares, sinks, frac) {
        ...
        registry.write['timeplt_audio_device.filter_w'] = (_a, off) => {
            ...
            const chipByte = (c0, c1, c2) => swap2(c0) | (swap2(c1) << 2) | (swap2(c2) << 4);
    ...
    run(target) {
        ...
        if (total < target)
            total += this.cpu.run(target - total);
        this.cycles += total;
        return total;
    }
    reset() {
