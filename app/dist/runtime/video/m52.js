...
export function buildBgLayout() {
    const xOffsets = [];
    for (let x = 0; x < 256; x++)
        xOffsets.push((x >> 2) * 8 + (x & 3)); // STEP4(0x000,1), STEP4(0x008,1), ...
    const yOffsets = [];
    for (let y = 0; y < 128; y++)
        yOffsets.push((y >> 5) * 0x4000 + (y & 31) * 0x200); // STEP32(0,0x200) x4 banks
    return {
        width: 256,
        height: 128,
        total: 1,
...
export function buildM52Palette(regions) {
    ...
    for (let i = 0; i < 512; i++) {
        const v = txPal[i];
        txPens[i] = packRGB(combineWeights(wr, v & 1, (v >> 1) & 1, (v >> 2) & 1), combineWeights(wg, (v >> 3) & 1, (v >> 4) & 1, (v >> 5) & 1), combineWeights(wb, (v >> 6) & 1, (v >> 7) & 1));
    }
    // background indirect colors (m52.cpp:189-198): same weights/bit order
    const bgIndirect = new Uint32Array(32);
    for (let i = 0; i < 32; i++) {
        const v = bgPal[i];
        bgIndirect[i] = packRGB(combineWeights(wr, v & 1, (v >> 1) & 1, (v >> 2) & 1), combineWeights(wg, (v >> 3) & 1, (v >> 4) & 1, (v >> 5) & 1), combineWeights(wb, (v >> 6) & 1, (v >> 7) & 1));
    }
    const bgPens = new Uint32Array(12);
    for (let i = 0; i < 12; i++)
        bgPens[i] = bgIndirect[BG_PEN_MAP[i]];
    ...
    for (let i = 0; i < 32; i++) {
        const v = sprPal[i];
        spIndirect[i] = packRGB(combineWeights(swr, (v >> 6) & 1, (v >> 7) & 1), combineWeights(swg, (v >> 3) & 1, (v >> 4) & 1, (v >> 5) & 1), combineWeights(swb, v & 1, (v >> 1) & 1, (v >> 2) & 1));
    }
 ...
 */
function normalizeBgRegion(region) {
    let allZero = true;
...
export class M52Video {
     ...
     */
    drawBackground(frame, bgx, bgy, image, flip) {
