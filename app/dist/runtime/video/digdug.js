...
export function buildDigdugPalette(proms) {
...
export class DigdugVideo {
    ...
    flpram;
    videolatch;
    charGfx; // gfx1: 1bpp text/alpha
    spriteGfx; // gfx2: 16x16 sprites
    bgGfx; // gfx3: 8x8 bg tile shapes
    bgMap; // gfx4: bg tilemap code ROM (4 pages of 0x400)
    pal;
    constructor(deps) {
        ...
        if (!gfx1 || !gfx2 || !gfx3 || !gfx4 || !proms)
            throw new Error('digdug video: missing gfx1-4/proms region');
        if (proms.length < 0x220)
            throw new Error('digdug video: proms region too small');
    ...
    drawBg(frame, flip, bgSelect, bgColorBank, bgDisable) {
        const w = this.width;
     ...
     */
    drawSpriteTile(frame, code, color, flipx, flipy, sx, sy) {
        ...
        for (let py = 0; py < 16; py++) {
            ...
            for (let px = 0; px < 16; px++) {
                const dx = sx + px;
                if (dx < clipMin || dx > clipMax)
                    continue;
                const pen = src[srcRow + (flipx ? 15 - px : px)];
                if (spriteTrans[colorBase + pen])
                    continue;
