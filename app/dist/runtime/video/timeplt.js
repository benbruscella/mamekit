...
export const TIMEPLT_SPRITE_LAYOUT = {
    width: 16,
    height: 16,
    total: 'RGN_FRAC(1,1)',
    planes: 2,
...
export function buildTimepltPalette(proms) {
    const five = (bits) => bits.reduce((acc, b, i) => acc + (b ? WEIGHTS[i] : 0), 0);
...
export class TimepltVideo {
    ...
    spriteGfx;
    pal;
    flip = false; // mainlatch Q1 -> flip_screen_set (inverted; board resolves)
    videoEnable = false; // mainlatch Q4 -> video_enable_w
    constructor(deps) {
        const tiles = deps.regions['tiles'];
        const sprites = deps.regions['sprites'];
        const proms = deps.regions['proms'];
        if (!tiles || !sprites || !proms)
            throw new Error('timeplt video: missing tiles/sprites/proms region');
        if (proms.length < 0x240)
            throw new Error('timeplt video: proms region too small');
