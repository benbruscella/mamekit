...
export function buildGalaxianPalette(proms) {
    ...
    for (let i = 0; i < 7; i++)
        bullets[i] = packRGB(0xff, 0xff, 0xff);
...
export function buildGalaxianStarTable() {
...
export class GalaxianVideo {
    ...
    constructor(deps) {
        ...
        if (!gfx1 || !proms)
            throw new Error('galaxian video: missing gfx1/proms region');
        if (proms.length < 0x20)
            throw new Error('galaxian video: proms region too small');
    ...
    setStarsEnable(data) {
        ...
        if (!this.starsEnabled && on)
            this.starOrigin = 0;
        this.starsEnabled = on;
    }
    /** galaxian_flip_screen_x_w */
    setFlipX(data) { this.flipX = data & 1; }
    /** galaxian_flip_screen_y_w */
    setFlipY(data) { this.flipY = data & 1; }
    reset() {
        this.starsEnabled = 0;
        this.starOrigin = 0;
        this.flipX = 0;
    ...
    drawStars(frame) {
        ...
        for (let fy = 0; fy < NATIVE_H; fy++) {
            ...
            for (let x = 0; x < NATIVE_W; x++) {
                ...
                let star = table[offs];
                if (++offs >= STAR_RNG_PERIOD)
                    offs = 0;
                if (enable && (star & 0x80))
                    frame[row + x] = colors[star & 0x3f];
                star = table[offs];
                if (++offs >= STAR_RNG_PERIOD)
                    offs = 0;
                if (enable && (star & 0x80))
        ...
        for (let fy = 0; fy < NATIVE_H; fy++) {
            ...
            for (let col = 0; col < 32; col++) {
                ...
                for (let px = 0; px < 8; px++) {
                    ...
                    let nx = (col << 3) | px;
                    if (flipX)
                        nx ^= 255; // TILEMAP_FLIPX mirrors the layer
    ...
    drawBullets(frame) {
        ...
        for (let fy = 0; fy < NATIVE_H; fy++) {
            ...
            if (shell >= 0)
                this.drawBullet(frame, fy, 255 - objram[0x60 + shell * 4 + 3], bullets[shell]);
            if (missile >= 0)
                this.drawBullet(frame, fy, 255 - objram[0x60 + 7 * 4 + 3], bullets[7]);
        }
    }
    /** 4-pixel run at x-4 .. x-1 (shots display while H counts 0xFC..0xFF). */
    drawBullet(frame, fy, x, color) {
        const row = fy * NATIVE_W;
        x -= 4;
        for (let i = 0; i < 4; i++, x++) {
            if (x >= 0 && x < NATIVE_W)
