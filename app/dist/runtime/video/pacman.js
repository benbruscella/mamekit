...
export function buildPacmanPalette(proms) {
...
export function pacmanScanRows(col, row) {
    row += 2;
    col -= 2;
    if (col & 0x20)
...
export class PacmanVideo {
    ...
    constructor(deps) {
        ...
        if (!gfx1 || !proms)
            throw new Error('pacman video: missing gfx1/proms region');
        if (gfx1.length < 0x2000)
            throw new Error('pacman video: gfx1 region too small');
        if (proms.length < 0x120)
            throw new Error('pacman video: proms region too small');
    ...
    drawOneSprite(frame, sr, sr2, offs, yhack) {
    ...
    drawSpriteTile(frame, code, color, flipx, flipy, sx, sy) {
        ...
        for (let py = 0; py < 16; py++) {
            const dy = sy + py;
            if (dy < SPRITE_CLIP_Y0 || dy > SPRITE_CLIP_Y1)
                continue;
            const srcRow = base + (flipy ? 15 - py : py) * 16;
            const dstRow = dy * w;
            for (let px = 0; px < 16; px++) {
                const dx = sx + px;
                if (dx < SPRITE_CLIP_X0 || dx > SPRITE_CLIP_X1)
                    continue;
                const pen = src[srcRow + (flipx ? 15 - px : px)];
                if (penTrans[colorBase + pen])
                    continue;
