...
export class NesPpu {
    ...
    writeReg(addr, data) {
        ...
        switch (addr & 7) {
            case 0: { // PPUCTRL
                ...
                if ((data & 0x80) && !(prev & 0x80) && (this.status & 0x80))
            ...
            case 7: { // PPUDATA
                const a = this.v & 0x3fff;
                if ((a & 0x3f00) === 0x3f00)
                    this.palette[paletteIndex(a)] = data & 0x3f;
                else if (a < 0x2000)
                    this.host.chrWrite(a, data);
                else
                    this.host.ntWrite(a & 0x0fff, data);
                this.v = (this.v + ((this.ctrl & 0x04) ? 32 : 1)) & 0x7fff;
                break;
            }
        }
    ...
    evaluateSprites(line, bg, spr) {
        ...
        for (let i = 0; i < 64; i++) {
            ...
            found++;
            if (!visible)
                continue; // evaluation (overflow) still counts
            const tile = oam[(i << 2) | 1];
            const attr = oam[(i << 2) | 2];
            const sx = oam[(i << 2) | 3];
            if (attr & 0x80)
                row = height - 1 - row; // flip V
            let pa;
            ...
            for (let px = 0; px < 8; px++) {
                ...
                if (xpos > 255)
                    break;
                if (xpos < 8 && !showLeft)
                    continue; // L8 sprite clip
                const bit = flipH ? px : 7 - px;
                const pat = ((lo >> bit) & 1) | (((hi >> bit) & 1) << 1);
                if (pat === 0)
                    continue;
                // sprite-0 hit: opaque sprite-0 pixel over opaque bg, x != 255
                if (i === 0 && bothLayers && bg[xpos] !== 0 && xpos !== 255)
                    this.status |= 0x40;
                if (spr[xpos] & 0x80)
                    continue; // a lower-index sprite already owns it
