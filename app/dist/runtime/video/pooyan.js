...
export const POOYAN_CHAR_LAYOUT = {
    width: 8,
    height: 8,
    total: 'RGN_FRAC(1,2)',
    planes: 4,
...
export const POOYAN_SPRITE_LAYOUT = {
    width: 16,
    height: 16,
    total: 'RGN_FRAC(1,2)',
    planes: 4,
...
export function buildPooyanPalette(proms) {
...
export class PooyanVideo {
    ...
    spriteGfx;
    pal;
    flip = false; // mainlatch Q7 -> flip_screen_set (inverted; board resolves the polarity)
    constructor(deps) {
        const tiles = deps.regions['tiles'];
        const sprites = deps.regions['sprites'];
        const proms = deps.regions['proms'];
        if (!tiles || !sprites || !proms)
            throw new Error('pooyan video: missing tiles/sprites/proms region');
        if (proms.length < 0x220)
            throw new Error('pooyan video: proms region too small');
