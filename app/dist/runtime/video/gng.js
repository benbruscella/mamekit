...
export const GNG_SPRITE_LAYOUT = {
    width: 16,
    height: 16,
    total: 'RGN_FRAC(1,2)',
    planes: 4,
...
// (tilemap.cpp:574-585).
export const GNG_BG_FRONT_MASKS = [0xff, 0x41]; // LAYER0
export const GNG_BG_BACK_MASKS = [0x00, 0xbe]; // LAYER1
 ...
 */
export function buildGngPens(base, ext, out) {
    for (let i = 0; i < 256; i++) {
        const e = ext[i];
        const b = base[i];
 ...
 */
function normalizeSpriteRegion(region) {
    let copy = null;
...
// ---------------------------------------------------------------------------
export class GngVideo {
    width = NATIVE_W;
    height = NATIVE_H;
    deps;
    charGfx; // 1024 8x8 2bpp chars
    tileGfx; // 1024 16x16 3bpp tiles
    spriteGfx; // 1024 16x16 4bpp sprites
    pens = new Uint32Array(256);
    constructor(deps) {
    ...
    drawFg(frame, flip) {
        ...
        for (let fby = 0; fby < NATIVE_H; fby++) {
            ...
            for (let fbx = 0; fbx < NATIVE_W; fbx++) {
                ...
                if (pen === 3)
                    continue; // set_transparent_pen(3)
