...
export class GalaxianSound {
    ...
    toneWave;
    pitch; // 8-bit pitch latch; 0xff = silent
    vol; // VOL1 | VOL2<<1
    toneCounter; // 4-bit 74393 counter
    toneCountdown;
    // --- background hum (3x 555 VCO swept by the LFO) -------------------------
    bgEnable;
    bgPhase; // cycles, [0, 1)
    bgInc; // cycles per sample
    freq; // LFO sweep value, MINFREQ..MAXFREQ
    lfoBits;
    lfoStepSamples; // 0 = LFO timer not armed yet
    lfoCounter;
    // --- noise / explosion ----------------------------------------------------
    noiseWave; // 4 s LFSR loop at NOISE_RATE, ±1
    noiseStep; // noise samples per output sample
    noisePos;
    noiseFrac;
    noiseEnable;
    noiseVol; // 0..100 integer envelope
    decayPeriod; // samples between envelope steps
    decayCounter;
    noiseLp; // one-pole low-pass state
    noiseLpK;
    // --- shoot ("pew") ---------------------------------------------------------
    shootWave; // 2 s one-shot, 0..1
    shootPos; // >= length: not playing
    lastFire;
    constructor(clock = 3072000) {
     ...
     */
    lfoFreqWrite(bit, data) {
        const val = data & 1;
        if (this.lfoBits[bit] === val)
            return;
    ...
    render(out) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            for (let v = 0; v < 3; v++) {
                if (this.bgEnable[v])
                    mix += this.bgPhase[v] < 0.5 ? BG_AMP : -BG_AMP;
