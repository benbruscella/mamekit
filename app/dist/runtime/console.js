...
import { parseINes, identify } from "./nes-ines.js";
...
function cartSvg(o) {
    ...
    const titleSvg = lines.map((l, i) => `<text x="34" y="${112 + i * 20}" font-family="ui-sans-serif,system-ui,sans-serif" font-size="15" font-weight="800" fill="${titleColor}">${esc(l)}</text>`).join('');
...
export async function runConsole(cfg) {
    ...
    const boot = (rec, resolved) => {
        ...
        const regions = { prg: resolved.ines.prg };
    ...
    function buildSlot(name) {
      ...
      max-width:${CART_W}px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap`);
        ...
        const slot = {
            ...
            play: () => { if (slot.litRec)
                bootCart(slot.litRec, slot.litResolved); },
            info: () => { if (slot.litRec)
                openInfoModal(slot.litRec, slot.litResolved, ejectSlotFn(slot));
            else
                openTargetModal(catEntry, name); },
            eject: () => { if (slot.litRec)
                armEject(buttons, render, () => void ejectSlot(slot)); },
        ...
        function render() {
            ...
            const cover = coverHost.firstElementChild;
        ...
        const ejectSlotFn = (s) => () => void ejectSlot(s);
        async function ejectSlot(s) {
            if (s.litRec) {
                try {
                    await store.remove(s.litRec.id);
                }
    ...
    function buildOther(rec, resolved) {
        const state = resolved?.tier === 'experimental' ? 'experimental' : 'unsupported';
    ...
    function route(rec, resolved, announce) {
        if (resolved?.tier === 'tested') {
            ...
            if (s) {
                if (s.litRec && s.litRec.id !== rec.id) {
                    if (announce)
                        flash(s.item);
                    return;
                } // slot already lit by another dump
        ...
        for (const f of files) {
            ...
            if (f.name.toLowerCase().endsWith('.zip') || (bytes[0] === 0x50 && bytes[1] === 0x4b)) {
                ...
                try {
                    ...
                    toast(`${f.name} isn't a readable zip`);
                    continue;
                }
                let shelved = 0;
                ...
                if (!shelved)
                    toast(`${f.name}: no iNES cartridges inside`);
            }
