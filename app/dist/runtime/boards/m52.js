...
export class M52Board {
    ...
    constructor(config, regions, inputs, sinks) {
        ...
        this.cyclesPerLine = [mainSpec, audioSpec].map(c => Math.round(c.clock / config.screen.refresh / this.vtotal));
        // --- sound devices --------------------------------------------------------
        // local AYs for register readback; audio renders in the ay8910 worklet
        for (let i = 0; i < 2; i++)
            this.ays.push(new AY8910(894886));
        ...
        const registry = {
            read: {
                ...
                'm52_state.protection_r': () => {
                    let pop = 0;
                    for (let t = this.bgxpos[0] & 0x7f; t !== 0; t >>= 1)
                        pop += t & 1;
                    return (pop ^ (this.bgxpos[0] >> 7)) & 0xff;
                },
            },
            write: {
                'm52_state.videoram_w': () => { },
                'm52_state.colorram_w': () => { },
                ...
                'irem_audio_device.m52_adpcm_w': (_a, off, d) => { if (off & 1)
                    this.msm.write(0, d); },
                'irem_audio_device.sound_irq_ack_w': () => {
                    if ((this.soundLatch & 0x80) !== 0)
        ...
        const regionFor = (tag) => regions[tag] ?? Object.entries(regions).find(([k]) => k.endsWith(`:${tag}`))?.[1];
        ...
        if (!audioRom)
            throw new Error(`missing rom region ${audioSpec.region}`);
        ...
        const maskedBus = {
            read: (a) => audioBus.read(a & audioMask),
            write: (a, d) => audioBus.write(a & audioMask, d),
        };
        this.audio = new M6803(maskedBus, {
            p1Read: () => {
                if (this.port2 & 0x08)
                    return this.ays[0].readReg(this.ayAddr[0]);
                if (this.port2 & 0x10)
                    return this.ays[1].readReg(this.ayAddr[1]);
                return 0xff;
            },
