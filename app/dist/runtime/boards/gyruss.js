...
export class GyrussBoard {
    ...
    constructor(config, regions, inputs, sinks) {
        ...
        this.cyclesPerLine = [mainSpec, subSpec, audioSpec, mcuSpec].map(c => Math.round(c.clock / config.screen.refresh / this.vtotal));
        ...
        const registry = {
            ...
            write: {
                ...
                'gyruss_state.slave_irq_mask_w': (_a, _o, d) => {
                    ...
                    if (!this.slaveIrqMask)
                        this.sub.setIrqLine(false);
                },
                'gyruss_state.spriteram_w': () => { },
        ...
        for (let i = 0; i < 5; i++) {
            ...
            registry.write[`ay${i + 1}.data_w`] = (_a, _o, d) => {
                ...
                if (this.ayAddr[chip] === 15 && chip < 2)
                    sinks.soundWrite(0x90 + chip, d, this.scanline / this.vtotal);
            };
        ...
        this.mcu = new Mcs48({
            ...
            writePort: (port, d) => {
                if (port === 1)
                    sinks.soundWrite(0x80, d, this.scanline / this.vtotal);
                if (port === 2)
                    this.mcu?.setIrqLine(false); // irq_clear_w (mcu may still be constructing: reset fires port writes)
            },
        });
        // --- video ----------------------------------------------------------------
