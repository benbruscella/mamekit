...
import { NesPpu } from "../video/nes-ppu.js";
import { NesApu } from "../nes-apu.js";
import { createCart } from "../nes-cart.js";
const P1 = 'ctrl1:JOYPAD';
const P2 = 'ctrl2:JOYPAD';
export class NesBoard {
    fbWidth = 256;
    fbHeight = 240;
    cpu;
    ppu;
    ...
    frameCount = 0;
    totalCycles = 0;
    cycleCarry = 0; // fractional cycles-per-line accumulator
    pendingStall = 0; // OAM-DMA + DMC steals, drained from the next budget
    line = 0; // current scanline (for soundWrite frac)
    cyclesPerLine;
    vtotal;
    strobe = 0;
    pad1 = 0; // shift registers
    pad2 = 0;
    constructor(config, regions, inputs, sinks) {
        if (!config.cart)
            throw new Error('nes board needs config.cart (inserted by the console room)');
        if (!regions.prg?.length)
            throw new Error('nes board needs regions.prg (cart program data)');
        this.inputs = inputs;
        this.sinks = sinks;
    ...
    cpuRead(addr) {
        ...
        if (addr === 0x4016 || addr === 0x4017) {
            ...
            if (this.strobe & 1)
                return (this.inputs.read(tag) & 1) | 0x40;
    ...
    cpuWrite(addr, data) {
        ...
        if (addr === 0x4014) {
            ...
            for (let i = 0; i < 256; i++)
                this.ppu.writeOam(this.cpuRead(base + i));
    ...
    frame(fb) {
        for (let line = 0; line < this.vtotal; line++) {
            this.line = line;
            if (line < 240)
                this.ppu.renderLine(line, fb);
            else if (line === 241)
                this.ppu.startVblank();
            else if (line === this.vtotal - 1)
                this.ppu.preRender();
            if (this.ppu.takeNmi())
                this.cpu.nmi();
