...
import { RocnropeVideo } from "../video/rocnrope.js";
import { portHandlers } from "../input.js";
export class RocnropeBoard {
    video;
    fbWidth;
    fbHeight;
    main;
    ...
    irqMask = 0;
    curLine = 0;
    cyclesPerLine; // fractional (tpsound: ~116.52)
    cycleCarry = [0, 0];
    vtotal;
    frameCount = 0;
    shares;
    constructor(config, regions, inputs, sinks) {
        ...
        this.mainlatch.onQ(7, s => {
            this.irqMask = s;
            if (!s)
                this.main.setIrqLine(false); // irq_mask_w: mask=0 clears the line
        });
        // --- handler registry ----------------------------------------------------
        const shares = {};
        this.shares = shares;
        ...
        const registry = {
            ...
            write: {
                ...
                'rocnrope_state.colorram_w': () => { },
                'rocnrope_state.videoram_w': () => { },
            },
        };
        // --- CPUs + buses ---------------------------------------------------------
    ...
    frame(fb) {
        ...
        for (let line = 0; line < this.vtotal; line++) {
            ...
            if (line === 240) { // vblank start (visible 16..239 native)
                if (this.irqMask)
                    this.main.setIrqLine(true); // held until mask clears
                this.video.vblank();
            }
