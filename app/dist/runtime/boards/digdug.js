...
import { Namco51 } from "../namco51.js";
import { Namco53 } from "../namco53.js";
import { Namco06 } from "../namco06.js";
import { ER2055 } from "../er2055.js";
import { DigdugVideo } from "../video/digdug.js";
export class DigdugBoard {
    video;
    fbWidth;
    fbHeight;
    cpus = [];
    ...
    constructor(config, regions, inputs, sinks) {
        ...
        const registry = {
            ...
            write: {
                'digdug_state.digdug_videoram_w': () => { },
    ...
    loadEarom() {
        ...
        try {
            ...
            if (bytes.length !== this.earom.data.length)
                return;
            for (let i = 0; i < bytes.length; i++)
                this.earom.data[i] = Number(bytes[i]) & 0xff;
        }
    ...
    saveEarom() {
        if (typeof localStorage === 'undefined')
            return;
        try {
            localStorage.setItem(this.earomKey, this.earom.data.join(','));
        }
        catch { /* ignore */ }
    }
    reset() {
    ...
    frame(fb) {
        ...
        for (let line = 0; line < this.vtotal; line++) {
            ...
            if (line === this.vbstart) {
                ...
                if (this.subIrqMask && !this.subsHeld)
