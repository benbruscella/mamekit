...
import { JunofrstVideo } from "../video/junofrst.js";
import { portHandlers } from "../input.js";
export class JunofrstBoard {
    video;
    fbWidth;
    fbHeight;
    main;
    ...
    mcu;
    mainlatch = new LS259();
    ay = new AY8910(1_789_772);
    ayAddr = 0;
    mainRom;
    blitRom;
    bankBase = 0x10000;
    blitterData = new Uint8Array(4);
    irqEnable = 0;
    irqToggle = false;
    audioIrqHeld = false;
    lastIrqTrigger = 0;
    ...
    constructor(config, regions, inputs, sinks) {
        ...
        this.cyclesPerLine = [mainSpec, audioSpec, mcuSpec].map(c => Math.round(c.clock / config.screen.refresh / this.vtotal));
        this.mainRom = regions[mainSpec.region];
        this.blitRom = regions['blitrom'];
        if (!this.mainRom || !this.blitRom)
            throw new Error('missing maincpu/blitrom regions');
        ...
        const registry = {
            ...
            write: {
                ...
                'aysnd.data_w': (_a, _o, d) => {
                    ...
                    if (this.ayAddr === 15)
                        sinks.soundWrite(0x90, d, this.curLine / this.vtotal);
                },
        ...
        this.mcu = new Mcs48({
            ...
            writePort: (port, d) => {
                if (port === 1)
                    sinks.soundWrite(0x80, d, this.curLine / this.vtotal); // DAC sample byte
                if (port === 2) {
                    if ((d & 0x80) === 0)
                        this.mcu?.setIrqLine(false); // ctor-safe: reset fires port writes
        ...
        for (let i = 0; i < 16; i++) {
            for (let j = 0; j < 16; j++) {
                ...
                if (px) {
                    ...
                    if (dest & 1)
                        vram[idx] = (vram[idx] & 0x0f) | (px << 4);
                    else
                        vram[idx] = (vram[idx] & 0xf0) | px;
                }
                dest++;
            }
            dest += 240;
    ...
    frame(fb) {
        ...
        for (let line = 0; line < this.vtotal; line++) {
            ...
            if (line === 240) { // vblank: main IRQ every other frame (30 Hz, tutankhm)
                ...
                if (this.irqToggle && this.irqEnable)
