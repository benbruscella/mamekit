...
interface Voice {
  /** 20-bit frequency (per-sample counter increment), voice 0 only has the low 4 bits. */
  frequency: number;
  /** free-running sample counter (uint32) */
  counter: number;
  /** 4-bit volume, 0..15 */
  volume: number;
  /** 3-bit waveform select, 0..7 */
  waveformSelect: number;
}

export class NamcoWSG implements SoundCore {
  /** Native output rate == chip clock (96000 for Pac-Man / Galaga). */
  readonly sampleRate: number;

  private readonly numVoices: number;
  /** namco.h: MIX_RES = 128 * MAX_VOICES — per-voice headroom so the sum never clips. */
  private readonly mixRes: number;
  /** Decoded wavetable: 8 waveforms x 32 samples, each (nibble - 8) in [-8, 7]. */
  private readonly wavetable: Int8Array;
  ...
  constructor(waveRom: Uint8Array, clock: number, voices: number = 3) {
  ...
  soundEnable(state: boolean): void {
    this.enabled = state;
  }
  ...
  write(offset: number, data: number): void {
    ...
    let ch: number;
    if (offset < 0x10) ch = Math.trunc((offset - 5) / 5); // C++ truncating division
    else if (offset === 0x10) ch = 0;
    else ch = Math.trunc((offset - 0x11) / 5);

    if (ch < 0 || ch >= this.numVoices) return;
  ...
  render(out: Float32Array): void {
    ...
    for (const voice of this.voices) {
      ...
      if (vol === 0) continue; // "only update if we have non-zero volume"
