...
export function buildApp(outRoot: string): boolean {
  ...
  const compositionBindings: string[] = [];
  const compositionArtifacts = new Map<string, string>();
  ...
  if (existsSync(hardwareManifestPath)) {
    ...
    for (const hardware of manifest.hardware ?? []) {
      ...
      if (hardware.executableKind === 'composition' && hardware.executableArtifact) {
        if (compositionArtifacts.has(hardware.executableArtifact)) continue;
        const binding = `composition_${compositionBindings.length}`;
        compositionArtifacts.set(hardware.executableArtifact, binding);
        compositionBindings.push(binding);
        hardwareImports.push(
          `import ${binding} from '../runtime/generated/${hardware.executableArtifact}';`,
        );
        continue;
      }
...
import { registerGeneratedBoard, registerGeneratedComposition } from '../runtime/core/generated-board.ts';
...
export function registerGeneratedMachines(): void {
  ...
  for (const composition of [${compositionBindings.join(', ')}]) {
    registerGeneratedComposition(composition);
  }
