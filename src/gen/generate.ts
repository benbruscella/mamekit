...
function gameMarkdown(d: {
  ...
  md.push(`# ${d.fullname}`);
  md.push('');
  md.push(`**${d.company} · ${d.year}** — transpiled from the MAME driver \`${d.driverFile}\` by mame2js.`);
  md.push('');
  ...
  md.push('---');
  md.push('');
  md.push(`*Generated by [mame2js](https://github.com/benbruscella/mame2js) from the knowledge graph of MAME driver \`${d.family}\`. Play it at [/app/?g=${d.game}](/app/?g=${d.game}) or [explore the knowledge graph](/${d.game}/viewer.html).*`);
  md.push('');
  return md.join('\n');
}
