...
export function installNesRuntime(context: SoundRuntimeContext): SoundRuntimeHooks {
  const plan = context.sound.nesApu;
  if (!plan) throw new Error(`${context.board.game}: NES sound binding has no generated APU plan`);
  verifyRuntimeTables(context.board.game, plan);
  const cpu = context.board.execution.cpus.find(candidate =>
    candidate.type?.toLowerCase() === 'rp2a03');
  if (!cpu) throw new Error(`${context.board.game}: NES APU has no RP2A03 owner`);
  ...
  const apu = new NesApu(cpu.clock, {
    onDmcStart: (address, length) => {
      ...
      for (let index = 0; index < length; index++) {
        ...
        if (source > 0xffff) source = 0x8000 + ((source - 0x8000) & 0x7fff);
        bytes[index] = context.readProgram(cpu.tag, source);
      }
      context.soundData(0, bytes);
    },
  });
  const updateIrq = (): void =>
    context.setCpuInputLine(cpu.tag, RP2A03_APU_IRQ_LINE, apu.irqAsserted() ? 1 : 0);
  ...
  return {
    tickCpu: (tag, cycles) => {
      ...
      if (stalls) context.stallCpu(cpu.tag, stalls);
      updateIrq();
    },
    reset: () => {
      apu.reset();
...
function verifyRuntimeTables(
  game: string,
  plan: NonNullable<SoundRuntimeContext['sound']['nesApu']>,
): void {
  const equal = (left: readonly number[], right: readonly number[]): boolean =>
    left.length === right.length && left.every((value, index) => value === right[index]);
  const duty = plan.dutyPatterns.map(pattern =>
    Array.from({ length: 8 }, (_, bit) => (pattern >> (7 - bit)) & 1));
  if (
    !equal(NES_LENGTH_TABLE, plan.lengthTable) ||
