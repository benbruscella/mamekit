...
export interface DvgPoint {
  x: number;
  y: number;
  intensity: number;
}

export interface DvgExecutionPlan {
  memoryBase: number;
  coordinateBits: number;
}

export function executeDvgDisplayList(
  readByte: (address: number) => number,
  plan: DvgExecutionPlan,
  maximumInstructions = 10_000,
): DvgPoint[] {
  const coordinateMask = (1 << (plan.coordinateBits + 2)) - 1;
  const invalidBit = 1 << plan.coordinateBits;
  const signBit = invalidBit;
  const points: DvgPoint[] = [];
  const stack: number[] = [];
  let pc = 0;
  let scale = 0;
  let x = 0;
  ...
  const append = (intensity: number): void => {
    if (!((x | y) & invalidBit)) points.push({ x, y, intensity });
  };
  const draw = (rawX: number, rawY: number, opcode: number): void => {
    ...
    while (remaining-- > 0) {
      ...
      for (let bit = 0; bit < 12; bit++) {
        if ((counter & ((1 << (bit + 1)) - 1)) !== ((1 << bit) - 1)) continue;
        if (multiplierX & (1 << (11 - bit))) stepX = true;
        if (multiplierY & (1 << (11 - bit))) stepY = true;
      }
      counter = (counter + 1) & 0x0fff;
      if (stepX) x = (x + directionX) & coordinateMask;
      if (stepY) y = (y + directionY) & coordinateMask;
    }
  };
