...
import { YM2203, YM2203_FM_GAIN, YM2203_SSG_GAIN } from './ym2203.ts';
...
class StreamResampler {
  ...
  setNativeRate(nativeRate: number): void {
    this.step = nativeRate / sampleRate;
  }
...
class Ym2203Processor extends AudioWorkletProcessor {
  ...
  constructor() {
    ...
    this.port.onmessage = (event: MessageEvent) => {
      const msg = event.data as Ym2203Message;
      switch (msg.type) {
        case 'init': {
