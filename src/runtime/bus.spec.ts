...
const bus = new Bus([
  ...
  { start: 0x3000, end: 0x3001, kind: 'ram', share: 'video', write: 'videoWrite' },
], rom, {
  read: {
    read: (address, offset) => {
  ...
  write: {
    ...
    videoWrite: (address, offset, data) => writes.push([address, offset, data]),
  },
});
...
assert.deepEqual(writes[1], [0x3001, 1, 0x77]);
