 ...
 */
export class GeneratedZ80PioDevice implements Device {
  private readonly definition: GeneratedDeviceDefinition;
  private readonly clock: number;
  private readonly ports: [PioPortState, PioPortState];
  private readonly listeners = new Map<string, DeviceCallbackListener[][]>();
  private readonly callbackInitials = new Map<string, number>();
  private readonly calls = new Map<string, (...args: any[]) => unknown>();
  private readonly methodParameters = new Map<string, string[][]>();
  ...
  constructor(
    ...
    for (const callback of definition.callbacks) {
      this.listeners.set(
        callback.signal,
        Array.from({ length: callback.slots }, () => []),
      );
  ...
  invoke(name: string, ...args: GeneratedCallArgument[]): unknown {
    ...
    switch (name) {
      ...
      case 'device_reset': this.reset(); return 0;
      case 'read': return this.read(values[0] ?? 0, false);
      case 'write': this.write(values[0] ?? 0, values[1] ?? 0, false); return 0;
      case 'read_alt': return this.read(values[0] ?? 0, true);
      case 'write_alt': this.write(values[0] ?? 0, values[1] ?? 0, true); return 0;
      case 'control_read': return this.controlRead();
      case 'control_write': this.controlWrite(values[0] ?? 0, values[1] ?? 0); return 0;
      case 'control_a_write': this.controlWrite(0, values[0] ?? 0); return 0;
      case 'control_b_write': this.controlWrite(1, values[0] ?? 0); return 0;
      case 'data_read': return this.dataRead(values[0] ?? 0);
      case 'data_write': this.dataWrite(values[0] ?? 0, values[1] ?? 0); return 0;
      case 'data_a_read': return this.dataRead(0);
      case 'data_b_read': return this.dataRead(1);
      case 'data_a_write': this.dataWrite(0, values[0] ?? 0); return 0;
      case 'data_b_write': this.dataWrite(1, values[0] ?? 0); return 0;
      ...
      case 'port_write':
        if (values.length >= 3) {
          ...
          this.portWrite(index, (this.ports[index]!.input & ~(1 << bit)) |
            ((values[2] ?? 0) << bit));
        } else this.portWrite(values[0] ?? 0, values[1] ?? 0);
        return 0;
      case 'port_a_read': return this.portRead(0);
      case 'port_b_read': return this.portRead(1);
      case 'port_a_write': this.portWrite(0, values[0] ?? 0); return 0;
      case 'port_b_write': this.portWrite(1, values[0] ?? 0); return 0;
      ...
      case 'strobe': this.strobe(values[0] ?? 0, Boolean(values[1])); return 0;
      case 'strobe_a': this.strobe(0, Boolean(values[0])); return 0;
      case 'strobe_b': this.strobe(1, Boolean(values[0])); return 0;
      case 'z80daisy_irq_state': return this.irqState();
      case 'z80daisy_irq_ack': return this.irqAcknowledge();
      case 'z80daisy_irq_reti': this.irqReti(); return 0;
      case 'check_interrupts': this.checkInterrupts(); return 0;
    }
  ...
  set(name: string, value: number): void {
    ...
    if (!port) return;
    switch (indexed?.[1] ?? name) {
      case 'm_input': port.input = value & 0xff; break;
      case 'm_output': port.output = value & 0xff; break;
      case 'm_ior': port.ior = value & 0xff; break;
      case 'm_mask': port.mask = value & 0xff; break;
      case 'm_vector': port.vector = value & 0xff; break;
    }
  }
  ...
  on(signal: string, listener: DeviceCallbackListener, slot = 0): Device {
    ...
    if (!listeners) throw new Error(`Z80PIO callback "${signal}" has no slot ${slot}`);
  ...
  cycleClock(): number { return this.clock; }
  dataAddressBits(): number | undefined { return this.definition.dataAddressBits; }
  bus(): GeneratedDeviceDefinition['bus'] { return this.definition.bus; }
  role(): string | undefined { return this.definition.role; }
  links(): readonly GeneratedDeviceLink[] { return this.definition.links ?? []; }
  invokeSlot(_name: string, ..._args: GeneratedCallArgument[]): unknown {
  ...
  private emit(signal: string, ...args: number[]): number {
  ...
  private read(offset: number, alternate: boolean): number {
  ...
  private write(offset: number, data: number, alternate: boolean): void {
    const index = (offset >> Number(alternate)) & 1;
    const control = alternate ? offset & 1 : (offset >> 1) & 1;
    if (control) this.controlWrite(index, data);
    else this.dataWrite(index, data);
  }

  private controlRead(): number {
    return ((this.ports[0].interruptControl & 0xc0) |
      (this.ports[1].interruptControl >> 4)) & 0xff;
  }

  private setReady(index: number, ready: boolean): void {
    const port = this.ports[index & 1]!;
    if (port.ready === ready) return;
    port.ready = ready;
    this.emit(index ? 'out_brdy_callback' : 'out_ardy_callback', Number(ready));
  }

  private setMode(index: number, mode: number): void {
    const port = this.ports[index & 1]!;
    if (mode === MODE_OUTPUT) {
    ...
    if (!(mode === MODE_BIDIRECTIONAL && index === 1)) port.mode = mode;
  }

  private controlWrite(index: number, data: number): void {
    const port = this.ports[index & 1]!;
    data &= 0xff;
  ...
  private dataRead(index: number): number {
    const port = this.ports[index & 1]!;
    let data = 0;
    if (port.mode === MODE_OUTPUT) data = port.output;
  ...
  private dataWrite(index: number, data: number): void {
    index &= 1;
    data &= 0xff;
    const port = this.ports[index]!;
    if (port.mode === MODE_OUTPUT || port.mode === MODE_BIDIRECTIONAL) {
      ...
      port.output = data;
      this.emit(
        index ? 'out_pb_callback' : 'out_pa_callback',
        0,
        port.ior | (port.output & (port.ior ^ 0xff)),
      );
    }
  }

  private portRead(index: number): number {
    const port = this.ports[index & 1]!;
    if (port.mode === MODE_OUTPUT) return port.output;
    if (port.mode === MODE_BIDIRECTIONAL) return index ? 0xff : port.output;
  ...
  private portWrite(index: number, data: number): void {
    index &= 1;
    const port = this.ports[index]!;
    if (port.mode !== MODE_BIT_CONTROL) return;
  ...
  private strobe(index: number, state: boolean): void {
    index &= 1;
    const port = this.ports[index]!;
    if (this.ports[0].mode === MODE_BIDIRECTIONAL) {
      if (port.ready) {
        if (port.strobe && !state) {
          if (index === 0) this.emit('out_pa_callback', 0, port.output);
          else this.ports[0].input = this.emit('in_pa_callback', 0) & 0xff;
  ...
  private triggerInterrupt(index: number): void {
  ...
  private checkInterrupts(): void {
    ...
    const asserted = !underService && this.ports.some(port =>
      port.interruptEnabled && port.interruptPending);
    this.emit('out_int_callback', Number(asserted));
  }

  private irqState(): number {
    let state = 0;
    for (const port of this.ports) {
      if (port.interruptUnderService) return 2;
      if (port.interruptEnabled && port.interruptPending) state = 1;
    }
    return state;
  }

  private irqAcknowledge(): number {
    for (const port of this.ports) {
      if (!port.interruptPending) continue;
      port.interruptPending = false;
