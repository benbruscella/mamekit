 ...
 * feeds a DAC under a slower CPU's direction.
 *
 * 250 us is fine enough that no board's handshake starves. It restores every
 * regressed board to within 0.4% of its pre-quantum write count.
 *
 * This is an empirical bound, not a derived one. The derived version is to
 * end each round at the next device timer expiry, as MAME clamps its own
 * timeslice, which would make the cap redundant for boards that schedule
 * real work. Handshakes no longer depend on it: `synchronize()` and
 * `perfect_quantum` are modelled directly (requestSynchronize, lockstep).
 */
...
export class GeneratedFrameRunner {
  ...
  private lockstep(lines: number, done: number): number {
    ...
    this.fineLines = Math.max(0, this.fineLines - (end - done) * lines);
    return end;
  }
