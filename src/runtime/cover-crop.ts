 ...
 */
export function fitCoverCropAspect(
  crop: CoverCrop,
  sourceWidth: number,
  sourceHeight: number,
  targetAspect: number,
 ...
 */
export function findDarkCoverCrop(
  pixels: Uint8ClampedArray,
  width: number,
  height: number,
): CoverCrop | undefined {
  if (width < 16 || height < 16 || pixels.length < width * height * 4) return undefined;
  ...
  const rowHasContent = (y: number) => {
    ...
    for (let x = 0; x < width; x++) {
      if (isContent(x, y) && ++count >= rowMinimum) return true;
    }
    return false;
  };
  const columnHasContent = (x: number) => {
    ...
    for (let y = 0; y < height; y++) {
      if (isContent(x, y) && ++count >= columnMinimum) return true;
    }
    return false;
  };
  ...
  if (removedX / width < 0.08 && removedY / height < 0.08) return undefined;
