...
import { MAIN_THREAD_STREAM_DEVICES, pumpStreamDevices } from "../stream-device-pump.js";
export function installSn76489Runtime(context) {
    ...
    for (const auxiliary of context.sound.auxiliaryDevices ?? []) {
        ...
        if (MAIN_THREAD_STREAM_DEVICES.includes(auxiliary.type))
            continue;
        const aliases = [
            auxiliary.deviceTag,
            ...
            for (const alias of aliases) {
                if (auxiliary.type === 'SAMPLES' && method === 'start') {
                    context.calls[`${alias}.${method}`] = (channel = 0, sample = 0, loop = 0) => {
                        context.soundWrite(channel & 0xff, (sample & 0x7f) | (loop ? 0x80 : 0), context.fraction(), name);
                        return 0;
                    };
                }
                    ...
                    context.calls[`${alias}.${method}`] = (channel = 0, volume = 0) => {
                        context.soundWrite(channel & 0xff, Math.max(0, Math.min(255, Math.round(Number(volume) * 255))), context.fraction(), name);
                        return 0;
                    };
                }
                else {
