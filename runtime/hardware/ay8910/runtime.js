...
const PORT_A = 14;
const PORT_B = 15;
export function installAy8910Runtime(context) {
    ...
    tags.forEach((tag, chip) => {
        ...
        const dataWrite = (data) => {
            ...
            const signal = register === PORT_A
                ...
                : register === PORT_B
                    ...
                    : undefined;
            if (signal)
                context.dispatch(tag, signal, data);
        };
        const dataRead = () => {
            const register = addresses.get(tag) ?? 0;
            const signal = register === PORT_A
            ...
            if (connected !== undefined)
                return connected;
            const callback = board.callbacks.find(candidate => candidate.ownerTag === tag && candidate.signal === signal);
        ...
        const dataAddressWrite = (offset, data) => {
            if (offset & 1)
                addressWrite(data);
            else
                dataWrite(data);
        };
        const addressDataWrite = (offset, data) => {
            if (offset & 1)
                dataWrite(data);
            else
                addressWrite(data);
        ...
        registry.write[`${tag}.data_address_w`] = (_address, offset, data) => dataAddressWrite(offset, data);
        registry.write[`${tag}.address_data_w`] = (_address, offset, data) => addressDataWrite(offset, data);
    ...
    for (const auxiliary of sound.auxiliaryDevices ?? []) {
        ...
        if (reference?.member) {
            state[reference.member] = {
                write: (...args) => {
                    context.soundWrite(0, args.at(-1) ?? 0, context.fraction(), `${auxiliary.deviceTag}.reference_w`);
                },
            };
        }
    }
 ...
 */
function bindAudioFilters(context, filters) {
    ...
    for (const route of sound.routes ?? []) {
        ...
        row[channel] = {
            filter_rc_set_RC: (type, r1, r2, r3, c) => {
                const values = [type, r1, r2, r3, c];
                const previous = filters.get(key);
                if (previous?.every((value, position) => value === values[position]))
                    return;
                filters.set(key, values);
                const base = FILTER_CONTROL_BASE + index * FILTER_CONTROL_STRIDE;
                values.forEach((value, parameter) => context.soundWrite(base + parameter, value, context.fraction()));
            },
        };
    }
    if (!rows.length)
