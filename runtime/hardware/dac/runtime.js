...
export function installDacRuntime(context) {
    for (const [chip, tag] of soundTags(context.sound).entries()) {
        ...
        for (const alias of deviceAliases(context.board, tag)) {
            for (const method of context.sound.writeMethods)
                context.calls[`${alias}.${method}`] = write;
        }
    }
    ...
    const williamsCommand = context.board.callbacks.find(callback => callback.signal === 'writepb_handler' &&
        callback.targetClass === 'williams_state' &&
        callback.targetMethod === 'snd_cmd_w');
    if (williamsCommand && context.board.devices?.some(device => device.tag === 'pia_2' && device.type === 'PIA6821')) {
        const write = (data = 0) => {
    ...
    for (const host of context.board.devices?.filter(device => /^GOTTLIEB_SOUND_(?:SPEECH_)?REV1A?$/.test(device.type)) ?? []) {
    ...
    for (const host of context.board.devices?.filter(device => device.type === 'MIDWAY_SOUNDS_GOOD') ?? []) {
        ...
        const read = () => Number(context.state.m_status ?? 0) & 3;
        const resetWrite = (state = 0) => {
