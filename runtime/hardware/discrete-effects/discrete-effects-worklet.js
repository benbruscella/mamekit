...
export class GeneratedDiscreteAudioCore {
    ...
    dacHighpass = 0;
    dkongMixerLowpass = 0;
    dkongMixerCoupling = 0;
    dkongAmplifierCoupling1 = 0;
    dkongAmplifierCoupling2 = 0;
    dkongStompLfsr = 0;
    dkongStompNoisePhase = 0;
    dkongStompNoise = false;
    dkongStompCounter = 0;
    dkongStompTriggerCap = 0;
    dkongStompEnvelope = 0;
    dkongStompIntegrateCap = 0;
    dkongStompVce = 0;
    dkongJrClimbLfsr = 0;
    dkongJrClimbNoisePhase = 0;
    dkongJrClimbNoise = 1;
    dkongJrClimbOneShot = 0;
    dkongJrWalkAge = Number.POSITIVE_INFINITY;
    dkongJrJumpAge = Number.POSITIVE_INFINITY;
    dkongJrStartupJump = true;
    dkongJrFallAge = 0;
    dkongJrMixerLowpass = 0;
    dkongJrMixerCoupling = 0;
    nodeValues = new Uint8Array(256);
    asteroidLfsr = 0;
    asteroidNoisePhase = 0;
    asteroidNoise = 1;
    asteroidExplosionPhase = 0;
    asteroidExplosionSample = 0;
    asteroidExplosionLowpass = 0;
    asteroidThrustInput = 0;
    asteroidThrustFast = 0;
    asteroidThrustSlow = 0;
    asteroidThumpPhase = 0;
    asteroidThumpFilter = 0;
    asteroidSaucerWarblePhase = 0;
    asteroidSaucerPhase = 0;
    asteroidShipFirePhase = 0;
    asteroidShipFireAge = 0;
    asteroidSaucerFirePhase = 0;
    asteroidSaucerFireAge = 0;
    asteroidLifePhase = 0;
    constructor(outputRate, _clock, plan) {
        this.outputRate = outputRate;
        this.plan = plan;
        this.active = plan?.voices.map(() => false) ?? [];
        this.envelope = plan?.voices.map(voice => voice.network === 'dkongjr-jump' ? 1 : 0) ?? [];
    ...
    write(offset, data) {
        ...
        if (offset >= 0 && offset < this.nodeValues.length) {
            ...
            else if (plan.outputNetwork === 'dkongjr' &&
        ...
        for (let index = 0; index < plan.voices.length; index++) {
            ...
            if (offset !== voice.node)
                continue;
            const active = voice.activeLow ? (data & 1) === 0 : (data & 1) !== 0;
            if (active !== this.active[index] &&
    ...
    sample() {
        ...
        for (let index = 0; index < plan.voices.length; index++) {
            ...
            if (voice.sustain && this.active[index])
                this.envelope[index] = 1;
            else
                this.envelope[index] *= Math.exp(-1 / releaseSamples);
            if (this.envelope[index] < 1e-5)
                this.envelope[index] = 0;
            let signal;
            if (voice.network === 'dkongjr-control')
                continue;
            ...
            if (voice.mode === 'noise') {
                ...
                if (voice.network === 'dkongjr-jump') {
                    ...
                    this.modulationPhase[index] = (this.modulationPhase[index] + 5.5 / this.outputRate) % 1;
                ...
                if (voice.vco) {
                    const vco = voice.vco;
                    this.modulationPhase[index] = (this.modulationPhase[index] +
                        vco.modulationFrequency / this.outputRate) % 1;
                    ...
                    const inputResistance = invertedInput === 0
                        ? parallel(vco.controlResistance1 + vco.controlResistance2, vco.oscillatorResistance)
                        : parallel(vco.controlResistance2, vco.oscillatorResistance);
                    const internalControlResistance = parallel(5_000, 10_000);
                    const totalResistance = parallel(inputResistance + vco.outputResistance, internalControlResistance);
                    ...
                    const alpha = 1 - Math.exp(-1 / (this.outputRate * totalResistance * vco.controlCapacitance));
                    this.controlVoltage[index] += (targetVoltage - this.controlVoltage[index]) * alpha;
                    ...
                    if (threshold >= 0.25 && threshold < vco.supplyVoltage) {
                        const charge = (vco.timerResistance1 + vco.timerResistance2) * vco.timerCapacitance * Math.log((vco.supplyVoltage - trigger) /
        ...
        if (plan.dischargeNode === undefined) {
            ...
            const releaseSamples = Math.max(1, (plan.dischargeRelease ?? 0.1) * this.outputRate);
            this.dacGate *= Math.exp(-1 / releaseSamples);
            if (this.dacGate < 1e-5)
                this.dacGate = 0;
        }
        ...
        this.dacHighpass = highAlpha * (this.dacHighpass + lowpass - this.previousLowpass);
        ...
        if (plan.outputNetwork === 'dkong2b') {
            return this.sampleDkongOutput(dkongStomp, dkongJump, dacOutput, dkongWalk);
        }
        mixed += dacOutput;
        if (plan.outputNetwork === 'dkongjr') {
    ...
    sampleDkongJrWalk(_index) {
        ...
        if (oneShot) {
            if (alternate) {
                this.phase[_index] = (this.phase[_index] + 360 / this.outputRate) % 1;
                this.modulationPhase[_index] = (this.modulationPhase[_index] + 180 / this.outputRate) % 1;
                ...
                this.phase[_index] = (this.phase[_index] + highFrequency / this.outputRate) % 1;
                this.modulationPhase[_index] = (this.modulationPhase[_index] + 1_100 / this.outputRate) % 1;
    ...
    sampleDkongJrClimb(index) {
        ...
        while (this.dkongJrClimbNoisePhase >= 1) {
            ...
            this.dkongJrClimbLfsr = (((this.dkongJrClimbLfsr << 1) & 0xffff) | input);
        ...
        const modulated = stepRcDiscMod(state, oneShot, this.dkongJrClimbNoise, 120, 4_700, 1, 100_000, 10e-6, 5, dt);
    ...
    sampleAsteroidOutput() {
        ...
        if (this.nodeValues[5]) {
            const frequency = Math.max(110, 820 - ((820 - 110) / 0.28) * this.asteroidShipFireAge);
        ...
        if (this.nodeValues[2]) {
            const frequency = Math.max(630, 830 - ((830 - 630) / 0.28) * this.asteroidSaucerFireAge);
        ...
        this.asteroidExplosionLowpass += (explosionTarget - this.asteroidExplosionLowpass) * rcCharge(dt, 3_042 * 1e-6);
        ...
        this.asteroidThrustFast += (this.asteroidThrustInput - this.asteroidThrustFast) * onePoleCoefficient(160, this.outputRate);
        this.asteroidThrustSlow += (this.asteroidThrustInput - this.asteroidThrustSlow) * onePoleCoefficient(45, this.outputRate);
        ...
        if (this.nodeValues[1]) {
            ...
            this.asteroidSaucerWarblePhase = (this.asteroidSaucerWarblePhase + warbleRate / this.outputRate) % 1;
            const warble = 1 - 4 * Math.abs(this.asteroidSaucerWarblePhase - 0.5);
            const frequency = 1_210 + 460 * warble - select * 250;
            this.asteroidSaucerPhase = (this.asteroidSaucerPhase + frequency / this.outputRate) % 1;
    ...
    sampleDkongStomp(index) {
        ...
        while (this.dkongStompNoisePhase >= 1) {
            ...
            const feedback = (((this.dkongStompLfsr >>> 10) ^
                (this.dkongStompLfsr >>> 23)) & 1);
            this.dkongStompLfsr = (((this.dkongStompLfsr << 1) | (feedback ^ 1)) & 0x00ff_ffff);
        ...
        this.dkongStompEnvelope += (envelopeTarget - this.dkongStompEnvelope) * rcCharge(dt, envelopeResistance * 3.3e-6);
        ...
        const diodeMix = Math.max(0, this.dkongStompEnvelope - 0.4, dividedNoise - 0.8);
        ...
        let capacitorCurrent;
        let emitterVoltage;
        let transistorResistance;
        let transistorCurrent;
        if (discharge) {
        ...
        this.dkongStompVce = 0.1 * vce + 0.9 * (5 - emitterVoltage - transistorCurrent * 0);
        return this.dkongStompIntegrateCap * (5_100 / 7_100);
    }
    sampleDkongTone(index, voice) {
        const vco = voice.vco;
        const state = this.dkongVoiceState[index];
        const dt = 1 / this.outputRate;
        ...
        const inputResistance = invertedInput === 0
            ? parallel(vco.controlResistance1 + vco.controlResistance2, vco.oscillatorResistance)
            : parallel(vco.controlResistance2, vco.oscillatorResistance);
        const totalResistance = parallel(inputResistance + vco.outputResistance, parallel(5_000, 10_000));
        const targetCurrent = vco.supplyVoltage / 5_000 +
            (invertedInput === 0
                ? vco.supplyVoltage /
                    (vco.controlResistance1 + vco.controlResistance2)
        ...
        const timer = step555Energy(state, vco, dt, voice.network === 'dkong-walk' ? 1.36 : 1);
        if (voice.network === 'dkong-walk') {
            const triggered = stepRcDiscMod(state, invertedInput, timer, 1_000, 4_700, 1_000, 10_000, 3.3e-6, 5, dt);
            const highpass = triggered - state.crCap;
            state.crCap += highpass * rcCharge(dt, 11_200 * 4.7e-6);
            return highpass * 0.5;
        }
        const trigger = stepRcDiscMod(state, invertedInput, 0, 10_000, 0, 0, 10_000, 1e-6, 5, dt);
    ...
    sampleDkongOutput(stomp, jump, dac, walk) {
        const dt = 1 / this.outputRate;
        ...
        const resistorMix = (stomp + jump * 1.9 + dac * 4.8 + walk * 4.23) / 4;
...
function stepInverterOscillator(state, vco, dt) {
    ...
    const transfer = (input) => {
        ...
        const exponent = (Math.log(-Math.log(outLow / supply)) -
            Math.log(-Math.log(outHigh / supply))) / Math.log(rise / fall);
        const coefficient = Math.exp(Math.log(-Math.log(outLow / supply)) -
    ...
    if (clamped) {
        ...
        difference = gate3 * ratio - (state.inverterCap + gate2) +
            input * (1 - ratio);
        difference *= 1 - Math.exp(-dt / (parallel(vco.modulationResistance, vco.modulationParallelResistance) *
            vco.modulationCapacitance));
    }
    else {
        difference = gate3 - (state.inverterCap + gate2);
        difference *= 1 - Math.exp(-dt / (vco.modulationResistance * vco.modulationCapacitance));
    }
    state.inverterCap += difference;
    state.inverterG2 = gate2;
    return gate3;
}
function step555Energy(state, vco, sampleTime, periodScale = 1) {
    const threshold = state.controlVoltage;
    const trigger = threshold / 2;
    if (threshold < 0.25)
        return state.timerHigh ? 4.5 : 0;
    if (state.timerCap >= threshold)
        state.timerHigh = false;
    else if (state.timerCap <= trigger)
    ...
    while (remaining > 1e-15 && transitions < 8) {
        if (state.timerHigh) {
            const timeConstant = (vco.timerResistance1 + vco.timerResistance2) * vco.timerCapacitance * periodScale;
            ...
            const used = -timeConstant * Math.log((vco.supplyVoltage - threshold) /
...
function stepRcDiscMod(state, input1, input2, resistance1, resistance2, resistance3, resistance4, capacitance, supply, dt) {
...
function rcCharge(dt, timeConstant) {
    return timeConstant > 0 ? 1 - Math.exp(-dt / timeConstant) : 1;
}
function voltageDivider(top, bottom) {
    const total = top + bottom;
    return total > 0 ? bottom / total : 0;
}
function parallelOrZero(left, right) {
    if (left <= 0)
        return right;
    if (right <= 0)
        return left;
...
function bandLimitedPulse(phase, step, duty) {
    let value = phase < duty ? 1 : -1;
    value += polyBlep(phase, step);
    value -= polyBlep((phase - duty + 1) % 1, step);
    return value;
}
function onePoleCoefficient(frequency, outputRate) {
    ...
    constructor() {
        ...
        this.port.onmessage = (event) => {
            ...
            if (message.type === 'init') {
                const core = new GeneratedDiscreteAudioCore(sampleRate, message.clock, message.discreteEffects);
                this.renderer = new GeneratedDiscreteAudioFrameRenderer(core, sampleRate, message.refresh ?? 60);
            }
