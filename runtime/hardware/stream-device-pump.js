 ...
 */
export const MAIN_THREAD_STREAM_DEVICES = ['VLM5030'];
/**
 ...
 */
export function pumpStreamDevices(context, types) {
    ...
    return {
        ...
        tickCpu: (cpuTag, cycles) => {
            ...
            for (const device of pumped) {
                ...
                const put = (index, value, max) => {
                    if (index >= 0 && index < due)
                        samples[index] = max ? value / max : 0;
                    return 0;
                };
                const stream = {
                    samples: () => due,
                    put_int: (_channel, index, value, max) => put(Number(index), Number(value), Number(max)),
                    put_int_clamp: (_channel, index, value, max) => put(Number(index), Math.max(-Number(max), Math.min(Number(max) - 1, Number(value))), Number(max)),
                    put: (_channel, index, value) => put(Number(index), Number(value), 1),
                };
                if (context.callDevice(device.tag, 'sound_stream_update', stream) === undefined)
                    continue;
