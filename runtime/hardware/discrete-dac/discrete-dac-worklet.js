...
export class GeneratedDiscreteAudioCore {
    ...
    write(offset, data) {
        if (offset === this.plan?.dac.node)
            this.dac = data & 0xff;
        else if (offset === this.plan?.volumeNode)
            this.volume = data & 0xff;
    }
    sample() {
        ...
        const fullScale = Math.max(1, Math.abs(plan.dac.offset), Math.abs(255 * plan.dac.gain + plan.dac.offset));
        ...
        for (let index = 0; index < plan.channels.length; index++) {
            ...
            const filtered = alpha * ((this.previousOutput[index] ?? 0) +
                filteredInput -
    ...
    constructor() {
        ...
        this.port.onmessage = (event) => {
            ...
            if (message.type === 'init') {
                const core = new GeneratedDiscreteAudioCore(sampleRate, message.clock, message.discreteDac);
