...
import { GAMEBOY_OUTPUT_RATE } from "./definition.js";
/**
...
export function installGameboyRuntime(context) {
    ...
    const gain = (channel) => routes.find(route => route.channel === channel)?.gain ?? 1;
    ...
    const speakerInputs = Math.max(1, new Set(routes.map(route => route.targetInput ?? 0)).size);
    let carry = 0;
    return {
        reset: () => {
            carry = 0;
        ...
        tickCpu: (cpuTag, cycles) => {
            ...
            const stream = {
                samples: () => due,
                put_int: (channel, index, value, max) => {
                    const scaled = max ? Number(value) / Number(max) : 0;
                    (Number(channel) === 0 ? left : right)[Number(index)] = scaled;
                    return 0;
                },
            };
            if (context.callDevice(tag, 'sound_stream_update', stream) === undefined)
                return;
            ...
            for (let index = 0; index < due; index++) {
                ...
                const sample = ((left[index] ?? 0) * gain(0) + (right[index] ?? 0) * gain(1)) / speakerInputs;
