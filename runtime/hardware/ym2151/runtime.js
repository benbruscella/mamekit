...
class Upd7759Runtime {
    ...
    repeatCount = 0;
    firstHeader = false;
    constructor(tag, clock, ownerCpu, dispatch) {
        this.tag = tag;
        this.clock = clock;
    ...
    stateKeys() { return Object.keys(this); }
    write(method, data) {
        if (method === 'port_w')
            this.fifo = data & 0xff;
            ...
            const next = data !== 0;
            if (this.state === UPD_IDLE && this.mdLine &&
    ...
    tick(cpuTag, cycles, cpuClock) {
        if (cpuTag !== this.ownerCpu || this.state === UPD_IDLE)
            return;
...
export function installYm2151Runtime(context) {
    const cpuFor = (tag) => context.board.execution.cpus.find(cpu => [...(cpu.ranges ?? []), ...(cpu.io?.ranges ?? [])].some(range => range.read?.startsWith(`${tag}.`) || range.write?.startsWith(`${tag}.`)))?.tag ??
        ...
        '';
    const timers = [];
    const instructionCycles = new Map();
    const cpuClocks = new Map(context.board.execution.cpus.map(cpu => [cpu.tag, Math.max(1, cpu.cycleClock ?? cpu.clock)]));
    const advanceTimers = (cpuTag, cycles) => {
        const cpuClock = cpuClocks.get(cpuTag);
        if (!cpuClock || cycles <= 0)
            return;
    ...
    const upd = (context.sound.auxiliaryDevices ?? [])
        ...
        .map(device => new Upd7759Runtime(device.deviceTag, device.clock, cpuFor(device.deviceTag), context.dispatch));
    for (const chip of upd) {
        const definition = context.sound.auxiliaryDevices?.find(device => device.deviceTag === chip.tag);
        for (const method of definition?.writeMethods ?? []) {
            ...
            for (const alias of deviceAliases(context.board, chip.tag)) {
                context.calls[`${alias}.${method}`] = (...args) => write(args.at(-1) ?? 0);
            }
        }
        for (const alias of deviceAliases(context.board, chip.tag)) {
    ...
    for (const auxiliary of context.sound.auxiliaryDevices ?? []) {
        ...
        for (const alias of deviceAliases(context.board, tag)) {
            ...
            context.calls[`${alias}.write`] = (...args) => {
            ...
            context.calls[`${alias}.set_volume`] = (...args) => {
            ...
            context.calls[`${alias}.set_bank`] = (...args) => {
    ...
    for (const auxiliary of context.sound.auxiliaryDevices ?? []) {
        ...
        for (const alias of deviceAliases(context.board, tag)) {
            ...
            context.calls[`${alias}.write`] = (...args) => {
                ...
                context.soundWrite(args.at(-2) ?? 0, args.at(-1) ?? 0, context.fraction(), name);
                return result;
            };
        }
    }
    ...
    for (const auxiliary of context.sound.auxiliaryDevices ?? []) {
        ...
        for (const alias of deviceAliases(context.board, tag)) {
            ...
            context.calls[`${alias}.start`] = (...args) => {
                const result = originalStart?.(...args);
                const channel = Number(args.at(-3) ?? args.at(-2) ?? 0);
                playing.add(channel);
                context.soundWrite(channel, Number(args.at(-2) ?? args.at(-1) ?? 0), context.fraction(), `${tag}.start`);
                return result;
            };
            const originalRaw = context.calls[`${alias}.start_raw`];
            context.calls[`${alias}.start_raw`] = (...args) => {
            ...
            context.calls[`${alias}.playing`] = (...args) => playing.has(Number(args.at(-1) ?? 0)) ? 1 : 0;
        }
    }
    ...
    const speech = (context.sound.auxiliaryDevices ?? [])
        ...
        .map(device => {
        const clock = context.board.devices?.find(candidate => candidate.tag === device.deviceTag)?.clock ?? device.clock;
    ...
    for (const auxiliary of context.sound.auxiliaryDevices ?? []) {
        ...
        for (const alias of deviceAliases(context.board, auxiliary.deviceTag)) {
            context.calls[`${alias}.write`] = (...args) => {
                context.soundWrite(Number(args.at(-2) ?? 0) || 0, args.at(-1) ?? 0, context.fraction(), name);
                return 0;
            };
        }
    }
