...
export function installSamplesRuntime(context) {
    ...
    bind('start', (channel, sample, loop = 0) => {
        ...
        context.soundWrite(channel & 0xff, (sample & 0x7f) | (loop ? 0x80 : 0), context.fraction(), 'start');
        return 0;
    });
    bind('stop', channel => {
        looping.delete(channel);
    ...
    bind('set_volume', (channel, volume) => {
        context.soundWrite(channel & 0xff, Math.max(0, Math.min(255, Math.round(Number(volume) * 255))), context.fraction(), 'set_volume');
        return 0;
    });
    for (const alias of aliases) {
