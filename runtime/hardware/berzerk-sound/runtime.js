...
import { S14001aCore } from "./berzerk-sound-worklet.js";
export function installBerzerkSoundRuntime(context) {
    ...
    for (const cpu of context.board.execution.cpus) {
        for (const range of [...(cpu.ranges ?? []), ...(cpu.io?.ranges ?? [])]) {
            ...
            if (!range.write || !['sh8253_w', 'sh6840_w', 'sfxctrl_w'].includes(method ?? ''))
                continue;
    ...
    bind('speech', 'start_w', line => {
        ...
        state.speechStart = line & 1;
        return 0;
    });
    bind('speech', 'set_unscaled_clock', clock => {
        state.speechClock = clock;
        state.speech.setClock(clock);
        context.soundWrite(0, clock, context.fraction(), 'speech_clock');
        return 0;
    });
    bind('speech', 'busy_r', () => state.speech.busy() ? 1 : 0);
    bind('s14001a_volume', 'set_gain', gain => {
        context.soundWrite(0, Math.round(gain * 255), context.fraction(), 'speech_gain');
        return 0;
    });
    return {
        state,
        tickCpu: (cpuTag, cycles) => {
            if (!ownerCpu || cpuTag !== ownerCpu.tag || cycles <= 0)
                return;
