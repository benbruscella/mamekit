...
class ExidySoundCore {
    timers = Array.from({ length: 3 }, () => ({
        cr: 0, state: 0, leftovers: 0, timer: 0, counter: 0, clocks: 0,
    }));
    volumes = new Float64Array(3);
    msbLatch = 0;
    sfxctrl = 0;
    clockCarry = 0;
    lfsrOldXor = 0;
    ...
    sample(outputRate) {
        ...
        if ((timer0.cr & 1) === 0) {
            ...
            if (noisy && !(this.sfxctrl & 1))
                noiseClocks = this.updateNoise(clocksThisSample);
            ...
            if (timer1.state && (timer1.cr & 0x80))
                sample += this.volumes[1];
            ...
            if (timer2.state && (timer2.cr & 0x80))
                sample += this.volumes[2];
        }
        return sample / 32768;
    }
    applyClock(timer, initialClocks) {
...
class ExidyPit8253Core {
    ...
    write(offset, data) {
        ...
        if (offset === 3) {
            ...
            if (access === 0)
                return; // counter latch command; the sound CPU never reads it
...
export class S14001aCore {
    rom;
    phase1 = false;
    state1 = SpeechState.Idle;
    state2 = SpeechState.Idle;
    darHigh1 = 0;
    darHigh2 = 0;
    darLow1 = 0;
    darLow2 = 0;
    ...
    clockCarry = 0;
    clockRate = 19_531.25;
    constructor(rom) { this.rom = rom; }
    /** Save-state roots (machine-state.ts): every field but the ROM. */
    stateKeys() { return Object.keys(this).filter(key => key !== 'rom'); }
    start(word) {
        this.word = word & 0x3f;
        if (!this.startLine)
            this.state1 = SpeechState.WordWait;
        this.startLine = true;
    ...
    busy() { return this.busy1 || this.state1 !== SpeechState.Idle; }
    ...
    read(address) { return this.rom[address & 0xfff] ?? 0; }
    clock() {
        if (this.phase1) {
            this.phase1 = false;
    ...
    play() {
        ...
        if (this.voiced2 && (this.length2 & 1))
            deltaAddress ^= 3;
        ...
        if (this.voiced2 && quarterStart && (quarter & 1))
            increment = 0;
        ...
        if (!this.voiced2 && this.darCarry2)
            this.darHigh1 = (this.darHigh1 + 1) & 0x1ff;
        this.romAddress1 = this.darLow1;
        if (this.voiced2 && (this.length1 & 1))
            this.romAddress1 ^= 0x1f;
        this.romAddress1 = (this.darHigh1 << 3) | (this.romAddress1 >>> 2);
        if (this.startLine)
            this.state1 = SpeechState.WordWait;
        else if (this.stop2 && this.lengthCarry2)
            this.state1 = SpeechState.Delay;
        else if (this.lengthCarry2) {
            this.state1 = SpeechState.DarMsb;
            this.romAddress1 = this.cwar1;
        }
        else
            this.state1 = SpeechState.Play;
    }
}
export class GeneratedBerzerkSoundCore {
    exidy = new ExidySoundCore();
    pit = new ExidyPit8253Core();
    speech;
    outputRate;
    venture;
    speechGain = 0;
    constructor(outputRate, speechRom = new Uint8Array(0x1000), venture = false) {
        this.outputRate = outputRate;
        this.speech = new S14001aCore(speechRom);
        this.venture = venture;
    }
    write(offset, data, method = 'sh6840_w') {
        if (method === 'sh6840_w')
            this.exidy.write6840(offset, data);
        else if (method === 'sfxctrl_w')
            this.exidy.writeControl(offset, data);
        else if (method === 'sh8253_w')
            this.pit.write(offset, data);
        else if (method === 'speech_start')
            this.speech.start(data);
        else if (method === 'speech_clock')
            this.speech.setClock(data);
        else if (method === 'speech_gain')
            this.speechGain = (data & 0xff) / 255;
    }
    sample() {
        if (this.venture) {
        ...
        constructor() {
            ...
            this.port.onmessage = (event) => {
                ...
                if (message.type === 'init') {
                    this.renderer = new GeneratedBerzerkSoundFrameRenderer(new GeneratedBerzerkSoundCore(sampleRate, message.sampleRom, message.deviceType === 'EXIDY_VENTURE'), sampleRate, message.refresh ?? 60);
                }
