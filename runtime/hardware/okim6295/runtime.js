...
export function installAuxiliaryOkim6295Runtime(context, device) {
    ...
    return {
        tickCpu: (cpuTag, cycles) => {
            ...
            // audio CPU by name.
            const audioCpu = context.board.execution.cpus.find(cpu => (cpu.ranges ?? []).some(range => range.write === methodName('write'))) ??
