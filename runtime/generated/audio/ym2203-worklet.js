...
class GeneratedYm2610AdpcmA {
    rom;
    regs = new Uint8Array(ADPCM_A.registers);
    playing = new Uint8Array(ADPCM_A.channels);
    nibble = new Uint8Array(ADPCM_A.channels);
    currentByte = new Uint8Array(ADPCM_A.channels);
    address = new Uint32Array(ADPCM_A.channels);
    accumulator = new Uint16Array(ADPCM_A.channels);
    stepIndex = new Uint8Array(ADPCM_A.channels);
    constructor(rom) {
        this.rom = rom;
        this.reset();
    }
    ...
    clock() {
        ...
        for (let channel = 0; channel < ADPCM_A.channels; channel++) {
            ...
            this.stepIndex[channel] = clamp(this.stepIndex[channel] + ADPCM_A.stepIncrement[data & 7], 0, ADPCM_A.steps.length - 1);
...
export class GeneratedYm2203Chip {
    ...
    constructor(deviceType = 'YM2203', sampleRom) {
    ...
    write(offset, data) {
        ...
        if ((offset & 1) === 0) {
            ...
            const selector = plan.prescale.selectors.find(candidate => bank === 0 && candidate.address === this.addresses[bank] &&
...
export class GeneratedYm3526Chip {
    ...
    constructor(clock, outputRate) {
        ...
        if (!opl)
            throw new Error('YM3526 plan was not emitted');
    ...
    write(port, data) {
        if ((port & 1) === 0)
            ...
            this.regs[this.address] = data & 0xff;
    }
    sample() {
        const opl = ym3526Plan;
    ...
    updateKey(operator, on, blockFreq) {
    ...
    envelopeRate(operator, state, blockFreq = 0) {
        const opl = ym3526Plan;
    ...
    clockEnvelope(operator, blockFreq) {
        const opl = ym3526Plan;
        const offset = opl.operatorOffsets[operator];
        ...
        sustain |= (sustain + 1) & 0x10;
        if (this.envelopeState[operator] === FM.egDecay &&
    ...
    operatorSample(operator, modulation) {
        const opl = ym3526Plan;
        const offset = opl.operatorOffsets[operator];
        if (this.envAttenuation[operator] > FM.egQuiet)
            return 0;
        ...
        if (ksl !== 0) {
            ...
            const attenuation = Math.max(0, table[bitfield(blockFreq, 6, 4)] - 8 * (bitfield(blockFreq, 10, 3) ^ 7));
...
export class GeneratedYm2203Mixer {
    ...
    lastSample = 0;
    portsPerChip;
    constructor(clock, chips, outputRate, routes, auxiliaryDevices, deviceType = 'YM2203', sampleRom) {
        this.chips = Array.from({ length: Math.max(0, chips) }, () => new GeneratedYm2203Chip(deviceType, sampleRom));
        ...
        const oplDevices = (auxiliaryDevices ?? []).filter(device => device.type === 'YM3526' || device.type === 'YM3812');
        this.oplChips = oplDevices.map(device => new GeneratedYm3526Chip(device.clock, outputRate));
    ...
    write(offset, data, method) {
        ...
        if (offset >= primaryPorts) {
            ...
            if (method === 'reset')
                this.oplChips[opl]?.reset();
            else
                this.oplChips[opl]?.write((offset - primaryPorts) & 1, data);
            return;
        }
    ...
    constructor() {
        ...
        this.port.onmessage = (event) => {
            ...
            if (message.type === 'init') {
                this.mixer = new GeneratedYm2203Mixer(message.clock ?? 1_500_000, message.chips ?? 1, sampleRate, message.routes, message.auxiliaryDevices, message.deviceType, message.sampleRom);
