...
export class GeneratedAy8910Core {
    ...
    volumeTables;
    constructor(clock, resistorLoads) {
        this.nativeRate = clock / plan.clockDivider;
        const table = (load) => plan.singleOutput.resistances.map((resistance, index) => {
        ...
        this.volumeTables = Array.from({ length: plan.channels }, (_, channel) => resistorLoads?.[channel] ? table(resistorLoads[channel]) : plan.volumeTable);
    }
    write(reg, data) {
    ...
    sampleChannels(output, analogOutputs = true) {
        ...
        for (let channel = 0; channel < plan.channels; channel++) {
            ...
            if (!plan.singleOutput.zeroIsOff || tiedLevel !== 0 || envelopeEnabled)
                pullups++;
        ...
        this.singleOutput = (drivenConductance + pullupConductance) / (conductance + pullupConductance +
...
export class GeneratedDac8Core {
    ...
    integrating = false;
    referenceLevels;
    constructor(bits = 8, referenceLevels) {
        this.mask = (1 << bits) - 1;
    ...
    write(method, data) {
        if (method === 'data_w' || method === 'write')
            this.value = data & this.mask;
...
export class GeneratedAy8910Mixer {
    ...
    discreteGraph;
    discreteGraphValues = new Map();
    discreteGraphOutputs = 1;
    outputRate;
    deviceTags;
    muted = false;
    constructor(clock, chips, outputRate, routes = [], auxiliaryDevices = [], discreteMixer, deviceTags = [], resistorLoads = []) {
        this.outputRate = outputRate;
        this.deviceTags = deviceTags;
        const count = Math.max(1, chips);
        this.cores = Array.from({ length: count }, (_, chip) => new GeneratedAy8910Core(clock, resistorLoads[chip]));
        ...
        this.sourceAnalogMix = auxiliaryDevices.some(device => (device.referenceLevels?.length ?? 0) > 0) ||
        ...
        this.auxiliary = auxiliaryDevices.flatMap(device => device.type === 'MSM5205' && msmPlan
            ...
            : device.type === 'DAC_4BIT_R2R' || device.type === 'DAC_8BIT_R2R'
                ? [{
                        ...
                        gain: device.gain,
                        outputGain: 1,
                        core: new GeneratedDac8Core(device.type === 'DAC_4BIT_R2R' ? 4 : 8, device.referenceLevels),
                    }]
                : []);
        for (const device of this.auxiliary) {
            const definition = auxiliaryDevices.find(candidate => candidate.deviceTag === device.deviceTag);
    ...
    beginSample() {
        for (const device of this.auxiliary) {
            if (device.core instanceof GeneratedDac8Core)
                device.core.beginSample();
        }
    }
    isTimedWrite(method) {
        return this.timedWrites.has(method ?? '');
    }
    writeAt(offset, data, method, position) {
        const auxiliaryWrite = this.auxiliaryWrites.get(method ?? '');
    ...
    sample() {
        ...
        if (this.discreteGraph)
            return this.sampleDiscreteGraph();
    ...
    resetDiscreteGraph(graph) {
        const rate = this.outputRate;
        const charge = (rc) => rc > 0 ? 1 - Math.exp(-1 / rate / rc) : 0;
        this.discreteGraph = graph.map(spec => {
            ...
            if (spec.op === 'stream') {
                ...
                if (spec.r2)
                    conductance += 1 / spec.r2;
                if (spec.r3)
                    conductance += 1 / spec.r3;
                ...
                let conductance = 0;
                for (const resistance of spec.resistances)
                    conductance += 1 / resistance;
                if (spec.rF && type === 'resistor')
                    conductance += 1 / spec.rF;
                if (type === 'opAmpWithRi')
                    conductance += 1 / spec.rI;
                ...
                step.exponents.push(spec.cF ? charge((type === 'resistor' ? step.rTotal : spec.rF) * spec.cF) : 0, 
    ...
    sampleDiscreteGraph() {
        ...
        const read = (operand) => operand === undefined ? 0 : 'node' in operand ? values.get(operand.node) ?? 0 : operand.value;
        let output = 0;
        for (const step of this.discreteGraph) {
            ...
            switch (spec.op) {
                ...
                case 'multiply':
                    ...
                    break;
                case 'crFilter': {
                    const vOut = read(spec.input) - state[0];
                ...
                case 'opAmpFilter': {
                    ...
                    if (spec.r2)
                        current += (read(spec.inputs[1]) - vRef) / spec.r2;
                ...
                case 'mixer': {
                    ...
                    if (spec.mixerType === 'opAmpWithRi')
                        current += vRef / spec.rI;
                    let v = current * (opAmp ? spec.rF : step.rTotal);
                    if (spec.mixerType === 'opAmpWithRi')
                        v = vRef + step.filterGain * (vRef - v);
                ...
                case 'output':
                    ...
                    output += read(spec.input) * spec.gain / 32768;
                    break;
            }
        }
    ...
    sampleDiscreteMixer() {
        ...
        for (const stage of mixer.mixers) {
            ...
            let value = stage.inputs.reduce((sum, input, index) => sum + (values.get(input) ?? 0) / stage.resistances[index], 0) / conductance;
        ...
        const output = mixer.outputs.reduce((sum, candidate) => sum + (values.get(candidate.node) ?? 0) * candidate.gain, 0) / Math.max(1, mixer.outputs.length);
        return Math.max(-1, Math.min(1, output));
    }
    recalculate(filter) {
        if (filter.c === 0) {
    ...
    render(writes) {
        ...
        for (let sampleIndex = 0; sampleIndex < count; sampleIndex++) {
            ...
            while (writeIndex < writes.length) {
                ...
                const exactSample = Math.max(0, Math.min(count, (write.frac ?? 0) * count));
                if (exactSample > sampleIndex + 1)
                    break;
                if (this.mixer.isTimedWrite(write.method)) {
                    this.mixer.writeAt(write.offset, write.data, write.method, exactSample - sampleIndex);
                }
    ...
    constructor() {
        ...
        this.port.onmessage = (event) => {
            ...
            if (message.type === 'init') {
                this.mixer = new GeneratedAy8910Mixer(message.clock ?? 1_789_772, message.chips ?? 1, sampleRate, message.routes, message.auxiliaryDevices, message.discreteMixer, message.deviceTags, message.resistorLoads);
