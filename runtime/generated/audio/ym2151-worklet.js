...
export class GeneratedSampleSpace {
    ...
    select(method, entry) {
        ...
        for (const window of this.windows) {
            if (!window.bank || method !== 'bank.' + window.bank.tag)
                continue;
            const offset = window.bank.entryOffsets[entry];
            if (offset !== undefined && offset !== null)
                this.fill(window, offset);
            selected = true;
        }
        return selected;
    }
...
export class GeneratedYm2151Mixer {
    ...
    constructor(clock, chips, outputRate, sampleRom, auxiliaryDevices = [], routes) {
        ...
        const okiSpace = oki?.sampleMap?.length
            ...
            : undefined;
        if (okiSpace)
            this.sampleSpaces.push(okiSpace);
        if (oki)
            this.oki = {
                tag: oki.deviceTag,
                gain: oki.gain,
                core: new OkiCore(okiSpace?.bytes ?? sampleRom ?? new Uint8Array(), oki.clock, outputRate, oki.initialMode !== 'PIN7_LOW'),
            };
    ...
    write(offset, data, method) {
        if (method?.startsWith('bank.')) {
            for (const space of this.sampleSpaces)
                space.select(method, data);
            return;
        }
