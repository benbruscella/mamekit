...
class GeneratedHD63701Y0 implements Cpu {
  ...
  private readonly portData = new Uint8Array(1);
  private readonly portDirection = new Uint8Array(1);
  ...
  step(): number {
    ...
    let dispatches = 0;
    while (true) {
      if (++dispatches > 8) throw new Error('HD63701Y0 dispatch loop exceeded 8');
  ...
  private readMemory(address: number): number {
    ...
    if ((location >= 64 && location <= 319)) return this.internalRam[location];
...
export const cpu: GeneratedCpuExecutable = {
  ...
  create: (bus: CpuBus): Cpu => new GeneratedHD63701Y0(bus),
};
