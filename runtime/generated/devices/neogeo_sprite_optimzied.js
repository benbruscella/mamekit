...
definition.compiledMethods = (() => {
    function method_draw_fixed_layer(runtime, bitmap, scanline) {
        ...
        for (let x = 0; ((Number(x) < Number(40)) ? 1 : 0); x = ((x) + (1))) {
            let code_and_palette = (((video_data).source[(video_data).offset]) & 0xffff);
            let code = ((((code_and_palette) & (4095))) & 0xffff);
            let gfx_offset = ((((((code) << (5))) | (((scanline) & (7))))) & (addr_mask));
    ...
    function method_draw_sprites(runtime, bitmap, scanline) {
        ...
        for (let sprite_index = 0; ((Number(sprite_index) <= Number(max_sprite_index)) ? 1 : 0); sprite_index = ((sprite_index) + (1))) {
            ...
            if (method_sprite_on_scanline(runtime, scanline, y, rows)) {
                let sprite_line = ((((scanline) - (y))) & (511));
                let zoom_line = ((sprite_line) & (255));
                ...
                let sprite_y = ((sprite_y_and_tile) & (15));
                let tile = ((sprite_y_and_tile) >>> (4));
                if (invert) {
                ...
                let zoom_x_table = ((([128, 2176, 2184, 10376, 10378, 10890, 10922, 43690, 43754, 47850, 47851, 48107, 48111, 64495, 64511, 65535][(((zoom_x) % 16) + 16) % 16] ?? 0)) & 0xffff);
                let gfx_base = ((((((code) << (8))) | (((sprite_y) << (4))))) & ((members.m_sprite_gfx_address_mask ?? runtime.member("m_sprite_gfx_address_mask"))));
    ...
    function method_draw_pixel(runtime, romaddr, dst, line_pens) {
        const members = runtime.members;
        let src = runtime.addressOf((members.m_region_sprites ?? runtime.member("m_region_sprites")), ((((((((romaddr) & ((~255)))) >>> (1))) | (((((((romaddr) & (8))) ^ (8))) << (3))))) | (((((romaddr) & (240))) >>> (2)))));
        let x = ((romaddr) & (7));
