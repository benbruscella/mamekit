...
// - src/mame/dataeast/decocpu7.cpp
// - src/mame/dataeast/decocpu7.h
import type {
  Cpu,
  CpuBus,
...
const GENERATED_METHOD_NAMES = new Set<string>(["do_adc","do_adc_d","do_adc_nd","do_arr","do_arr_d","do_arr_nd","do_sbc","do_sbc_d","do_sbc_nd","do_cmp","do_bit","do_asl","do_lsr","do_ror","do_rol","do_asr","set_nz","set_l","set_h","page_changing","dec_SP","inc_SP","memory_read","memory_opcode","memory_arg","memory_write"]);

class GeneratedDECO_CPU7 implements Cpu {
  ...
  step(): number {
    ...
    let dispatches = 0;
    while (true) {
      if (++dispatches > 8) throw new Error('DECO_CPU7 dispatch loop exceeded 8');
      ...
      case 0xab00: {
        this.m_A = (((this.m_X = ((this.method_memory_arg(this.m_PC)) & 0xff))) & 0xff);
        this.m_PC = ((((this.m_PC) + (1))) & 0xffff);
        this.method_set_nz(this.m_A);
        return this.cycles;
      }
      ...
      case 0xff00: {
        ...
        default:
          throw new Error('DECO_CPU7 has no generated opcode ' +
  ...
  get(name: string): number {
    switch (name) {
      ...
      case "m_had_written": return this.m_had_written;
      default: return 0;
    }
  }
  ...
  set(name: string, value: number): void {
    switch (name) {
      ...
      case "m_had_written": this.m_had_written = ((value) ? 1 : 0); return;
      default: return;
    }
  }
  ...
  invoke(name: string, ...args: number[]): number {
    switch (name) {
      ...
      default: throw new Error('DECO_CPU7 has no generated method "' + name + '"');
    }
  }
  ...
  private method_memory_read(adr: number = 0): number {
    return (++this.cycles, this.readMemory((adr) & 65535) & 0xff);
    return 0;
  }

  private method_memory_opcode(adr: number = 0): number {
    let res = (((++this.cycles, this.readOpcode((adr) & 0xffff) & 0xff)) & 0xff);
    if (this.m_had_written) {
      this.m_had_written = ((0) ? 1 : 0);
      if (((Number(((adr) & (260))) === Number(260)) ? 1 : 0)) {
        res = ((bitswap(res, 6, 5, 3, 4, 2, 7, 1, 0)) & 0xff);
      }
    }
    return res;
    return 0;
...
export const cpu: GeneratedCpuExecutable = {
  type: "DECO_CPU7",
  summary: {"opcodes":256,"compiledOpcodes":256,"methods":26,"compiledMethods":26,"diagnostics":0},
  accessors: {"get_sync":"m_sync"},
  create: (bus: CpuBus): Cpu => new GeneratedDECO_CPU7(bus),
};
