...
definition.compiledMethods = (() => {
    function method_read_sample(runtime, channel, addr) {
    ...
    function method_sound_stream_update(runtime, stream) {
        const members = runtime.members;
        const h_m_channel = members.m_channel ?? runtime.member("m_channel");
        for (let j = ((0) | 0); ((Number(j) < Number((typeof (runtime.dereference(stream)).samples === 'function' ? (runtime.dereference(stream)).samples() : typeof (runtime.dereference(stream)).samples === 'number' || typeof (runtime.dereference(stream)).samples === 'boolean' ? (runtime.dereference(stream)).samples : runtime.container(stream, "samples")))) ? 1 : 0); j = ((((j) + (1))) | 0)) {
            let lsum = ((0) | 0);
            let rsum = ((0) | 0);
            for (let i = ((0) | 0); ((Number(i) < Number(2)) ? 1 : 0); i = ((((i) + (1))) | 0)) {
                let channel = h_m_channel[i];
                if (channel.play) {
                    let vol_a = ((((channel.vol[0]) * (2))) | 0);
                    let vol_b = ((((channel.vol[1]) * (2))) | 0);
                    let addr = ((((channel.addr) & (131071))) >>> 0);
                    ...
                    let out = ((((((method_read_sample(runtime, i, addr)) & (127))) - (64))) | 0);
