...
class GeneratedHD63701Y0 {
    ...
    step() {
        ...
        while (true) {
            if (++dispatches > 8)
                throw new Error('HD63701Y0 dispatch loop exceeded 8');
            switch ((this.m_ref >>> 8) & 0xffff) {
                ...
                default:
                    throw new Error('HD63701Y0 has no generated opcode ' +
    ...
    readMemory(address) {
        ...
        if (location === 23) {
            ...
            const input = Number(this.bus.signal?.("in_p6_cb", 0) ?? 0xff) & 0xff;
            return direction === 0xff
                ? this.portData[0]
                : (input & ~direction) | (this.portData[0] & direction);
        }
        if ((location >= 64 && location <= 319))
    ...
    invoke(name, ...args) {
        switch (name) {
            ...
            default: throw new Error('HD63701Y0 has no generated method "' + name + '"');
        }
    }
    generatedStart() {
    }
...
export const cpu = {
    ...
    create: (bus) => new GeneratedHD63701Y0(bus),
};
export default cpu;
