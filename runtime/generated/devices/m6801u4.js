...
class GeneratedM6801U4 {
    bus;
    irqData = 0xff;
    irqHold = false;
    internalRam = new Uint8Array(0x10000);
    portData = new Uint8Array(4);
    portDirection = new Uint8Array(4);
    ...
    step() {
        ...
        while (true) {
            if (++dispatches > 8)
                throw new Error('M6801U4 dispatch loop exceeded 8');
    ...
    readMemory(address) {
        ...
        if (location === 6) {
            ...
            const data = (this.portHandshakeControl & 8) ||
                direction === 0xff
                ? this.portData[2]
    ...
    updateInternalInput(inputnum, state) {
        ...
        if (!this.portHandshakeInputState &&
            ...
            const direction = this.portDirection[2];
            const input = Number(this.bus.signal?.("in_p3_cb", 0) ?? 0xff) & 0xff;
            this.portData[2] =
...
export const cpu = {
    ...
    create: (bus) => new GeneratedM6801U4(bus),
};
export default cpu;
