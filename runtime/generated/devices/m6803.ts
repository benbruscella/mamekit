...
class GeneratedM6803 implements Cpu {
  ...
  private readonly portData = new Uint8Array(2);
  private readonly portDirection = new Uint8Array(2);
  ...
  step(): number {
    ...
    let dispatches = 0;
    while (true) {
      if (++dispatches > 8) throw new Error('M6803 dispatch loop exceeded 8');
  ...
  private readMemory(address: number): number {
    ...
    if ((location >= 128 && location <= 255)) return this.internalRam[location];
  ...
  private method_increment_counter(amount: number): number {
...
export const cpu: GeneratedCpuExecutable = {
  ...
  create: (bus: CpuBus): Cpu => new GeneratedM6803(bus),
};
