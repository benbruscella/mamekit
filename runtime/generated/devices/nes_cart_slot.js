...
definition.slot.options["1"].compiledMethods = (() => {
    ...
    function method_read_m(runtime, offset) {
        const members = runtime.members;
        let bank = (((((runtime.readIndex((members.m_reg ?? runtime.member("m_reg")), 1)) >>> (2)) & ((1 << (2)) - 1))) & 0xff);
        0;
...
definition.slot.options["2"].compiledMethods = (() => {
    ...
    function method_update_prg_banks(runtime, prg_bank_start, prg_bank_end) {
        const members = runtime.members;
        for (let prg_bank = prg_bank_start; ((Number(prg_bank) <= Number(prg_bank_end)) ? 1 : 0); prg_bank = ((prg_bank) + (1))) {
            0;
...
definition.slot.options["3"].compiledMethods = (() => {
    ...
    function method_bank_chr(runtime, shift, start, bank, source) {
        ...
        let size = ((1024) << (shift));
        let bank_start = ((bank) * (size));
        let kbyte = ((1) << (shift));
        for (let i = 0; ((Number(i) < Number(kbyte)) ? 1 : 0); i = ((i) + (1))) {
