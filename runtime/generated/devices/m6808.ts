...
class GeneratedM6808 implements Cpu {
  ...
  step(): number {
    ...
    let dispatches = 0;
    while (true) {
      if (++dispatches > 8) throw new Error('M6808 dispatch loop exceeded 8');
  ...
  private readMemory(address: number): number {
    ...
    if ((location >= 0 && location <= 127)) return this.internalRam[location];
...
export const cpu: GeneratedCpuExecutable = {
  ...
  create: (bus: CpuBus): Cpu => new GeneratedM6808(bus),
};
