...
export function mediumNoun(kind, count = 2) {
...
const TILE_W = 200;
const TILE_H = 250;
function labelText(o, x, y, wrap, size, dim) {
    const lines = wrapCartTitle(o.title, wrap);
    const titleColor = dim ? '#8b8b86' : '#181818';
    const subColor = dim ? '#7a7a75' : '#5f5744';
    const title = lines.map((line, index) => `<text x="${x}" y="${y + index * (size + 3)}" font-family="ui-sans-serif,system-ui,sans-serif" font-size="${size}" font-weight="800" fill="${titleColor}">${esc(line)}</text>`).join('');
...
function cartridgeSvg(o, hue, dim) {
    const stripe = stripeFor(o, hue);
...
export function mediaSvg(kind, o) {
    ...
    for (const catalog of catalogs) {
        ...
        for (const [index] of hits) {
            ...
            if (!best || matched > best.matched)
                best = { list: catalog.list, entry, matched };
        }
    }
    return best;
}
 ...
 */
export function mountableImages(members, extensions, entry) {
    ...
    return images.sort((a, b) => (order.get(a.name.toLowerCase()) ?? 1e9) - (order.get(b.name.toLowerCase()) ?? 1e9));
}
// --- the room ---------------------------------------------------------------------------
export async function runSoftwareRoom(cfg) {
    ...
    let inRoom = true;
    let modalDepth = 0;
    const shelfFor = (list) => shelves.find(shelf => shelf.list === list);
    ...
    function loadShelf(shelf) {
        ...
        pending = (async () => {
            try {
                ...
                if (r.ok)
                    catalogs.set(shelf.list, await r.json());
            }
    ...
    function button(text, solid, enabled) {
    ...
    // --- tabs: one per shelf ------------------------------------------------------------
    let active = shelves.find(shelf => shelf.mountable) ?? shelves[0];
    ...
    function render(reset = false) {
        ...
        const matches = rows.filter(row => {
            if (filter === 'verified' && !row.avail)
                return false;
            if (filter === 'working' && row.entry.supported === 'no')
                return false;
    ...
    function smallBtn(text, attr, solid, enabled) {
    ...
    function buildTile(row) {
        ...
        const state = !shelf.mountable ? 'display' : avail ? 'available' : 'catalog';
      ...
      transition:opacity .15s ease;box-shadow:0 6px 14px rgba(0,0,0,.55)`;
        ...
        const reveal = (shown) => { fetchBtn.style.opacity = shown ? '1' : '0'; infoBtn.style.opacity = shown ? '1' : '0'; };
    ...
    async function fetchSet(row, cover, label) {
        const avail = row.avail;
        if (fetching.has(avail.file))
            return;
        ...
        try {
            ...
            if (!bytes) {
                toast(`${row.entry.description}: no web source had it — drop your own dump instead`);
                return;
            }
     ...
     */
    async function ingest(name, bytes, hint) {
        if (bytes.length > MAX_SET) {
            toast(`${name}: bigger than 16 MiB — not a software set`);
            return false;
        }
        ...
        const shelf = shelfFor(identified?.list) ?? hint ?? shelves.find(candidate => mountableImages(members, candidate.extensions).length > 0);
        if (!shelf) {
            toast(`${name}: no image inside that any shelf here accepts`);
            return false;
        }
    ...
    async function boot(rec) {
        ...
        if (rec) {
            const shelf = shelfFor(rec.list);
            if (!shelf?.mountable) {
                toast(`${rec.name}: this medium does not mount yet`);
                return;
            }
            ...
            let members;
            try {
                members = isZip ? await readZip(bytes) : new Map([[rec.name, bytes]]);
            }
            catch {
                toast(`${rec.name}: could not read the set`);
                return;
            }
            images = mountableImages(members, shelf.extensions, ownedEntry(rec));
            if (!images.length) {
                toast(`${rec.name}: no image inside to mount`);
                return;
            }
    ...
    function openInfo(row) {
      ...
      background:linear-gradient(#141838,#0c0f24);border:2px solid ${GOLD};box-shadow:0 24px 80px rgba(0,0,0,.8);font:14px/1.55 ui-sans-serif,system-ui`);
        ...
        if (soft.year)
            fact('Year', soft.year);
        if (soft.publisher)
            fact('Publisher', soft.publisher);
        fact('Set', soft.name + (soft.cloneof ? ` (clone of ${soft.cloneof})` : ''));
        if (soft.parts)
            fact('Parts', String(soft.parts));
        if (soft.supported)
            fact('MAME status', soft.supported === 'no' ? 'not working' : 'partially working');
        for (const rom of soft.prg.roms)
            fact(rom.file ?? 'image', `${(rom.size / 1024).toFixed(0)} KB · crc ${rom.crc}`);
        ...
        if (shelf.mountable) {
            ...
            go.addEventListener('click', () => {
                close();
                if (avail) {
                    const tile = grid.querySelector(`[data-catalog-set="${CSS.escape(soft.name)}"]`);
    ...
    async function handleFiles(files) {
        for (const f of files) {
            if (f.size > MAX_SET) {
                toast(`${f.name}: bigger than 16 MiB — not a software set`);
                continue;
            }
            let bytes;
            try {
    ...
    addEventListener('keydown', ev => {
        if (!inRoom || modalDepth > 0 || ev.target === search)
            return;
        if (ev.key === 'Escape')
            location.href = './?tab=computers';
    });
    // --- go ----------------------------------------------------------------------------------
    paintTabs();
    if (active)
        void showShelf(active);
    else {
        const empty = el('div', 'grid-column:1/-1;padding:38px;color:#7982b8;text-align:center');
    ...
    await Promise.all([...new Set(recs.map(rec => rec.list).filter(Boolean))]
        .map(list => shelfFor(list)).filter((shelf) => Boolean(shelf)).map(loadShelf));
