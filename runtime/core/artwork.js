 ...
 */
export async function loadArtwork(game, prefer) {
    try {
...
async function layArtwork(files) {
    ...
    if (!view)
        return null;
    const findFile = (name) => files.get(name) ?? files.get(name.toLowerCase());
    const png = findFile(view.file);
    if (!png)
        return null;
    let bmp = await createImageBitmap(new Blob([png.slice().buffer], { type: 'image/png' }));
...
export function parseArtworkLayout(source) {
    ...
    for (const match of lay.matchAll(/<view\s+name="([^"]+)"[^>]*>([\s\S]*?)<\/view>/g)) {
        ...
        for (const kind of ['bezel', 'backdrop']) {
            const legacy = new RegExp(`<${kind}\\s+[^>]*element="([^"]+)"[^>]*>[\\s\\S]*?<\\/${kind}>`).exec(body);
            if (legacy) {
                artTag = legacy[0];
                artElement = legacy[1];
                break;
...
export function findWindow(bmp) {
    ...
    const clear = (x, y) => alpha[(y * w + x) * 4 + 3] < 16;
    const cx = w >> 1, cy = h >> 1;
    if (!clear(cx, cy))
        return null; // center is painted — no window (marquee art etc.)
