...
// half-shipped pair.
import { webArtworkUrl } from "./artwork-source.js";
/**
 ...
 */
export async function loadArtwork(game) {
 ...
 */
async function loadShippedBezel(game) {
    try {
 ...
 */
export function composeBezel(source, view) {
    let bmp = source;
    ...
    if (view.bounds) {
        ...
        const cx = c.getContext('2d');
        cx.drawImage(bmp, (view.art.x - view.bounds.x) * artScaleX, (view.art.y - view.bounds.y) * artScaleY);
        bmp = c;
    }
 ...
 */
export function parseArtworkLayout(source) {
    const lay = source.replace(/<!--[\s\S]*?-->/g, '');
    const elementBodies = new Map();
    const images = new Map();
    const alphaImages = new Map();
    for (const match of lay.matchAll(/<element\s+name="([^"]+)"[^>]*>([\s\S]*?)<\/element>/g)) {
        ...
        if (file)
            images.set(match[1], file);
        const alpha = /<image\s[^>]*?\balphafile="([^"]+)"/.exec(match[2])?.[1];
        if (alpha)
            alphaImages.set(match[1], alpha);
    }
    const views = [];
    for (const match of lay.matchAll(/<view\s+name="([^"]+)"[^>]*>([\s\S]*?)<\/view>/g)) {
        ...
        let artTag;
        let artElement;
        for (const kind of ['bezel', 'backdrop']) {
            const legacy = [...body.matchAll(new RegExp(`<${kind}\\s+[^>]*element="([^"]+)"[^>]*>[\\s\\S]*?<\\/${kind}>`, 'g'))].sort((left, right) => {
        ...
        const overlayPlacement = overlayCollection
            ? placedLayoutElements(overlayCollection).find(candidate => /\bblend="multiply"/.test(candidate.tag))
            : undefined;
        const tints = overlayPlacement
            ? layoutTints(elementBodies.get(overlayPlacement.ref) ?? '', layoutBounds(overlayPlacement.tag), screen)
            : [];
        const rotate = Number(/<orientation\s+[^>]*rotate="(\d+)"/.exec(artTag ?? '')?.[1] ?? 0);
        views.push({
            name,
            ...(bounds ? { bounds } : {}),
            screen,
    ...
    const frames = (view) => view.art.x <= view.screen.x &&
    ...
    // it always had.
    const nameScore = (view) => /bezel/i.test(view.name) ? 2 : /upright/i.test(view.name) ? 1 : 0;
    ...
    const sharp = (view) => view.art.w >= SHIPPED_BEZEL_WIDTH;
    views.sort((a, b) => Number(frames(b)) - Number(frames(a)) ||
        nameScore(b) - nameScore(a) ||
        Number(sharp(b)) - Number(sharp(a)));
    return views[0] ?? null;
}
function placedLayoutElements(source) {
    return [...source.matchAll(/<element\s+[^>]*ref="([^"]+)"[^>]*>[\s\S]*?<\/element>/g)].map(match => ({ ref: match[1], tag: match[0] }));
}
function layoutBounds(source) {
    ...
    const w = attrs.width ?? (Number.isFinite(attrs.right) ? attrs.right - x : Number.NaN);
    const h = attrs.height ?? (Number.isFinite(attrs.bottom) ? attrs.bottom - y : Number.NaN);
    return { x, y, w, h };
}
function layoutTints(elementBody, placement, screen) {
