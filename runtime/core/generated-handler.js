...
import { executeGeneratedMachineHandler, prepareGeneratedMachineHandler, prepareGeneratedMachineCall, } from "../ir/execute.js";
import { wireDeviceCallbacks } from "./generated-machine.js";
import { applyBoardTransforms } from "./generated-effects.js";
export { applyCombineData, applyGeneratedAndAssign, applyGeneratedDivision, applyGeneratedShiftRight, applyGeneratedMacro, compileGeneratedMachineHandler, dereferenceGeneratedValue, executeGeneratedCallbackHandler, executeGeneratedHandler, executeGeneratedMachineHandler, executeGeneratedMachineProgram, executeGeneratedProgram, generatedContainerAccessor, generatedPackedView, generatedPeriodicLines, generatedAdd, generatedPointerStore, generatedWideBinary, generatedReferent, GENERATED_FIELD_WIDTHS, prepareGeneratedMachineHandler, generatedValuesEqual, } from "../ir/execute.js";
/**
...
bindingsFor = () => bindings) {
    ...
    const deviceClasses = new Map((machine.devices ?? [])
    ...
    const cpuTagFor = (key) => machine.execution.cpus.find(cpu => [...(cpu.ranges ?? []), ...(cpu.io?.ranges ?? []), ...(cpu.opcode?.ranges ?? [])]
 ...
 */
function handlerSpace(handler, bindings, cpuTag) {
...
function makeReadHandler(machine, handler, bindings, cpuTag) {
...
function makeWriteHandler(machine, handler, bindings, cpuTag) {
    ...
    return (addr, offset, data, memMask) => {
        executeGeneratedMachineHandler(machine, handler, bindings, {
            addr,
            offset,
            data,
            state: data,
