...
export function consoleDeckSvg(W, H, opts = {}) {
    ...
    const stripe = (y) => `<rect x="${n(fx(0.045))}" y="${n(fy(y))}" width="${n(cw * 0.91)}" height="${n(ch * 0.038)}" fill="#1b1a16"/>`;
    const button = (x) => `<rect x="${n(fx(x))}" y="${n(btnY)}" width="${n(btnW)}" height="${n(btnH)}" rx="1.5" fill="#3c3a34"/>`
    ...
    const port = (x) => `<rect x="${n(fx(x))}" y="${n(portY + portH * 0.24)}" width="${n(cw * 0.038)}" height="${n(portH * 0.52)}" rx="1" fill="#24221d"/>`;
