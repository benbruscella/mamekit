...
function resolveOffset(value, regionBits) {
    if (typeof value === 'number')
        return value;
    const m = RGN_FRAC_RE.exec(value.trim());
    if (!m)
        throw new Error(`gfx: cannot parse offset "${value}"`);
    const num = Number(m[1]);
    const den = Number(m[2]);
    if (den === 0)
        throw new Error(`gfx: RGN_FRAC denominator is 0 in "${value}"`);
...
export function decodeGfx(layout, rom, byteOffset = 0) {
    ...
    const count = typeof layout.total === 'string'
        ...
        : layout.total;
    if (layout.xOffsets.length < width)
        throw new Error('gfx: xOffsets shorter than width');
    if (layout.yOffsets.length < height)
        throw new Error('gfx: yOffsets shorter than height');
    if (layout.planeOffsets.length < planes)
        throw new Error('gfx: planeOffsets shorter than planes');
    const planeOffs = new Int32Array(planes);
    for (let p = 0; p < planes; p++)
        planeOffs[p] = resolveOffset(layout.planeOffsets[p], regionBits);
    const xOffs = new Int32Array(width);
    for (let x = 0; x < width; x++)
        xOffs[x] = resolveOffset(layout.xOffsets[x], regionBits);
    const yOffs = new Int32Array(height);
    for (let y = 0; y < height; y++)
        yOffs[y] = resolveOffset(layout.yOffsets[y], regionBits);
        ...
        for (let p = 0; p < planes; p++) {
            ...
            for (let y = 0; y < height; y++) {
                ...
                for (let x = 0; x < width; x++) {
                    ...
                    if (rom[off >> 3] & (0x80 >> (off & 7)))
                        pixels[dstRow + x] = pixels[dstRow + x] | planebit;
                }
            }
        }
    }
