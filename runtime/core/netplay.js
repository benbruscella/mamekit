...
// machine is playing.
import { paintTitleButton, titleButton } from "./controls.js";
import { Session } from "./session.js";
import { createGuest, createHost, } from "./netlink.js";
/** Frames of input delay for a round trip, kept inside what feels playable. */
export function delayForRtt(rtt, refresh) {
...
export function createNetplay(options) {
    ...
    const toggle = titleButton('2 player', 'Two player game', 'Play with somebody else, in their own browser', 'normal', 'twoPlayer');
     ...
     */
    const primary = (text, label, run, kind = 'go') => {
    ...
    const secondary = (text, label, run) => {
    ...
    const pasteBox = (label, action, run) => {
        const field = document.createElement('input');
      ...
      background:#080b1e;border:1px solid #262e5e;color:#cbd1ff;font:11px ui-monospace,monospace`;
        ...
        go.onclick = submit;
        field.onkeydown = event => { if (event.key === 'Enter')
            submit(); };
     ...
     */
    const step = (number, title, done, ...items) => {
    ...
    const show = (...items) => { row.replaceChildren(...items); };
    // --- the room ------------------------------------------------------------
    const post = (message) => link?.send(message);
    /**
     ...
     */
    const startRoom = (delay, local) => {
        if (session.room)
            return; // both hellos crossed, or a `begin` arrived twice
        player = local;
        desync = undefined;
        stalled = 0;
        ...
        try {
            ...
            console.warn(`netplay: cold boot failed: ${error.message}`);
            link?.close();
            return;
        }
        session = new Session({
    ...
    const attach = (peer, host) => {
        link = peer;
        peer.onClose = reason => endRoom(reason);
        peer.onMessage = message => {
            const room = message;
            if (room.kind === 'begin') {
                startRoom(room.delay, 2);
                return;
            }
     ...
     */
    const acceptReply = (text) => {
        if (!offered || current.name !== 'inviting')
            return;
        const reject = (why) => { complaint = why; render(); };
        ...
        invite.accept(code).catch((error) => {
            stage({
                name: 'failed',
                why: `That code did not work: ${error.message}`,
                retry: 'invite',
            });
        });
    };
    ...
    const listener = globalThis.document;
    const watchPaste = (on) => {
    ...
    function render() {
        ...
        switch (current.name) {
                ...
                const paste = secondary('Paste their code', 'Paste their code', () => {
                    void readClipboard().then(text => {
                        if (text)
                            ...
                            complaint = 'This browser would not hand over the clipboard. Paste their code into the box instead.';
                            render();
                        }
                    });
                });
                show(step(1, 'Send them this link', false, send, codeBox(url, 'invite link')), step(2, 'Paste the code they send back', false, line(paste, hint('or press ⌘V anywhere on this page')), pasteBox('paste their code', 'Connect', acceptReply), ...(complaint ? [hint(complaint, 'warn')] : [])), line(secondary('Cancel', 'Cancel the invite', () => { cancelInvite(); stage({ name: 'intro' }); })));
            ...
            case 'replying': {
                ...
                show(step(1, 'You opened their invite', true, hint('Your browser answered it.')), step(2, 'Send this code back to them', false, copy, codeBox(code, 'reply code'), hint('The same way they sent you the link. The game starts the moment they paste it in.')));
                // Most browsers allow a copy without asking; do it now so the code is
                // already in hand when they go back to the conversation.
                void copyToClipboard(code).then(ok => { if (ok)
                    copy.textContent = 'Copied — send it back to them'; });
                return;
            }
            case 'joining': {
            ...
            case 'live': {
                ...
                if (link?.rtt !== undefined)
                    parts.push(`${link.rtt}ms away`);
    ...
    const invite = async () => {
        ...
        try {
            ...
            stage({ name: 'failed', why: `Could not start a two-player game: ${error.message}`, retry: 'invite' });
        }
    };
    const join = async (code) => {
        ...
        try {
            ...
            stage({ name: 'failed', why: `That invite could not be answered: ${error.message}` });
        }
    };
    ...
    toggle.onclick = () => {
        const opening = !open;
        setPanel(opening);
        if (opening && current.name === 'intro')
            void invite();
        toggle.blur();
    };
    stage({ name: 'intro' });
    if (options.joinCode)
        void join(options.joinCode);
    return {
        control,
        get live() { return session.room; },
        begin() {
            session.publish();
        ...
        status() {
            ...
            if (stalled > STALL_FRAMES)
                parts.push('⏳ waiting for player two');
            return parts.join(' · ');
        },
    };
}
