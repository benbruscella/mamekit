 ...
 */
export function titleButton(text, label, title, tone = 'normal', mark) {
...
export function setTitleButtonText(button, text) {
    const word = button.querySelector('[data-word]');
    if (word)
        word.textContent = text;
    else
        button.textContent = text;
}
export function paintTitleButton(button, active) {
    const enabled = !button.disabled;
    const tone = TONES[button.dataset.tone || 'normal'] ?? TONES.normal;
...
export function deckPanel(title) {
...
export function deckButton(text, options = {}) {
    const button = document.createElement('button');
    button.type = 'button';
    button.textContent = text;
...
export function setDeckButtonState(button, active) {
    const enabled = !button.disabled;
