import { applyCombineData, applyGeneratedAndAssign, applyGeneratedDivision, applyGeneratedShiftRight, applyGeneratedMacro, dereferenceGeneratedValue, generatedPackedView, generatedContainerAccessor, generatedAdd, generatedPointerStore, generatedWideBinary, executeGeneratedProgram, generatedReferent, generatedValuesEqual, GENERATED_FIELD_WIDTHS, } from "./generated-handler.js";
    ...
    constructor(definition, clock, options = {}) {
        ...
        this.bindings = {
            ...
            calls: {
                ...
                CALL_METHOD: (name, ...args) => this.invoke(String(name), ...args),
                logerror: () => 0,
                clock: () => clock,
                ...
                stream_alloc: (_inputs, _outputs, rate) => {
                    if (Number(rate) > 0)
                        this.members.__stream_rate = Number(rate);
    ...
    executeMethod(method, parameterNames, args) {
        try {
            if (this.definition.type.toUpperCase() === 'GENERIC_LATCH_8' &&
                ...
                if (!schedule)
                    return this.invoke('sync_callback', data);
...
function integerArray(length, bits, signed) {
    if (bits === 8)
        return signed ? new Int8Array(length) : new Uint8Array(length);
    if (bits === 16)
        return signed ? new Int16Array(length) : new Uint16Array(length);
    if (bits === 32)
        return signed ? new Int32Array(length) : new Uint32Array(length);
