...
import { openCartStore } from "./cartstore.js";
import { parseINes, identify, inesFromSoftlistSet, mountedINesPrg, } from "./nes-ines.js";
import { cartImageFromSoftlistSet, resolveFlatCart } from "./cart-identify.js";
import { readZip, crc32 } from "./zip.js";
import { artworkSources } from "./artwork-source.js";
import { consoleDeckSvg } from "./console-art.js";
import { cartAvailability, fetchRomBytes, fetchRomJson, } from "./rom-source.js";
const GOLD = '#f2c200';
 ...
 * list.
 */
export let activeShell = CART_SHELLS.nes;
/** Test seam: the room sets this from the software list's cartridge interface. */
export const useCartShell = (name) => { activeShell = shellForInterface(name); };
export const shellForInterface = (name) => name === 'a2600_cart' ? CART_SHELLS.vcs
...
const stripSet = (s) => s.replace(/\s*\(.*\)$/, ''); // drop the "(Europe, rev. A)" region suffix
const browseSlug = (value) => value.normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/&/g, ' and ')
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .replace(/-{2,}/g, '-') || 'unknown';
const browseUrl = (facet, value) => `browse/${facet}/${browseSlug(value)}/`;
function el(tag, css) {
 ...
 */
function titleLine(text, maxWidth) {
...
export function wrapCartTitle(s, max = 17) {
...
function clampCode(code, max = 26) {
...
export function cartSvg(o) {
    ...
    const titleSvg = lines.map((l, i) => `<text x="${shell.title.x}" y="${shell.title.y + i * 19}" font-family="ui-sans-serif,system-ui,sans-serif" font-size="14" font-weight="800" fill="${titleColor}">${esc(l)}</text>`).join('');
...
async function fetchOwnEntry(cfg) {
    try {
 ...
 */
function resolveRec(rec, catalog, support) {
    const bytes = new Uint8Array(rec.bytes);
    if (!support.ines)
        return resolveFlatCart(bytes, catalog, support);
    const ines = parseINes(bytes);
    return ines ? identify(ines, catalog, support) : null;
}
export async function runConsole(cfg) {
     ...
     */
    const boardOf = (slot) => slot || support.defaultSlot || '';
    const playsOnBoard = (slot) => {
        const board = boardOf(slot);
        return board !== '' && support.slots.includes(board);
    };
    const boardLabel = (slot) => {
        const board = boardOf(slot);
    ...
    const availabilityPromise = cartsUrl
        ? fetch(`../${cfg.dataPath}/${cartsUrl}`)
            ...
            .catch(() => null)
            .then(index => index ?? fetchRomJson(`${setKey}/_manifest.json`))
        : fetchRomJson(`${setKey}/_manifest.json`);
    ...
    const playable = (r) => r !== null && r.playable && coreSupported;
    const boot = (rec, resolved) => {
        inRoom = false;
        ...
        if (resolved.ines) {
            ...
            if (resolved.ines.chr)
                regions.chr = resolved.ines.chr; // omitted => CHR-RAM cart
            cart = {
     ...
     */
    const cartLink = (rec) => `${location.pathname}?cart=${encodeURIComponent(rec.id.split(':').pop() ?? rec.id)}`;
    const bootCart = (rec, resolved) => {
    ...
    hero.innerHTML = consoleDeckSvg(230, 132, { idPrefix: 'hero' });
    {
      ...
      filter:drop-shadow(0 6px 14px rgba(0,0,0,.55))`;
        photo.addEventListener('error', () => {
            if (!photo.src.endsWith(archival))
                photo.src = archival;
        });
    ...
    if (!store.persistent)
        banner('Private browsing — carts last only this session', 'data-banner-private', '#e8b64c');
    if (!coreSupported)
        banner('Console core still compiling — carts can be shelved but not played', 'data-banner-core', '#8b93c4');
    ...
    // broken.
        ...
        const chips = (entry.silentGaps ?? [])
            ...
            .join(', ');
        banner(`Cartridges play, but silently — ${chips || 'the sound hardware'} is not generated yet`, 'data-banner-silent', '#e0a53a');
    }
    ...
    const mkBtn = (text, attr, solid, enabled) => {
    ...
    const armEject = (buttons, rebuild, onConfirm) => {
        if (buttons.dataset.confirm)
            return;
        ...
        let t;
        const done = () => { clearTimeout(t); delete buttons.dataset.confirm; };
     ...
     */
    function fitLabelToArt(cover, img) {
    ...
    function applyCartArt(cover, name) {
    ...
    function renderCatalog(reset = false) {
        ...
        const matches = rows.filter(row => {
            if (filter === 'verified' && row.tier !== 'verified')
                return false;
            if (filter === 'playable' && !playsOnBoard(row.slot))
                return false;
            if (!['all', 'playable', 'verified'].includes(filter) && row.slot !== filter)
                return false;
     ...
     */
    function mergeAvailability(manifest) {
        for (const cart of cartAvailability(manifest)) {
            if (cart.tier === 'verified' && cart.name)
                availByName.set(cart.name, cart);
            else
                bucketExtras.push(cart);
        }
        if (!availByName.size && !bucketExtras.length)
            return;
    ...
    function buildOther(rec, resolved) {
        ...
        const state = resolved?.tier === 'tested' ? 'lit'
      ...
      max-width:${CART_W}px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap`);
        ...
        if (!resolved) {
            status.style.color = '#e0504d';
            status.textContent = 'CANNOT READ — EJECT';
        }
        else if (resolved.tier === 'tested') {
            status.style.color = '#5ecf7a';
            status.textContent = '✓ VERIFIED';
        }
        else if (resolved.tier === 'experimental') {
            status.style.color = '#e8b64c';
            status.textContent = 'EXPERIMENTAL — UNTESTED';
        }
        else {
            status.style.color = '#8b93c4';
            status.textContent = (resolved.reason ?? 'MAPPER NOT SUPPORTED').toUpperCase();
        }
    ...
    async function removeOther(o) {
        try {
            await store.remove(o.rec.id);
        }
        catch { /* in-memory / gone */ }
    ...
    const cards = () => [...others, ...catalogTiles];
    let selected = -1;
    const select = (i) => {
        const list = cards();
        if (!list.length) {
    ...
    const flash = (item) => {
        ...
        const list = cards();
        setTimeout(() => { if (list[selected]?.item !== item)
            item.style.outline = 'none'; }, 1400);
    };
    ...
    function route(rec, resolved, announce) {
    ...
    function openModal(build) {
      ...
      border-radius:0 0 10px 10px;box-shadow:0 -8px 24px rgba(0,0,0,.35)`);
        ...
        const close = () => { backdrop.remove(); removeEventListener('keydown', onKey, true); modalDepth--; };
        const onKey = (ev) => { if (ev.key === 'Escape') {
            ev.stopPropagation();
            ev.preventDefault();
            close();
        } };
        backdrop.addEventListener('click', ev => { if (ev.target === backdrop)
            close(); });
    ...
    const row = (parent, name, value, href) => {
        ...
        if (href && v instanceof HTMLAnchorElement)
            v.href = href;
        v.textContent = value;
        r.append(l, v);
        parent.appendChild(r);
    };
    const linkedValues = (parent, name, values, facet) => {
        const r = el('div', 'display:flex;gap:10px;margin:2px 0');
        const l = el('span', 'color:#6b76b8;min-width:120px;flex-shrink:0');
        l.textContent = name;
    ...
    const footerBtn = (text, solid, enabled = true) => {
        const b = document.createElement('button');
        b.textContent = text;
        b.disabled = !enabled;
    ...
    const badge = (resolved) => {
        if (!resolved)
            return { text: 'CANNOT READ — EJECT', color: '#e0504d' };
        if (resolved.tier === 'tested')
            return { text: `✓ VERIFIED · ${stripSet(resolved.meta?.description ?? '').toUpperCase()}`, color: '#5ecf7a' };
        if (resolved.tier === 'experimental')
            return { text: 'EXPERIMENTAL — UNTESTED', color: '#e8b64c' };
    ...
    function openInfoModal(rec, resolved, onEject) {
        ...
        openModal((scroller, footer, close) => {
            ...
            if (meta) {
                ...
                if (meta.year)
                    row(cat, 'Year', meta.year);
                if (meta.publisher)
                    row(cat, 'Publisher', meta.publisher);
                if (meta.pcb)
                    row(cat, 'PCB', meta.pcb);
                if (meta.mirroring)
                    row(cat, 'Mirroring', meta.mirroring);
            ...
            if (rec.ines) {
                ...
                if (resolved?.slot)
                    row(tech, 'Board', resolved.slot.toUpperCase());
            }
            if (resolved?.reason)
                row(tech, 'Status', resolved.reason);
    ...
    function openTargetModal(catEntry, name, zipName) {
        ...
        openModal((scroller, footer, close) => {
            ...
            if (catEntry) {
                ...
                if (catEntry.year)
                    row(cat, 'Year', catEntry.year);
                if (catEntry.publisher)
                    row(cat, 'Publisher', catEntry.publisher);
                if (catEntry.pcb)
                    row(cat, 'PCB', catEntry.pcb);
                if (catEntry.prg?.roms[0])
                    row(cat, 'PRG CRC', catEntry.prg.roms[0].crc);
                if (catEntry.chr?.roms[0])
                    row(cat, 'CHR CRC', catEntry.chr.roms[0].crc);
    ...
    function openAboutModal() {
        openModal((scroller, footer, close) => {
            ...
            if (entry?.manufacturer && entry?.year)
                subh.appendChild(document.createTextNode(' · '));
            if (entry?.year) {
            ...
            for (const cpu of cfg.board.cpus) {
                ...
                row(hw, cpu === cfg.board.cpus[0] ? 'Processors' : '', `${family} "${cpu.tag}" @ ${(cpu.clock / 1e6).toFixed(3)} MHz`, browseUrl('cpu', family));
            }
            if (cfg.sound && cfg.sound.kind !== 'none') {
                const sound = `${cfg.sound.kind.toUpperCase()}${cfg.sound.chips ? ` × ${cfg.sound.chips}` : ''}`;
                row(hw, 'Sound', sound + (cfg.sound.clock ? ` @ ${(cfg.sound.clock / 1e6).toFixed(3)} MHz` : ''), browseUrl('sound', sound));
            }
            ...
            if (entry?.driverFile)
                row(ppl, 'Driver source', entry.driverFile, browseUrl('driver', entry.driverFile));
            if (entry?.copyrightHolders)
                row(ppl, 'Written by', entry.copyrightHolders, browseUrl('written-by', entry.copyrightHolders));
            if (entry?.license)
                row(ppl, 'License', entry.license, browseUrl('license', entry.license));
            ...
            if (entry?.credits?.people.length) {
                linkedValues(ppl, 'Credited by MAMEDEV', entry.credits.people.slice(0, 6).map(person => person.name), 'author');
                row(ppl, 'Credit source', entry.credits.source);
            }
            if (entry?.commitActivity) {
                const activity = entry.commitActivity;
                row(ppl, 'Commit activity', `${activity.commits} commits by ${activity.authors} commit authors, `
    ...
    // --- cart ingestion ------------------------------------------------------------------
    async function shelve(name, bytes) {
        const ines = support.ines ? parseINes(bytes) : null;
        if (support.ines && !ines)
            return; // callers pre-check; belt and braces
        const resolved = ines
    ...
    async function ingest(name, bytes) {
        if (bytes.length > MAX_CART) {
            toast(`${name}: bigger than 8 MiB — not a cartridge`);
            return false;
        }
        if (name.toLowerCase().endsWith('.zip') || (bytes[0] === 0x50 && bytes[1] === 0x4b)) {
            let zentries;
            try {
                zentries = await readZip(bytes);
            }
            catch {
                toast(`${name} isn't a readable zip`);
                return false;
            }
            if (!support.ines) {
            ...
            for (const [zname, data] of zentries) {
                if (data.length > MAX_CART)
                    continue;
                if (parseINes(data)) {
                    await shelve(zname.split('/').pop() ?? zname, data);
                    shelved++;
                }
            }
            if (shelved)
    ...
    async function handleFiles(files) {
        for (const f of files) {
            if (f.size > MAX_CART) {
                toast(`${f.name}: bigger than 8 MiB — not a cartridge`);
                continue;
            }
            slotBusy(f.name);
            let bytes;
            try {
                ...
                toast(`${f.name}: could not read the file`);
                continue;
            }
    ...
    function beginCartFetch(cover, label) {
      ...
      border-radius:12px;background:linear-gradient(180deg,transparent,${GOLD}2e,transparent)`);
        if (!stillMotion) {
            ...
            scan.animate([{ transform: 'translateY(-40%)' }, { transform: 'translateY(300%)' }], { duration: 900, iterations: Infinity, easing: 'ease-in-out' });
        }
        const settle = () => {
            scan.remove();
    ...
    async function fetchCart(cart, title, anim) {
        if (fetching.has(cart.file)) {
            anim?.fail();
            return;
        }
    ...
    let depth = 0;
    addEventListener('dragover', ev => { if (inRoom)
        ev.preventDefault(); });
    addEventListener('dragenter', ev => { if (!inRoom)
        return; ev.preventDefault(); if (++depth === 1)
        slotArmed(); });
    addEventListener('dragleave', () => { if (!inRoom)
        return; if (--depth <= 0) {
        depth = 0;
        slotIdle();
    } });
    ...
    addEventListener('keydown', ev => {
        ...
        switch (ev.key) {
            ...
            case 'ArrowLeft':
                ...
                break;
            case 'ArrowDown':
                select(selected < 0 ? 0 : selected + perRow);
                ev.preventDefault();
                break;
            case 'ArrowUp':
                select(selected < 0 ? 0 : selected - perRow);
    ...
    for (const rec of recs)
        route(rec, resolveRec(rec, catalog, support), false);
    select(0);
