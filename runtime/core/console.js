    ...
    function openAboutModal() {
        openModal((scroller, footer, close) => {
            ...
            if (cfg.sound && cfg.sound.kind !== 'none')
                row(hw, 'Sound', cfg.sound.kind + (cfg.sound.clock ? ` @ ${(cfg.sound.clock / 1e6).toFixed(3)} MHz` : ''));
            ...
            if (entry?.driverFile)
                row(ppl, 'Driver source', entry.driverFile);
            if (entry?.copyrightHolders)
                row(ppl, 'Written by', entry.copyrightHolders);
            if (entry?.license)
                row(ppl, 'License', entry.license);
            if (entry?.gitHistory) {
