...
import { artworkUrl } from "./artwork-source.js";
...
function cartSvg(o) {
    ...
    const titleSvg = lines.map((l, i) => `<text x="60" y="${112 + i * 19}" font-family="ui-sans-serif,system-ui,sans-serif" font-size="14" font-weight="800" fill="${titleColor}">${esc(l)}</text>`).join('');
...
export async function runConsole(cfg) {
    ...
    const boot = (rec, resolved) => {
        ...
        const regions = { prg: mountedINesPrg(resolved.ines) };
    ...
    function renderCatalog(reset = false) {
        ...
        const matches = rows.filter(row => {
            if (filter === 'verified' && row.tier !== 'verified')
                return false;
            if (filter === 'playable' && !(row.slot !== '' && support.slots.includes(row.slot)))
                return false;
    ...
    function openTargetModal(catEntry, name) {
