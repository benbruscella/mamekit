...
class GeneratedMotionObjects {
    ...
    xscroll = 0;
    yscroll = 0;
    constructor(plan, gfx, spriteRam, slipRam, width, height) {
        this.plan = plan;
        this.gfx = gfx;
    ...
    renderObject(clip, at) {
        ...
        const penBase = ((colorIndex * this.gfx.granularity()) | (priority << 12)) + plan.paletteBase;
        ...
        if (xpos >= this.bitmap.width)
            xpos -= plan.bitmapWidth;
        if (ypos >= this.bitmap.height)
            ...
            for (let y = 0, sy = ypos; y < height; y++, sy += yadv) {
                ...
                for (let x = 0, sx = xpos; x < width; x++, sx += xadv, code++) {
                    if (sx <= -clip.min_x - tileWidth || sx > clip.max_x)
                        continue;
                    this.gfx.transpen_raw(this.bitmap, clip, code, penBase, hflip, vflip, sx, sy, plan.transparentPen);
                }
            }
            return;
        }
...
export class GeneratedMameVideoPrimitives {
    ...
    ramPaletteMirror;
    priorityBitmap;
    constructor(machine, regions, state, bindings, updatePartial, memoryRead) {
        this.machine = machine;
        ...
        if (motionObjectsPlan && motionObjectsGfx) {
            const share = (name) => () => {
                ...
                return ArrayBuffer.isView(bytes) ? bytes : undefined;
            };
            this.motionObjects = new GeneratedMotionObjects(motionObjectsPlan, motionObjectsGfx, share(motionObjectsPlan.spriteShare), share(motionObjectsPlan.slipShare), this.width, this.height);
        }
     ...
     */
    drawOutrunLayers(screen, bitmap, cliprect) {
        ...
        const road = wordsView(this.state.m_segaic16road_roadram ?? this.state['m_segaic16road:roadram']);
