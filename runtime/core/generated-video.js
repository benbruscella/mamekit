...
class GeneratedRamPalette {
    ...
    entry(pen) {
        ...
        const shiftFor = (byte) => 8 * (this.plan.endianness === 'big' ? totalBytes - byte - 1 : byte);
...
export class GeneratedGfxElement {
     ...
     */
    prio_transmask(bitmap, clip, code, color, flipX, flipY, sx, sy, _priorityBitmap, priority, transparentMask) {
        if (priority !== 0)
            return;
        this.draw(bitmap, clip, code, color, flipX, flipY, sx, sy, transparentMask);
    }
...
export class GeneratedMameVideoPrimitives {
            ...
            if (spriteHandler) {
                const renderSprites = (...rawArgs) => {
                    ...
                    const draw = (code, color, flipX, flipY, sx, sy) => {
                        gfx.transpen(bitmap, clip, code, color, screenFlipped ? Number(!flipX) : flipX, screenFlipped ? Number(!flipY) : flipY, screenFlipped ? 512 - 16 - sx : sx, screenFlipped ? 256 - 16 - sy : sy, 15);
                    };
    ...
    directScreenUpdate(handler, screen, bitmap, cliprect) {
        ...
        if (vector?.type === 'DVG' && this.memoryRead && bitmap.direct) {
            ...
            for (const point of points) {
                ...
                if (point.intensity > 0) {
                    drawAdditiveVectorLine(pixels, width, height, lastX, lastY, x, y, Math.min(255, point.intensity << 4));
                }
                lastX = x;
                lastY = y;
            }
        ...
        if (this.directScreenShape === 'gauntlet-tilemaps') {
            ...
            const pf = playfield;
            const al = alpha;
            const word = (bytes, index) => (bytes[index * 2] ?? 0) | ((bytes[index * 2 + 1] ?? 0) << 8);
            bitmap.fill(0);
    ...
    drawOutrunLayers(screen, bitmap, cliprect) {
        ...
        for (let row = 0; row < 28; row++) {
            for (let column = 0; column < 40; column++) {
                ...
                gfx.transpen(bitmap, cliprect, data & 0x1ff, (data >>> 9) & 0x07, 0, 0, column * 8, row * 8, 0);
            }
        }
        return true;
    }
    ...
    drawTnx1(screen, bitmap, cliprect) {
        ...
        for (let y = cliprect.min_y; y <= cliprect.max_y; y++) {
            ...
            for (let x = cliprect.min_x; x <= cliprect.max_x; x++) {
                ...
                composed['pix='](y, x, ((backgroundRaw[rowBase | ((column >>> 2) & 0x3f)] ?? 0) >>> shift) & 0x0f);
            }
        }
