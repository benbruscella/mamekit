...
import { executeDvgDisplayList } from "../hardware/vector/dvg.js";
import { generatedDirectScreenShape, } from "./generated-video-shape.js";
export { generatedDirectScreenShape } from "./generated-video-shape.js";
...
export function isIndexedScreen(machine) {
    ...
    if (machine.execution.screenUpdate?.indexed)
        return true;
 ...
 */
export function exidySpriteCollisions(gfx, backgroundPixel, sprite1, sprite2, collisionMask = 0x1c) {
    const collisions = [];
    const pixel = (sprite, x, y) => {
    ...
    for (let y = 0; y < gfx.height && collisions.length < 128; y++) {
        for (let x = 0; x < gfx.width && collisions.length < 128; x++) {
            ...
            if (pixel(sprite1, x, y)) {
                let currentMask = 0;
                if (backgroundPixel(global1X, global1Y) !== 0)
                    currentMask |= 0x04;
...
export function exidySpriteCollisionMask(gfx, backgroundPixel, sprite1, sprite2) {
    return exidySpriteCollisions(gfx, backgroundPixel, sprite1, sprite2).reduce((mask, collision) => mask | collision.mask, 0);
}
/**
...
export class GeneratedVideoRenderer {
    ...
    penBuffer;
    partialNextY;
    constructor(machine, primitives, deviceScreenUpdate) {
        ...
        if (machine.execution.screenUpdate?.deviceTag && !deviceScreenUpdate) {
            throw new Error(`generated machine "${machine.game}": screen update belongs to device ` +
    ...
    renderRegion(frame, minY, maxY) {
        ...
        const bitmap = {
            ...
            'pix&': (y, x = 0) => ({
                ...
                source: target,
                offset: Math.floor((y - scaledYOffset) / yScale) * this.width +
...
class GeneratedRectangle {
    ...
    left() { return this.min_x; }
    right() { return this.max_x; }
    top() { return this.min_y; }
    bottom() { return this.max_y; }
    width() { return this.max_x + 1 - this.min_x; }
    height() { return this.max_y + 1 - this.min_y; }
    contains(x, y) {
 ...
 */
export function generatedBusWrittenShares(machine) {
    ...
    for (const range of ranges) {
        if (range.share && range.kind === 'ram' && !range.readOnly)
            written.add(range.share);
    }
    return written;
}
class GeneratedRamPalette {
    ...
    entry(pen) {
        ...
        const shiftFor = (byte, half) => 8 * (half + (this.plan.endianness === 'big'
    ...
    constructor(plan, regions, game) {
        ...
        const device = {
            ...
            set_pen_indirect: (pen, indirect) => {
                if (pen >= 0 && pen < this.indirect.length)
                    this.indirect[pen] = indirect & 0xffff;
                return 0;
            },
            set_pen_color: (pen, colorOrRed, green, blue) => {
                if (pen < 0 || pen >= this.colors.length)
        ...
        executeGeneratedHandler(plan.program, {
            ...
            referenceCalls: {
                memregion: (...args) => {
                    ...
                    if (!bytes)
                        throw new Error(`${game}: missing palette ROM region "${tag}"`);
...
class GeneratedPalette {
    ...
    constructor(plan, regions) {
        ...
        const coreCount = Math.max(plan.colorCount, ...(plan.computedColors ?? []).map(group => group.base + group.count), ...(plan.indexedColors ?? []).map(group => group.base + group.colors.length), ...(plan.promColors ?? []).map(group => group.base + group.count));
        ...
        for (let index = 0; index < plan.colorCount; index++) {
            ...
            for (const [channelIndex, channel] of plan.channels.entries()) {
                ...
                if (plan.resNet) {
                    ...
                    value = computeMameTtlSanyoResNet(inputs, channel.resistances, channel.pullup, channel.pulldown, plan.resNet.amplifiers[channelIndex] ?? 'none');
                }
                else {
                    value = 0;
                    for (let bit = 0; bit < channel.bits.length; bit++) {
        ...
        if (plan.normalize)
            normalizePaletteRange(core, plan.normalize);
...
function palettePromBit(source, channel, position) {
...
class GeneratedTnx1Palette {
    /** Save-state roots (machine-state.ts). */
    stateKeys() {
        return ['bank', 'colors'];
    }
    colors = new Uint32Array(80);
    colorProm;
    spriteProm;
    bank = -1;
    constructor(plan, regions) {
        ...
        if (!colorProm || !spriteProm) {
            throw new Error(`generated TNX1 palette: missing ROM region "${!colorProm ? plan.colorRegion : plan.spriteRegion}"`);
        }
    ...
    sync(state) {
        ...
        for (let index = 0; index < 16; index++) {
            this.colors[index] = this.decode(this.colorProm[colorBank + index] ?? 0, [1200, 680, 470], [680, 470]);
            const text = this.colorProm[32 + colorBank + index] ?? 0;
            this.colors[16 + index * 2] = 0xff000000;
            this.colors[17 + index * 2] = this.decode(text, [1000, 470, 220], [470, 220]);
        }
        const spriteBank = (bank & 0x07) * 32;
        for (let index = 0; index < 32; index++) {
            const address = spriteBank + index;
            const aliased = (address & 0x3f) | ((address & 0x20) << 1);
            this.colors[48 + index] = this.decode(this.spriteProm[aliased] ?? 0, [1000, 470, 220], [470, 220]);
        }
    }
    decode(value, rgbResistors, blueResistors) {
        return packRgb(computeMameTtlSanyoResNet(value & 0x07, rgbResistors, 470, 0, 'darlington'), computeMameTtlSanyoResNet((value >>> 3) & 0x07, rgbResistors, 470, 0, 'darlington'), computeMameTtlSanyoResNet((value >>> 6) & 0x03, blueResistors, 680, 0, 'darlington'));
    }
    transpen_mask(_gfx, _color, transparent) {
...
class GeneratedMotionObjects {
    ...
    xscroll = 0;
    yscroll = 0;
    constructor(plan, gfx, spriteRam, slipRam, width, height, gfxSets = () => []) {
        this.plan = plan;
        this.gfx = gfx;
        ...
        const powerOfTwo = (mask) => 2 ** Math.ceil(Math.log2(Math.max(1, mask)));
     ...
     */
    iterate_dirty_rects(clip, body) {
        body(clip);
    }
    extract(parameter, at) {
    ...
    draw(clip) {
        ...
        if (startBand > stopBand)
            startBand -= plan.bitmapHeight >> plan.slipShift;
        if (plan.slipShift === 0)
            stopBand = startBand;
        ...
        for (let band = startBand; band <= stopBand; band++) {
            ...
            if (plan.slipShift !== 0) {
                ...
                if (minY >= this.objectBitmap.height)
                    minY -= plan.bitmapHeight;
            ...
            if (plan.reverse) {
                for (let at = last; at >= 0; at -= 4)
                    this.renderObject(bandClip, at);
            }
            else {
    ...
    renderObject(clip, at) {
        ...
        const penBase = ((colorIndex * gfx.granularity()) | (priority << 12)) + plan.paletteBase;
        if (!this.extract(plan.absolute, at)) {
            xpos -= this.xscroll;
            ypos -= this.yscroll;
        }
        ypos -= height << tileYShift;
        if (this.nextXpos !== 123456)
            xpos = this.nextXpos;
        this.nextXpos = 123456;
        if (this.extract(plan.neighbor, at) !== 0) {
            if (!plan.nextNeighbor)
                xpos = this.lastXpos + tileWidth;
            else
                this.nextXpos = xpos + tileWidth;
        }
        this.lastXpos = xpos;
        xpos &= plan.bitmapWidth - 1;
        ypos &= plan.bitmapHeight - 1;
        if (xpos >= this.objectBitmap.width)
            xpos -= plan.bitmapWidth;
        if (ypos >= this.objectBitmap.height)
            ypos -= plan.bitmapHeight;
        if (plan.special.mask !== 0 && this.extract(plan.special, at) === plan.specialValue)
            return;
        let xadv = tileWidth;
        if (hflip) {
            xpos += (width - 1) << tileXShift;
            xadv = -xadv;
        }
        let yadv = tileHeight;
        if (vflip) {
            ypos += (height - 1) << tileYShift;
            yadv = -yadv;
        }
        if (!plan.swapXy) {
            for (let y = 0, sy = ypos; y < height; y++, sy += yadv) {
                if (sy <= clip.min_y - tileHeight) {
                    code += width;
                    continue;
                }
                if (sy > clip.max_y)
                ...
                for (let x = 0, sx = xpos; x < width; x++, sx += xadv, code++) {
                    if (sx <= -clip.min_x - tileWidth || sx > clip.max_x)
                        continue;
                    gfx.transpen_raw(this.objectBitmap, clip, code, penBase, hflip, vflip, sx, sy, plan.transparentPen);
                }
            }
            return;
        }
        for (let x = 0, sx = xpos; x < width; x++, sx += xadv) {
            if (sx <= clip.min_x - tileWidth) {
                code += height;
                continue;
            }
            if (sx > clip.max_x)
            ...
            for (let y = 0, sy = ypos; y < height; y++, sy += yadv, code++) {
                if (sy <= -clip.min_y - tileHeight || sy > clip.max_y)
                    continue;
                gfx.transpen_raw(this.objectBitmap, clip, code, penBase, hflip, vflip, sx, sy, plan.transparentPen);
            }
        }
...
export class GeneratedGfxElement {
    ...
    transtable(bitmap, clip, code, color, flipX, flipY, sx, sy, drawModes) {
        this.draw(bitmap, clip, code, color, flipX, flipY, sx, sy, drawModeMask(drawModes));
    }
    prio_transtable(bitmap, clip, code, color, flipX, flipY, sx, sy, priorityBitmap, priorityMask, drawModes) {
        if (!(priorityBitmap instanceof GeneratedPriorityBitmap)) {
            throw new Error('prio_transtable needs the screen priority bitmap');
        }
        this.draw(bitmap, clip, code, color, flipX, flipY, sx, sy, drawModeMask(drawModes), { kind: 'claim', bitmap: priorityBitmap, mask: priorityMask | (1 << 31) });
    }
    zoom_transtable(bitmap, clip, code, color, flipX, flipY, sx, sy, scaleX, scaleY, drawModes) {
        this.drawZoom(bitmap, clip, code, color, flipX, flipY, sx, sy, scaleX, scaleY, drawModeMask(drawModes));
    }
    prio_zoom_transtable(bitmap, clip, code, color, flipX, flipY, sx, sy, scaleX, scaleY, priorityBitmap, priorityMask, drawModes) {
        if (!(priorityBitmap instanceof GeneratedPriorityBitmap)) {
            throw new Error('prio_zoom_transtable needs the screen priority bitmap');
        }
        this.drawZoom(bitmap, clip, code, color, flipX, flipY, sx, sy, scaleX, scaleY, drawModeMask(drawModes), { kind: 'claim', bitmap: priorityBitmap, mask: priorityMask | (1 << 31) });
    }
    /**
     ...
     */
    prio_transmask(bitmap, clip, code, color, flipX, flipY, sx, sy, priorityBitmap, priorityMask, transparentMask) {
        if (!(priorityBitmap instanceof GeneratedPriorityBitmap)) {
     ...
     */
    prio_transpen(bitmap, clip, code, color, flipX, flipY, sx, sy, priorityBitmap, priorityMask, transparentPen) {
        if (!(priorityBitmap instanceof GeneratedPriorityBitmap)) {
    ...
    opaque(bitmap, clip, code, color, flipX, flipY, sx, sy) {
     ...
     */
    transpen_raw(bitmap, clip, code, penBase, flipX, flipY, sx, sy, transparentPen) {
    ...
    set_colors(value) {
        if (Number.isFinite(value) && value > 0)
            this.colorCount = Math.trunc(value);
    }
    ...
    set_granularity(value) {
        if (Number.isFinite(value) && value > 0)
            this.penGranularity = Math.trunc(value);
    }
    get_data(code) {
    ...
    draw(bitmap, clip, code, color, flipX, flipY, sx, sy, transparentMask = 0, priority) {
        const gfx = this.decoded;
        const element = modulo(code, gfx.count);
        const base = element * gfx.width * gfx.height;
            ...
            for (let px = 0; px < gfx.width; px++) {
                ...
                if (priority) {
                    if (priority.kind === 'stamp') {
                        ...
                        priority.bitmap.set(x, y, (priority.bitmap.get(x, y) & priority.keep) | priority.value);
                    }
                    else {
                        ...
                        if (((1 << (claimed & 0x1f)) & priority.mask) !== 0)
                            continue;
                    }
                }
    ...
    drawZoom(bitmap, clip, code, color, flipX, flipY, sx, sy, scaleX, scaleY, transparentMask, priority) {
        ...
        for (let dy = 0; dy < height; dy++) {
            ...
            for (let dx = 0; dx < width; dx++) {
                ...
                bitmap['pix='](y, x, this.indexed ? colorBase + pen : this.palette.colors[colorBase + pen] ?? 0xff000000);
            }
        }
    }
}
function drawModeMask(drawModes) {
    let transparent = 0;
...
class GeneratedTilemap {
    ...
    pixmapCacheComplete = false;
    active = true;
    flip = 0;
    pixmapBitmap;
    dynamicTransmasks = new Map();
    constructor(plan, machine, bindings, gfx, baseMemory) {
        ...
        this.pixmapBitmap = new GeneratedIndexedBitmap(plan.columns * plan.tileWidth, plan.rows * plan.tileHeight);
    }
    /**
    ...
    basemem_read(offset) {
        ...
        if (index < 0)
            return 0;
        if ((this.plan.bytesPerEntry ?? 1) < 2)
            return bytes[index] ?? 0;
        const at = index * 2;
        if (this.wordShare())
            return new Uint16Array(bytes.buffer, bytes.byteOffset + at, 1)[0];
    ...
    pixmap() {
        ...
        const draw = (row, column, index) => {
            ...
            this.gfx[tile.gfx]?.draw(bitmap, clip, tile.code, tile.color, tile.flags & 1, (tile.flags >> 1) & 1, column * this.plan.tileWidth, row * this.plan.tileHeight);
        };
        if (!this.pixmapCacheComplete || this.mapper) {
            ...
            for (let row = 0; row < this.plan.rows; row++) {
                for (let column = 0; column < this.plan.columns; column++) {
                    draw(row, column, mapStandardTile(this.plan.mapper, column, row, this.plan.columns, this.plan.rows));
                }
            }
        ...
        for (const index of [...this.dirtyIndices]) {
            if (index < 0 || index >= this.plan.columns * this.plan.rows)
                continue;
    ...
    set_transmask(group, foreground, background) {
    ...
    tileAt(tileIndex) {
        ...
        if (needsUpdate) {
            ...
            if (deviceTileInfo) {
                ...
                executeGeneratedMachineProgram(this.machine, this.tileInfo, bindings, { tilemap: this, tileinfo, tile_index: tileIndex });
            }
    ...
    draw(_screen, bitmap, clip, _flags, _priority = 0, _priorityMask = 0xff) {
        if (!this.active)
            return;
        ...
        const lastOutputRow = scrollsVertically
            ? this.plan.rows - 1
            : Math.min(this.plan.rows - 1, Math.floor((clip.max_y - yDelta) / this.plan.tileHeight));
        ...
        const lastOutputColumn = scrollsHorizontally
            ? this.plan.columns - 1
            : Math.min(this.plan.columns - 1, Math.floor((clip.max_x - xDelta) / this.plan.tileWidth));
        ...
        for (let outputRow = firstOutputRow; outputRow <= lastOutputRow; outputRow++) {
            ...
            for (let outputColumn = firstOutputColumn; outputColumn <= lastOutputColumn; outputColumn++) {
                ...
                if (!intersectsVerticalClip)
                    continue;
                if (!generatedTileCategoryMatches(tile.category, _flags))
                    continue;
                ...
                if (!(_flags & 0x80)) {
                    ...
                    if (groupMask !== undefined) {
                        ...
                        transparentMask = generatedTileGroupIndirectMask(gfx, tile.group, this.plan.transparentIndirect);
                    }
                ...
                for (let wrappedX = firstWrappedX; wrappedX <= firstWrappedX + mapWidth * 2; wrappedX += mapWidth) {
                    ...
                    for (let wrappedY = firstWrappedY; wrappedY <= firstWrappedY + mapHeight * 2; wrappedY += mapHeight) {
                        ...
                        gfx.draw(bitmap, clip, tile.code, tile.color, Number(tileFlipX), Number(tileFlipY), wrappedX, wrappedY, transparentMask, priority);
                    }
                }
            }
        }
 ...
 */
function generatedTilemapPriorityOp(screen, priority, priorityMask) {
    const code = (priority & 0xff) | ((priorityMask & 0xff) << 8);
    if (code === 0xff00)
        return undefined;
    const bitmap = screen?.priority?.();
    if (!(bitmap instanceof GeneratedPriorityBitmap))
        return undefined;
...
function generatedTileTransparentMask(mask, flags) {
...
export function generatedTileCategoryMatches(category, flags) {
 ...
 */
export function generatedTileGroupIndirectMask(gfx, group, transparent) {
 ...
 */
export function cpsGfxromBankMap(config, type, sourceCode) {
    const shifts = { 1: 1, 2: 0, 4: 1, 8: 3 };
    ...
    for (const range of config.bank_mapper ?? []) {
        ...
        if (!size)
            return -1;
        let base = 0;
        for (let bank = 0; bank < range.bank; bank++)
            base += sizes[bank] ?? 0;
...
export function decodeTaitoSjRamPixel(characterRam, bank, code, x, y, sprite) {
...
export function decodeVicDualPixel(characterLine, colorProm, x) {
...
export function williamsPaletteColor(value) {
    const weighted = (bits, resistances) => {
        const conductance = resistances.map(resistance => 1 / resistance);
        const total = conductance.reduce((sum, item) => sum + item, 0);
        return Math.round(bits.reduce((sum, bit, index) => sum + Number(Boolean(value & (1 << bit))) * conductance[index], 0) * 255 / total);
    };
    return packRgb(weighted([0, 1, 2], [1200, 560, 330]), weighted([3, 4, 5], [1200, 560, 330]), weighted([6, 7], [560, 330]));
}
/** Taito SJ's layer shifters include a different fixed pixel skew per plane. */
export function taitoSjLayerScrollX(raw, layer, flipped) {
...
const computeSegaSystem16Channel = (value, effect) => {
    ...
    for (let bit = 0; bit < segaSystem16Conductance.length; bit++) {
        if (value & (1 << bit))
            high += segaSystem16Conductance[bit];
    }
    return Math.round(255 * high / (segaSystem16ColorTotal + effectConductance));
};
const segaSystem16Channels = {
    normal: Uint8Array.from({ length: 32 }, (_, value) => computeSegaSystem16Channel(value, 'normal')),
    shadow: Uint8Array.from({ length: 32 }, (_, value) => computeSegaSystem16Channel(value, 'shadow')),
    highlight: Uint8Array.from({ length: 32 }, (_, value) => computeSegaSystem16Channel(value, 'highlight')),
};
/** Sega's five-bit resistor DAC, including its 470-ohm shadow/highlight leg. */
export function segaSystem16Channel(value, effect = 'normal') {
    return segaSystem16Channels[effect][value & 31];
}
/** Decode the non-linear bit wiring used by Sega's System 16 palette RAM. */
export function segaSystem16PaletteEntry(raw) {
    ...
    return {
        normal: packRgb(segaSystem16Channel(red), segaSystem16Channel(green), segaSystem16Channel(blue)),
        effect: packRgb(segaSystem16Channel(red, effect), segaSystem16Channel(green, effect), segaSystem16Channel(blue, effect)),
    };
}
/**
 ...
 */
class GeneratedInitialisedPalette {
    /** Save-state roots (machine-state.ts). */
    stateKeys() {
        return ['colors'];
    }
    ...
    set_pen_color(pen, colorOrRed, green, blue) {
        ...
        if (pen >= 0 && pen < this.colors.length)
            this.colors[pen] = color;
    }
    transpen_mask() { return 0; }
    black_pen() { return 0; }
    pens() { return this.colors; }
}
/**
...
export class GeneratedMameVideoPrimitives {
    ...
    ramPaletteMirror;
    priorityBitmap;
    constructor(machine, regions, state, bindings, updatePartial, memoryRead, shares = {}, 
    ...
    vectorPoints) {
        ...
        if (bitmapPlan && !ArrayBuffer.isView(state[bitmapPlan.member])) {
            state[bitmapPlan.member] = new Uint8Array((bitmapPlan.rowStart + bitmapPlan.rows) * bitmapPlan.bytesPerRow);
        }
        ...
        if (machine.video?.palette) {
            this.palettes.set('m_palette', machine.video.palette.dynamic?.kind === 'tnx1-banked'
        ...
        if (machine.video?.paletteProgram) {
            this.palettes.set('m_palette', new GeneratedProgramPalette(machine.video.paletteProgram, regions, machine.game));
        }
        const neoGeoHardware = hasDeviceType(machine, 'NEOGEO_SPRITE_OPTIMZIED');
        const ramPalettePlan = machine.video?.ramPalette ?? (neoGeoHardware
            ? {
                ...
                bytesPerEntry: 2,
                channels: [
                    { channel: 'r', bits: 5, shift: 10 },
                    { channel: 'g', bits: 5, shift: 5 },
                    { channel: 'b', bits: 5, shift: 0 },
                ],
            }
            : undefined);
        if (neoGeoHardware) {
            ...
            state.m_paletteram ??= new Array(0x2000).fill(0);
            state.m_palette_lookup ??= Array.from({ length: 32 }, () => new Array(4).fill(0));
            state.resistances ??= [3900, 2200, 1000, 470, 220];
        }
        if (ramPalettePlan) {
            this.ramPalette = new GeneratedRamPalette(ramPalettePlan, shares, generatedBusWrittenShares(machine));
        ...
        this.gfx = (machine.video?.gfx ?? []).map(entry => {
            ...
            const region = entry.ram && ArrayBuffer.isView(stateRegion)
                ? stateRegion
                : regions[entry.region] ?? (ArrayBuffer.isView(stateRegion) ? stateRegion : undefined);
            if (!region)
        ...
        if (motionObjectsPlan && motionObjectsGfx) {
            ...
            const share = (name) => {
                ...
                const memory = ArrayBuffer.isView(bound) ? bound : undefined;
                return () => memory;
            };
            this.motionObjects = new GeneratedMotionObjects(motionObjectsPlan, motionObjectsGfx, share(motionObjectsPlan.spriteShare), share(motionObjectsPlan.slipShare), this.width, this.height, () => this.gfx);
        }
        ...
        const referenceCalls = {
            ...
            'std::make_unique<gfx_element>': (...args) => {
                const palette = generatedArgumentValue(args[0]);
                const layout = generatedArgumentValue(args[1])
                    ?.gfxLayout;
                const source = generatedArgumentValue(args[2]);
                ...
                const rom = bytes;
                return new GeneratedGfxElement(entry, decodeGfx(layout, rom.subarray(offset)), palette ?? this.palette, indexed);
            },
            'std::make_unique': (...args) => new GeneratedIndexedBitmap(Math.max(1, Number(generatedArgumentValue(args[0]) ?? this.width)), Math.max(1, Number(generatedArgumentValue(args[1]) ?? this.height))),
            memregion: (...args) => {
                const tag = String(generatedArgumentValue(args[0]) ?? '');
                const bytes = regions[tag];
                if (!bytes)
            ...
            copyscrollbitmap: (...rawArgs) => {
                const destination = generatedArgumentValue(rawArgs[0]);
                const source = generatedArgumentValue(rawArgs[1]);
                const clip = generatedArgumentValue(rawArgs[6]);
                if (!destination || !source || !clip)
                    return 0;
        ...
        const roadHandler = machine.handlers?.find(handler => handler.method === 'draw_road' &&
        ...
        if (roadHandler) {
            ...
            if (ArrayBuffer.isView(colorProms)) {
                state.m_vertical_position_modifier = polePositionVerticalModifiers(colorProms);
            }
            const drawRoad = (...rawArgs) => {
                const bitmap = generatedArgumentValue(rawArgs[0]);
                ...
                if (!bitmap || !ArrayBuffer.isView(roadRegion) ||
                    !ArrayBuffer.isView(roadMemory) || !ArrayBuffer.isView(vertical))
                    return 0;
                const roadBytes = roadRegion;
                const roadWords = roadMemory;
                const verticalWords = vertical;
                ...
                for (let y = 128; y < Math.min(256, height); y++) {
                    ...
                    let destination = 0;
                    const yOffset = (((verticalWords[y] ?? 0) + Number(state.m_road16_vscroll ?? 0)) >> 3) & 0x1ff;
                    ...
                    for (let x = 0; x < 256; x++)
                        bitmap['pix='](y, x, scanline[x + xScroll]);
                }
                return 0;
            };
        ...
        const zoomSpriteHandler = machine.handlers?.find(handler => handler.method === 'zoom_sprite' &&
        ...
        const spriteHandler = machine.handlers?.find(handler => handler.method === 'draw_sprites' &&
        ...
        if (zoomSpriteHandler && spriteHandler) {
            const zoomSprite = (...rawArgs) => {
                ...
                if (!bitmap || !ArrayBuffer.isView(scale) || !gfx || !this.palette)
                    return 0;
                const scaleLut = scale;
            ...
            const drawSprites = (...rawArgs) => {
                const bitmap = generatedArgumentValue(rawArgs[0]);
                const spriteMemory = state.m_sprite16_memory;
                if (!bitmap || !ArrayBuffer.isView(spriteMemory))
                    return 0;
                const words = spriteMemory;
                for (let index = 0; index < 64; index++) {
                    ...
                    if (sy >= 128)
                        color |= 0x40;
                    zoomSprite(bitmap, Number(Boolean(sizeCode & 0x8000)), code, color, Number(flipX), sx, sy, sizeX, sizeY);
                }
                return 0;
            };
        ...
        const cpsBankMapper = machine.handlers?.find(handler => handler.method === 'gfxrom_bank_mapper' &&
            handler.body?.includes('m_game_config->bank_mapper') &&
            handler.body.includes('m_game_config->bank_sizes'));
        if (cpsBankMapper) {
            const map = (...rawArgs) => cpsGfxromBankMap(state.m_game_config, Number(generatedArgumentValue(rawArgs[0]) ?? 0), Number(generatedArgumentValue(rawArgs[1]) ?? 0));
            ...
            referenceCalls.ALLOC = (...rawArgs) => new Uint16Array(Math.max(0, Number(generatedArgumentValue(rawArgs[0]) ?? 0)));
            const spriteHandler = machine.handlers?.find(handler => handler.ownerClass === cpsBankMapper.ownerClass &&
                handler.method === 'cps1_render_sprites' &&
                handler.body?.includes('m_buffered_obj.get()') &&
                handler.body.includes('handle blocked sprites'));
            if (spriteHandler) {
                const renderSprites = (...rawArgs) => {
                    const bitmap = generatedArgumentValue(rawArgs[1]);
                    const clip = generatedArgumentValue(rawArgs[2]);
                    const words = state.m_buffered_obj;
                    const gfx = this.gfx[2];
                    const config = state.m_game_config;
                    if (!bitmap || !clip || !gfx || !ArrayBuffer.isView(words))
                        return 0;
                    const objects = words;
                    const last = Math.min(Number(state.m_last_sprite_offset ?? -4), objects.length - 4);
                    if (last < 0)
                        return 0;
                    ...
                    const draw = (code, color, flipX, flipY, sx, sy) => {
                        gfx.prio_transpen(bitmap, clip, code, color, screenFlipped ? Number(!flipX) : flipX, screenFlipped ? Number(!flipY) : flipY, screenFlipped ? 512 - 16 - sx : sx, screenFlipped ? 256 - 16 - sy : sy, priority, 0x02, 15);
                    };
        ...
        if (this.motionObjects && motionObjectsMember)
            state[motionObjectsMember] = this.motionObjects;
        ...
        const createdTilemaps = [];
        for (const plan of machine.video?.tilemaps ?? []) {
            if (plan.userDataMember && !state[plan.userDataMember]) {
                const size = Math.max(...machine.video.tilemaps
            ...
            const tilemap = new GeneratedTilemap(plan, machine, () => this.bindings, plan.decodeMember
                ? this.gfxByDecode.get(plan.decodeMember) ?? []
                : this.gfx, plan.baseShare ? shares[plan.baseShare] : undefined);
            createdTilemaps.push(tilemap);
            const indexed = /^(m_\w+)\[\s*(\d+)\s*\]$/.exec(plan.member);
            if (indexed) {
                const array = state[indexed[1]] ?? [];
        ...
        const resistorWeightTables = new WeakMap();
        calls.compute_resistor_weights ??= (...rawArgs) => {
            const args = rawArgs;
                ...
                for (let active = 0; active < network.count; active++) {
                    ...
                    for (let index = 0; index < network.count; index++) {
                        ...
                        if (!resistance)
                            continue;
                        if (index === active)
                            highConductance += 1 / resistance;
                        else
                            lowConductance += 1 / resistance;
                    }
                    const low = 1 / lowConductance;
                    const high = 1 / highConductance;
                    network.weights[active] = Math.min(maximum, Math.max(minimum, (maximum - minimum) * low / (high + low) + minimum));
                }
                maximumOutput = Math.max(maximumOutput, network.weights.reduce((sum, value) => sum + value, 0));
            }
            ...
            for (const network of networks) {
                ...
                resistorWeightTables.set(network.output, scaled);
            }
            return scale;
        };
        calls.combine_weights ??= (...rawArgs) => {
            const args = rawArgs;
            const output = args[0];
            const weights = output && typeof output === 'object'
                ? resistorWeightTables.get(output) ?? Array.from(output, Number)
                : [];
            const value = args.slice(1).reduce((sum, bit, index) => sum + (weights[index] ?? 0) * Number(bit), 0);
            return Math.trunc(value + 0.5);
        };
        calls['m_screen.width'] = () => this.width;
        calls['m_screen.height'] = () => this.height;
        calls['machine().tilemap().create'] = () => createdTilemaps[Math.min(nextCreatedTilemap++, createdTilemaps.length - 1)] ?? 0;
        const bitmapMembers = new Map();
        for (const handler of machine.handlers ?? []) {
            for (const match of (handler.body ?? '').matchAll(/\b(m_\w*bitmap\w*)\s*(?:\[\s*(\d+)\s*\])?/g)) {
                bitmapMembers.set(match[1], Math.max(bitmapMembers.get(match[1]) ?? 0, match[2] ? Number(match[2]) + 1 : 0));
            }
        }
        ...
        if (this.palette instanceof GeneratedInitialisedPalette) {
            const callback = machine.callbacks.find(candidate => candidate.signal === 'palette_init');
            if (callback) {
                executeGeneratedCallbackHandler(machine, callback, this.bindings, { palette: this.palette });
            }
        }
    }
     ...
     */
    gfxDecodeInterface(group) {
        return {
            gfx: index => group[index] ?? 0,
            set_gfx: (index, element) => {
                if (!(element instanceof GeneratedGfxElement))
                    return 0;
                group[index] = element;
                if (group !== this.gfx && this.gfxByDecode.size === 1)
                    this.gfx[index] = element;
                return 0;
            },
        };
    }
    ...
    directScreenUpdate(handler, screen, bitmap, cliprect) {
        ...
        if (this.directScreenShape === 'exidy-character-ram') {
            ...
            const collisions = exidySpriteCollisions(gfx.decoded, (x, y) => bitmap.pix?.(y, x) ?? 0, sprite1, sprite2, collisionMask);
            const scheduleCollision = this.bindings.calls?.['machine().schedule_exidy_collision'];
            ...
            gfx.transpen(bitmap, cliprect, sprite2.code, 1, 0, 0, sprite2X, sprite2Y, 0);
            if (sprite1Enabled) {
                gfx.transpen(bitmap, cliprect, sprite1.code, 0, 0, 0, sprite1.x, Math.max(0, sprite1.y), 0);
            }
            return true;
        }
        ...
        if (this.directScreenShape === 'm62-category-sprites') {
            ...
            this.state.m_bg_tilemap?.clear_transmasks();
            return false;
        }
        ...
        if (vector?.type === 'DVG' && this.memoryRead && bitmap.direct) {
            ...
            for (const point of points) {
                ...
                if (point.intensity > 0) {
                    drawAdditiveVectorLine(pixels, width, height, lastX, lastY, x, y, 
        ...
        if (this.directScreenShape === 'berzerk-color-bitmap' && bitmap.direct) {
            ...
            if (!ArrayBuffer.isView(videoRaw) || !ArrayBuffer.isView(colorRaw))
                return false;
            const video = videoRaw;
            const colors = colorRaw;
            const pixels = bitmap.direct.pixels;
            ...
            const pens = Array.from({ length: 16 }, (_unused, color) => {
                ...
                return packRgb(color & 1 ? intensity : 0, color & 2 ? intensity : 0, color & 4 ? intensity : 0);
            });
        ...
        if (this.directScreenShape === 'gauntlet-tilemaps') {
            ...
            if (!ArrayBuffer.isView(playfield) || !ArrayBuffer.isView(alpha) ||
        ...
        if (this.directScreenShape === 'dkong-scanline-sprites') {
            ...
            for (let offset = base; drawn < 16 && offset < base + 0x200; offset += 4) {
                ...
                if (((y + addY + 1 + bufferedScanline) & 0xf0) !== 0xf0)
                    continue;
            ...
            for (let offset = 0; offset < objects.length; offset += 4) {
                ...
                for (let yc = 0; yc < 32; yc++) {
                    ...
                    for (let xc = 0; xc < 2; xc++) {
                        const graphicsOffset = gfxOffset +
                            xc * 0x40 +
                            (yc & 7) * 2 +
                            (control & 3) * 0x10;
        ...
        if (this.directScreenShape === 'technos-tilemap-sprites') {
            const background = this.state.m_bg_tilemap;
            const foreground = this.state.m_fg_tilemap;
            const spriteRaw = this.state.m_spriteram;
            const scrollXRaw = this.state.m_scrollx_lo;
            const scrollYRaw = this.state.m_scrolly_lo;
            const gfx = this.gfx[1];
            if (!background || !foreground || !ArrayBuffer.isView(spriteRaw) || !gfx)
                return false;
            const firstByte = (value) => ArrayBuffer.isView(value)
                ? Number(value[0] ?? 0)
                : Number(value ?? 0);
            background.set_scrollx(0, (Number(this.state.m_scrollx_hi ?? 0) << 8) | firstByte(scrollXRaw));
            background.set_scrolly(0, (Number(this.state.m_scrolly_hi ?? 0) << 8) | firstByte(scrollYRaw));
            background.draw(screen, bitmap, cliprect, 0, 0);
            const sprites = spriteRaw;
            ...
            for (let index = 0; index + 4 < sprites.length; index += 5) {
                ...
                const color = hardware === 2
                    ? (sprites[index + 2] ?? 0) >>> 5
                    : (sprites[index + 2] ?? 0) >>> 4;
                let code = (sprites[index + 3] ?? 0) | (((sprites[index + 2] ?? 0) & (hardware === 2 ? 0x1f : 0x0f)) << 8);
                if (hardware === 1) {
                    if (sx < -7 && sx > -16)
                        sx += 256;
                    if (sy < -7 && sy > -16)
                ...
                code &= ~size;
                const draw = (offset, x, y) => gfx.transpen(bitmap, cliprect, code + offset, color, flipX, flipY, x, y, 0);
                if (size === 0)
                    draw(0, sx, sy);
                else if (size === 1) {
                    draw(0, sx, sy + dy);
     ...
     */
    drawSystem16BLayers(screen, bitmap, cliprect) {
        const bytesView = (value) => ArrayBuffer.isView(value)
            ? new Uint8Array(value.buffer, value.byteOffset, value.byteLength)
            : undefined;
        const wordsView = (value) => {
        ...
        if (!tile || !text || !gfx)
            return false;
        const system16a = hasDeviceType(this.machine, 'SEGA_SYS16A_SPRITES');
        ...
        bitmap.fill(0, cliprect);
        const drawLayer = (which, transparent) => {
            ...
            for (let row = 0; row < 29; row++) {
                for (let column = 0; column < 41; column++) {
                    ...
                    if (transparent)
                        ...
                        gfx.opaque(bitmap, cliprect, code, color, 0, 0, x, y);
                }
            }
        };
        drawLayer(1, false);
        ...
        for (let row = 0; row < 28; row++) {
            for (let column = 0; column < 40; column++) {
                ...
                gfx.transpen(bitmap, cliprect, data & (system16a ? 0xff : 0x1ff), (data >>> (system16a ? 8 : 9)) & 0x07, 0, 0, column * 8, row * 8, 0);
            }
        }
        return true;
    }
    /** Compose System 16A's paged tilemaps, text and scanline sprite hardware. */
    drawSystem16ALayers(bitmap, cliprect) {
        const wordsView = (value) => {
            if (!ArrayBuffer.isView(value))
                return undefined;
        ...
        const gfx = this.gfx[0];
        if (!tile || !text || !sprites || !palette || !spriteRom || !gfx)
            return false;
        ...
        const decodedPixel = (code, x, y) => decoded.pixels[(code % decoded.count) * 64 + (y & 7) * 8 + (x & 7)] ?? 0;
        const put = (x, y, pen) => bitmap['pix='](y, x, pen);
        const layerPixel = (which, x, y) => {
        ...
        for (let y = cliprect.min_y; y <= cliprect.max_y; y++) {
            for (let x = cliprect.min_x; x <= cliprect.max_x; x++) {
                ...
                if (background.pixel)
                    priority[output] = background.category ? 2 : 1;
                ...
                if (textPixel) {
                    put(x, y, gfx.colorbase() + ((textData >>> 8) & 7) * gfx.granularity() + textPixel);
    ...
    drawCosmicBitmapSprites(bitmap, cliprect) {
        const videoRaw = this.state.m_videoram;
        const spriteRaw = this.state.m_spriteram;
        const mapRaw = this.state.m_user1;
        const gfx16 = this.gfx[0];
        const gfx32 = this.gfx[1];
        if (!ArrayBuffer.isView(videoRaw) || !ArrayBuffer.isView(spriteRaw) ||
        ...
        for (let offset = sprites.length - 4; offset >= 0; offset -= 4) {
            ...
            gfx.transpen(bitmap, cliprect, attributes & 0x80 ? code : code >>> 2, color, 0, (~attributes) & 0x40, 256 - (sprites[offset + 2] ?? 0), sprites[offset + 1] ?? 0, 0);
        }
        return true;
    }
    drawWilliams(bitmap, cliprect) {
        const videoRaw = this.state.m_videoram;
        const paletteRaw = this.state.m_paletteram;
        if (!ArrayBuffer.isView(videoRaw) || !ArrayBuffer.isView(paletteRaw))
            return false;
        const video = videoRaw;
    ...
    drawVicDual(bitmap, cliprect) {
        ...
        const lastY = Math.min(cliprect.max_y, direct.scaledYOffset + direct.height * direct.yScale - 1);
    ...
    drawTnx1(screen, bitmap, cliprect) {
        ...
        const tilemap = this.state.m_fg_tilemap;
        const gfx = this.gfx[1];
        const storage = (value) => ArrayBuffer.isView(value) || Array.isArray(value);
        if (!storage(backgroundRaw) || !storage(spriteRaw) || !storage(dmaRaw) ||
            !storage(scrollRaw) || !(spriteBitmap instanceof GeneratedIndexedBitmap) ||
            !tilemap || !gfx)
            return false;
        this.palette?.sync?.(this.state);
        const io = this.state.m_io_mconf;
        ...
        for (let y = cliprect.min_y; y <= cliprect.max_y; y++) {
            ...
            for (let x = cliprect.min_x; x <= cliprect.max_x; x++) {
                ...
                composed['pix='](y, x, ((backgroundRaw[rowBase | ((column >>> addressing.columnShift) & 0x3f)] ?? 0) >>>
                    shift) & 0x0f);
            }
        }
        ...
        for (let y = cliprect.min_y; y <= cliprect.max_y; y++) {
            ...
            for (let offset = 4; offset < dmaLength; offset += 4) {
                ...
                attribute.row = row;
                attribute.code = ((Number(spriteRaw[offset + 2] ?? 0) & 0x7f) +
                    ((flags & 0x10) << 3) +
                    ((flags & 0x04) << 6)) ^ 0x1ff;
            ...
            for (const attribute of attributes) {
                ...
                for (let x = 0; x < 16; x++) {
                    ...
                    if (pixelX < 0 || pixelX >= 512)
                        continue;
                    if (flipped)
                        pixelX ^= 0x1ff;
                    const pen = decoded.pixels[sourceBase + (x ^ attribute.flipX)] ?? 0;
                    spriteBitmap['pix='](y, pixelX, pen ? gfx.entry.colorBase + attribute.color * gfx.granularity() + pen : 0);
                }
            }
            for (let x = cliprect.min_x; x <= cliprect.max_x; x++) {
                const pen = spriteBitmap.pix(y, x);
                if (pen)
                    composed['pix='](y, x, pen);
            }
        }
    ...
    drawSystem1(bitmap, cliprect) {
        ...
        const numbers = (value) => ArrayBuffer.isView(value) || Array.isArray(value);
        if (!Array.isArray(pages) || !(pages[0] instanceof GeneratedTilemap) ||
        ...
        const videoRam = video;
        const spriteRam = sprites;
        const spriteBytes = spriteRom;
        const lookupProm = lookup;
                ...
                for (let words = 0; words < 0x8000; words++, x += 4, current += delta) {
                    ...
                    if (first !== 0) {
                        for (let repeat = 0; repeat < 2; repeat++) {
                            ...
                            if (effectiveX < cliprect.min_x || effectiveX > cliprect.max_x)
                                continue;
    ...
    drawTaitoSj(bitmap, cliprect) {
        ...
        if (!direct ||
            ...
            return false;
        const characterRam = chars;
        const videoRam = video;
        const colors = colorbank;
        const scrollRam = scroll;
        const columns = columnScroll;
        const spriteRam = sprites;
        const mode = videoMode[0] ?? 0;
        const priorityValue = priority[0] ?? 0;
        const collisionRam = collision;
        const priorityProm = prom;
        ...
        for (let layer = 0; layer < 3; layer++) {
            ...
            for (let offset = 0; offset < 0x400; offset++) {
                ...
                for (let y = 0; y < 8; y++) {
                    for (let x = 0; x < 8; x++) {
                        const pen = decodeTaitoSjRamPixel(characterRam, bank, code, flipX ? 7 - x : x, flipY ? 7 - y : y, false);
                        if (pen)
                            layers[layer][(tileY * 8 + y) * 256 + tileX * 8 + x] =
                                color * 8 + pen;
                    }
                }
            }
        ...
        const layerPixel = (layer, outputX, hardwareY) => {
            if (!(mode & (0x10 << layer)))
                return 0x40;
        ...
        const drawLayer = (layer) => {
            ...
            for (let outputY = 0; outputY < direct.height; outputY++) {
                ...
                for (let outputX = 0; outputX < direct.width; outputX++) {
                    ...
                    if (pen !== 0x40)
                        target[outputY * direct.width + outputX] = pen;
                }
            }
        };
        const spriteInfo = (which, applyGlobalFlip) => {
            const page = mode & 0x04 ? 0x80 : 0;
            const offset = page + which * 4;
            const position = taitoSjSpritePosition(spriteRam[offset] ?? 0, spriteRam[offset + 1] ?? 0);
            let sx = position.x;
            let sy = position.y;
        ...
        const spritePen = (info, x, y) => decodeTaitoSjRamPixel(characterRam, info.bank, info.code, info.flipX ? 15 - x : x, info.flipY ? 15 - y : y, true);
        const drawSprites = () => {
            if (!(mode & 0x80))
                return;
            for (let sprite = 0x1f; sprite >= 0; sprite--) {
                ...
                for (let y = 0; y < 16; y++) {
                    ...
                    if (outputY < 0 || outputY >= direct.height)
                        continue;
                    for (let x = 0; x < 16; x++) {
        ...
        for (const item of order)
            item === 0 ? drawSprites() : drawLayer(item - 1);
        ...
        if (mode & 0x80) {
            const active = Array.from({ length: 0x20 }, (_, which) => which >= 0x10 && which <= 0x17 ? undefined : spriteInfo(which, false));
            for (let first = 0; first < 0x20; first++) {
                ...
                for (let second = first + 1; second < 0x20; second++) {
                    ...
                    if (Math.abs(signed(left.x) - signed(right.x)) >= 16 ||
                    ...
                    for (let y = minY; y <= maxY && !hit; y++) {
                        for (let x = minX; x <= maxX; x++) {
                            if (spritePen(left, x - left.x, y - left.y) &&
            ...
            for (let which = 0; which < 0x20; which++) {
                ...
                for (let y = 0; y < 16; y++) {
                    const hardwareY = info.y + y;
                    if (hardwareY < 0 || hardwareY > 255)
                        continue;
                    for (let x = 0; x < 16; x++) {
                        const hardwareX = info.x + x;
                        if (hardwareX < 0 || hardwareX > 255 || !spritePen(info, x, y))
                            continue;
                        for (let layer = 0; layer < 3; layer++) {
    ...
    writePaletteRam(offset, data, ext = false) {
        ...
        if (!ext)
            this.bitmapPalette?.write8(offset, data);
    }
    reset() {
    ...
    resolveScreenPens(pens, frame, start, count) {
        if (this.directScreenShape === 'taitosj-layered-char-ram') {
            ...
            if (!ArrayBuffer.isView(paletteRam))
                return;
            const bytes = paletteRam;
            ...
            for (let index = start; index < end; index++) {
                ...
                const component = (bits) => bits.reduce((sum, bit, position) => sum + weights[position] * Number(!(bit < 8 ? low & (1 << bit) : high & (1 << (bit - 8)))), 0);
                frame[index] = packRgb(component([14, 15, 0]), component([11, 12, 13]), component([8, 9, 10]));
            }
            return;
        }
        this.palette?.sync?.(this.state);
        ...
        for (let outputY = 0; outputY < plan.rows; outputY++) {
            ...
            for (let byte = 0; byte < plan.bytesPerRow; byte++) {
                ...
                for (let pixel = 0; pixel < pixelsPerByte; pixel++) {
                    ...
                    const paletteIndex = value + (plan.paletteBankMember
...
function createRamPalette(plan, bytes) {
    if (plan.lookup) {
        ...
        for (let index = 0; index < colors.length; index++) {
            ...
            const intensity = (raw >>> plan.lookup.intensityShift) & plan.lookup.intensityMask;
            const rgb = { r: 0, g: 0, b: 0 };
...
function generatedPointerNumber(value) {
    const pointer = value;
    if (pointer?.generatedPointer) {
        if (typeof pointer.target?.get === 'function')
            return Number(pointer.target.get() ?? 0);
        if (pointer.source)
            return Number(pointer.source[pointer.offset ?? 0] ?? 0);
    }
 ...
 */
function computeMameTtlSanyoResNet(inputs, resistances, biasResistance, groundResistance, amplifier) {
    const vcc = 5;
    const vBias = 5;
    const vLow = 0.05;
    const vHigh = 4;
    ...
    if (groundResistance)
        conductance += 1 / groundResistance;
    let openCollector = conductance > 0 && weightedVoltage / conductance > vHigh;
    for (let bit = 0; bit < resistances.length; bit++) {
        const resistance = resistances[bit];
        if (!resistance || !((inputs >> bit) & 1) || openCollector)
            continue;
    ...
    if (amplifier === 'darlington')
        voltage = Math.max(0.7, voltage);
    else if (amplifier === 'emitter')
...
function normalizePaletteRange(colors, range) {
    ...
    for (let index = start; index <= end; index++) {
        ...
        const target = targetMinimum + Math.trunc((luminance - minimum) * (targetMaximum - targetMinimum + 1) /
            (maximum - minimum));
        colors[index] = packRgb(clamp(target + Math.trunc(1140 * v / 1000)), clamp(target - Math.trunc(395 * u / 1000) - Math.trunc(581 * v / 1000)), clamp(target + Math.trunc(2032 * u / 1000)));
    }
}
function packRgb(red, green, blue) {
    return (0xff000000 | (blue << 16) | (green << 8) | red) >>> 0;
}
function drawAdditiveVectorLine(pixels, width, height, fromX, fromY, toX, toY, intensity, 
