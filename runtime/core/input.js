...
export class KeyboardInput {
    ...
    constructor(bindings, _dipDefaults, ports) {
        ...
        for (const [index, b] of bindings.entries()) {
            ...
            if (axis) {
                ...
                f.dir = axis.dir;
                if (!this.sticks.has(f.stick))
                    this.sticks.set(f.stick, { previous: 0, four: 0 });
    ...
    settle(list) {
        ...
        for (const index of this.carried) {
            ...
            if (field && field.stick === undefined)
                this.apply(field, this.isHeld(field));
        }
        this.carried.clear();
        this.frameCount++;
        ...
        for (const event of list) {
            ...
            if (event.kind === 'release')
                tapped.clear();
            else if (event.kind === 'edge' && event.down)
                tapped.add(event.binding);
        }
        ...
        for (const index of tapped) {
            const field = this.fields[index];
            if (!field || field.toggle || field.relativeDelta !== undefined)
                continue;
            if (this.isHeld(field))
                continue;
            this.carried.add(index);
            if (field.stick === undefined)
    ...
    settleSticks() {
        for (const [name, switches] of this.stickFields) {
            ...
            if (decision !== (this.loggedGate.get(name) ?? '')) {
                ...
                if (decision)
                    console.log(`[input f${this.frameCount}] ${decision} | ${this.dump()}`);
            }
        }
    }
    /**
     ...
     */
    resolveOpposites(switches, current) {
        for (const [a, b] of [[JOY_UP, JOY_DOWN], [JOY_LEFT, JOY_RIGHT]]) {
    ...
    onKey(ev, down) {
        ...
        if (!hits) {
            if (this.debug && down && !ev.repeat)
                console.log(`[input f${this.frameCount}] ${ev.code} unbound`);
            return;
        }
        ev.preventDefault();
        for (const h of hits) {
