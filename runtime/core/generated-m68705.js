...
export class GeneratedM68705P5Device {
    ...
    portInput = Uint8Array.from([0xff, 0xff, 0xff]);
    ...
    parameters(name) {
        if (name === 'execute_set_input')
            return ['inputnum', 'state'];
    ...
    readPort(port) {
        ...
        if (callback)
            this.portInput[port] = Number(callback() ?? 0xff) & ~mask;
