...
class IrBoard {
    ...
    constructor(machine, config, regions, inputs, sinks) {
        ...
        for (const specification of machine.execution.cpus) {
            ...
            if (specification.io) {
                const ioBus = new Bus(specification.io.ranges, new Uint8Array(0), registry, this.shares, specification.io.space?.dataWidth === 16 ? 16 : generatedCpuDataWidth(type), undefined, specification.io.space?.endianness ?? generatedCpuEndianness(type));
        ...
        for (const callback of machine.callbacks) {
            ...
            if (!device || !member || !callback.targetMethod)
                continue;
            ...
            device.bindCall(member, (...args) => executeGeneratedMachineHandler(machine, handler, this.bindings, Object.fromEntries(parameters.map((parameter, index) => [
                parameter.name,
        ...
        if (hasDeviceType(machine, 'SEGAIC16_ROAD')) {
            ...
            registry.write['segaic16road.segaic16_road_control_0_w'] =
                (_address, _offset, data, memMask = 0xffff) => {
                    if (memMask & 0x00ff)
                        this.state.__outrunRoadControl = data & 3;
                };
            const sizeMasks = [0x00ffff, 0x01ffff, 0x07ffff, 0x1fffff];
            const window = (address, index, offset, length, mirror, kind, share) => {
                const sizeMask = sizeMasks[registers[0x10 + 2 * index] & 3];
                const base = (registers[0x11 + 2 * index] << 16) & ~sizeMask;
                const mirrorMask = mirror & sizeMask;
                const start = base + (offset & sizeMask);
            ...
            const mapperRegister = (wordOffset) => wordOffset & 0x1f;
            const ioRead = (byteOffset) => {
            ...
            const ioWrite = (byteOffset, data, memMask) => {
                ...
                switch (offset & 0x38) {
                    ...
                    case 0x10:
                        if (memMask & 0x00ff)
                            this.state.__outrunOutputs = data & 0xff;
                        break;
                    case 0x18:
                    case 0x30:
                        break;
            ...
            const readMappedWord = (address) => {
                ...
                if (hit.kind === 'io')
                    return ioRead(hit.offset);
                return registry.read['segaic16road.segaic16_road_control_0_r']?.(address, hit.offset >>> 1) ?? 0xffff;
            };
            const writeMappedWord = (address, data, memMask = 0xffff) => {
                const hit = decode(address);
                if (!hit) {
                ...
                if (hit.kind === 'ram')
                    ...
                    registry.write['segaic16road.segaic16_road_control_0_w']?.(address, hit.offset >>> 1, data, memMask);
                }
