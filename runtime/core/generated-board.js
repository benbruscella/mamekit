...
import { createCpu, hasGeneratedCpu } from "./generated-cpu.js";
...
class IrBoard {
    ...
    constructor(machine, config, regions, inputs, sinks) {
                        ...
                        return new Proxy({}, {
                            get: (_target, property) => {
                                ...
                                if (method === 'cycles_to_attotime') {
                                    return (cycles) => cycles / Math.max(1, cpuSpec.cycleClock ?? cpuSpec.clock);
                                }
                                ...
                                if (method === 'attotime_to_cycles') {
                                    return (seconds) => Math.floor(Number(seconds) * Math.max(1, cpuSpec.cycleClock ?? cpuSpec.clock));
                                }
                                if (method === 'reset')
        ...
        for (const specification of machine.execution.cpus) {
            ...
            const ranges = hasHandler(machine, 'bank_select_w') && specification.tag === 'maincpu' && regions.banked
                ...
                : sourceRanges.some(range => range.read === 'mapper.read') &&
                    ...
                    : hasDeviceType(machine, 'NEOGEO_SPRITE_OPTIMZIED') && specification.tag === 'maincpu'
                        ...
                        : sourceRanges;
            const bus = new Bus(ranges, rom, registry, this.shares, specification.space?.dataWidth === 16 ? 16 : generatedCpuDataWidth(type), regions, specification.space?.endianness ?? generatedCpuEndianness(type));
            ...
            const cpu = createCpu(type, {
                ...
                timing: (elapsed, target) => {
                    ...
                    this.cpuSliceCycles.set(specification.tag, Math.max(0, Math.min(target, elapsed)));
        ...
        if (machine.execution.screenUpdate) {
            ...
            }, address => this.cpuBuses.get(machine.execution.cpus[0]?.tag ?? '')?.read(address) ?? 0, this.shares);
            ...
            for (const range of [...(cpu.ranges ?? []), ...(cpu.io?.ranges ?? [])]) {
                for (const [kind, key] of [['read', range.read], ['write', range.write]]) {
                    ...
                    if (!device.methodNames().includes(method))
                        continue;
                    if (kind === 'read') {
                        registry.read[key] = (_address, offset) => device.arity(method) ? device.call(method, offset) : device.call(method);
                    }
                    else {
                        registry.write[key] = (_address, offset, data) => {
    ...
    installMemoryBanks(machine, regions, registry) {
...
export function generatedStateArray(member) {
    ...
    if (!length)
        return undefined;
    if (member.bits === 8)
        return member.signed ? new Int8Array(length) : new Uint8Array(length);
    if (member.bits === 16)
        return member.signed ? new Int16Array(length) : new Uint16Array(length);
    if (member.bits === 32)
        return member.signed ? new Int32Array(length) : new Uint32Array(length);
