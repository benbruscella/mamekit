...
import { dispatchGeneratedCallback, dispatchGeneratedCallbacks, executeGeneratedCallbackHandler, executeGeneratedMachineHandler, generatedHandlerRegistry, wireGeneratedDevice, } from "./generated-handler.js";
import { bindBoardEffects, } from "./generated-effects.js";
...
class IrBoard {
    ...
    constructor(machine, config, regions, inputs, sinks) {
                ...
                const device = createDevice(specification.type, {
                    ...
                    finder: rawTag => {
                        ...
                        if (!cpuSpec)
                            return 0;
                        return {
                            cycles_to_attotime: (cycles) => cycles / Math.max(1, cpuSpec.cycleClock ?? cpuSpec.clock),
        ...
        for (const [tag, bytes] of Object.entries(regions)) {
            bindGeneratedRegionState(this.state, tag, bytes, machine.video?.regionBindings);
        }
        ...
        for (const specification of machine.execution.cpus) {
            ...
            const bus = new Bus(specification.ranges ?? [], rom, registry, this.shares);
            ...
            if (specification.opcode) {
                ...
                const opcodeBus = new Bus(specification.opcode.ranges, opcodeRom, registry, this.shares);
            ...
            const cpu = createCpu(type, {
                ...
                out: bus.out,
                signal: (signal, state) => {
                    const callback = machine.callbacks.find(candidate => candidate.ownerTag === specification.tag &&
                        candidate.signal === signal);
                    const value = callback
                        ? executeGeneratedCallbackHandler(machine, callback, this.bindings, { state, data: state })
                        : undefined;
                    if (value !== undefined)
                        return value;
            ...
            const interruptVector = () => acknowledge
        ...
        for (const callback of machine.callbacks) {
            if (callback.signal !== 'q_out_cb' || callback.slot === undefined)
                continue;
            const source = this.devices.get(callback.ownerTag);
            if (!source)
                continue;
            dispatchGeneratedCallback(machine, callback.id, (source.get('m_q') >> callback.slot) & 1, this.effects);
        }
    ...
    configureHostedProcessor(tag, device, host, firmware, sinks) {
        ...
        for (const callback of this.machine.callbacks.filter(candidate => candidate.ownerTag === tag && candidate.targetMethod)) {
            if (!host.methodNames().includes(callback.targetMethod))
                continue;
            device.on(callback.signal, (...args) => host.call(callback.targetMethod, ...args), callback.slot ?? 0);
        }
        host.bindCall('m_cpu.set_input_line', (_line, state) => device.call('execute_set_input', 0, state));
        ...
        for (const map of machine.maps ?? []) {
            for (const range of map.ranges) {
                for (const [kind, key] of [['read', range.read], ['write', range.write]]) {
                    ...
                    if (!device || !device.methodNames().includes(method))
                        continue;
                    if (kind === 'read') {
                        ...
                        registry.write[key] = (_address, offset, data) => {
                            ...
                            if (parameters[0]?.includes('address_space')) {
                                const cpuTag = machine.execution.cpus.find(cpu => cpu.ranges?.some(candidate => candidate.start === range.start &&
    ...
    installDeclarativeHandlers(machine, config, inputs, registry) {
            ...
            registry.read[key] = (address, offset) => {
                ...
                for (const custom of customs) {
                    ...
                    if (!handler?.program || handler.program.diagnostics.length)
                        continue;
                    const result = executeGeneratedMachineHandler(machine, handler, this.bindings, {}) ?? 0;
        ...
        for (const key of usedHandlers(machine, 'write')) {
            ...
            if (paletteWrite) {
                ...
                registry.write[key] = (_address, offset, data) => this.videoPrimitives?.writePaletteRam?.(offset, data, ext);
                continue;
            }
    ...
    installMemoryBanks(machine, regions, registry) {
        for (const bank of machine.execution.banks ?? []) {
            ...
            registry.write[`bank.${bank.tag}`] = (_address, offset, data) => {
                ...
                if (index >= 0 && index < region.length)
                    region[index] = data;
            };
        }
    }
     ...
     */
    installGeneratedSoundHandlers(machine, sinks, registry) {
        if (!machine.sound)
            return undefined;
    ...
    handlerExecutor(key) {
        ...
        if (!handler?.program)
            return undefined;
        const firstParameter = /(\w+)\s*$/.exec((handler.parameters ?? '').split(',')[0]?.trim() ?? '')?.[1];
        return state => {
    ...
    effectBindings(sinks) {
        return {
            cpuLine: (tag, line, delivery) => {
                ...
                if (line === 'halt')
                    return state => { this.cpuHeld.set(tag, Boolean(state)); };
            ...
            audioWrite: (tag, method) => state => sinks.soundWrite(0, state, this.soundFraction(), `${tag}.${method}`),
        };
    }
}
...
export function bindGeneratedShareState(state, tag, bytes) {
 ...
 */
export function bindGeneratedRegionState(state, tag, bytes, bindings = {}) {
...
function usedHandlers(machine, kind) {
    return (machine.maps ?? []).flatMap(map => map.ranges.flatMap(range => range[kind] ? [range[kind]] : []));
}
