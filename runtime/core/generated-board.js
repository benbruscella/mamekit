...
import { createDevice, hasGeneratedDevice, } from "./generated-device.js";
...
import { compileGeneratedMachineHandler, dispatchGeneratedCallback, dispatchGeneratedCallbacks, executeGeneratedCallbackHandler, executeGeneratedMachineHandler, generatedHandlerRegistry, wireGeneratedDevice, } from "./generated-handler.js";
...
function generatedCpuDataWidth(type) {
...
class IrBoard {
    ...
    constructor(machine, config, regions, inputs, sinks) {
                ...
                const device = createDevice(specification.type, {
                    ...
                    calls: {
                        ...
                        machine: () => ({
                            ...
                            time: () => generatedAttotime(this.frameRunner?.frameCount /
        ...
        for (const [tag, bytes] of Object.entries(regions)) {
            bindGeneratedRegionState(this.state, tag, bytes, machine.video?.regionBindings, machine.video?.regionBindingOffsets);
        }
        ...
        for (const device of this.devices.values()) {
            device.bindCall('machine().time', () => generatedAttotime(this.frameRunner?.frameCount /
                Math.max(1, this.machine.execution.screen.refresh) || 0));
        }
        ...
        for (const specification of machine.execution.cpus) {
            ...
            const ranges = machine.game === 'defender' && specification.tag === 'maincpu' && regions.banked
                ...
                : machine.game === 'outrun' && specification.tag === 'maincpu'
                    ...
                    : machine.family === 'neogeo' && specification.tag === 'maincpu'
                        ...
                        : sourceRanges;
            const bus = new Bus(ranges, rom, registry, this.shares, generatedCpuDataWidth(type), regions);
            ...
            const interruptVector = () => {
                return acknowledge
                    ? executeGeneratedCallbackHandler(machine, acknowledge, this.bindings) ?? 0xff
                    : 0xff;
            };
            ...
            calls[`m_${specification.tag}.total_cycles`] = () => this.cpuCycles.get(specification.tag) ?? 0;
        ...
        const hostedProcessors = (machine.devices ?? []).flatMap(specification => {
            ...
            if (usesProtectionProtocolBridge(machine, specification.tag))
                return [];
                                    ...
                                    device: () => ({
                                        execute: () => ({
                                            adjust_icount: (delta) => {
                                                if (!cpuTag || delta >= 0)
                                                    return;
                                                this.cpuStalls.set(cpuTag, (this.cpuStalls.get(cpuTag) ?? 0) - delta);
                                            },
                                        }),
                                    }),
                                };
    ...
    handlerExecutor(key, firstArgument, scopedCpuTag) {
        ...
        return (state, ...sourceArgs) => {
            ...
            const args = generatedSignalHandlerArguments(handler.parameters, state, firstArgument, sourceArgs);
            return compiled
                ? compiled(args)
 ...
 */
export function usesProtectionProtocolBridge(machine, childTag) {
...
export function bindGeneratedInputState(state, members, inputs) {
