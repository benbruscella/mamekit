...
import { createDevice, generatedTimerBacklog, hasGeneratedDevice, } from "./generated-device.js";
...
import { HOST_SERVICE_CALLS, } from "../ir/board.js";
    ...
    constructor(machine, config, regions, inputs, sinks) {
        ...
        for (const [tag, device] of this.devices) {
            ...
            for (const method of device.methodNames()) {
                const invoke = (...args) => device.invoke(method, ...args);
        ...
        for (const specification of machine.execution.cpus) {
            ...
            const cpu = createCpu(type, {
                ...
                timing: (elapsed, target) => {
                    ...
                    if (specification.tag === timerClockCpu?.tag) {
                        ...
                        if (current < previousTimingCycles)
                            previousTimingCycles = 0;
            ...
            const interruptVector = () => this.interruptVector(specification.tag);
        ...
        if (machine.devices?.some(device => device.type === 'Z80DMA')) {
            ...
            const transfer = () => {
                ...
                for (byteCounter = 0; byteCounter <= count && enabled && isReady(); byteCounter++) {
                    ...
                    if (!(wr1 & 0x20))
                        addressA = (addressA + (wr1 & 0x10 ? 1 : -1)) & 0xffff;
                    if (!(wr2 & 0x20))
                        addressB = (addressB + (wr2 & 0x10 ? 1 : -1)) & 0xffff;
                }
                enabled = false;
            ...
            const read = () => {
                ...
                if (mask && (mask & (mask - 1))) {
                    do
                        readIndex = (readIndex + 1) & 7;
                    while (!(mask & (1 << readIndex)));
                }
                return value & 0xff;
            };
            ...
            cpuLine: (tag, line, delivery) => {
                ...
                return state => {
                    const acknowledge = this.machine.callbacks.some(callback => callback.ownerTag === tag &&
                        callback.signal === 'set_irq_acknowledge_callback');
                    this.cpus.get(tag)?.setIrqLine(state !== 0, acknowledge ? () => this.interruptVector(tag) : 0xff, delivery === 'hold' && state !== 0);
                };
            },
