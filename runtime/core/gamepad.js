 ...
 */
const FOLD = [['b0', 'IPT_BUTTON1'], ['b1', 'IPT_BUTTON2'], ['b7', 'IPT_BUTTON3']];
...
export class GamepadInput {
    ...
    constructor(input, bindings, read) {
        ...
        for (let player = 1; player <= players; player++) {
            for (const [control, type] of FOLD) {
                if (this.targets.has(`${player}:${control}`))
                    continue;
                for (const binding of bindings) {
        ...
        input.onReleaseAll(() => { for (const slot of this.slots)
            slot.active.clear(); });
    }
    ...
    controlNames(binding) {
        ...
        if (!slot)
            return [];
        const names = (this.controls.get(binding) ?? []).map(control => slot.mapping === 'standard' || !/^b\d+$/.test(control)
    ...
    poll() {
        ...
        for (const pad of pads) {
            ...
            if (!slot) {
                ...
                changed = true;
                if (this.debug)
                    console.log(`[gamepad] player ${player}: ${pad.id} (mapping "${pad.mapping}")`);
            }
        ...
        for (const slot of [...this.slots]) {
            ...
            changed = true;
            if (this.debug)
                console.log(`[gamepad] player ${slot.player} disconnected: ${slot.id}`);
        }
        if (changed)
    ...
    edge(slot, control, down) {
        ...
        if (!bindings) {
            if (this.debug && down)
                console.log(`[gamepad] player ${slot.player} ${control} unbound`);
            return;
        }
        for (const binding of bindings)
