...
export function menuTabs(games) {
    return ['arcade', 'console'].filter(tab => games.some(game => (game.kind === 'console' ? 'console' : 'arcade') === tab));
}
...
export async function runMenu() {
    ...
    const requestedTab = new URLSearchParams(location.search).get('tab') === 'consoles' ? 'console' : 'arcade';
    ...
    for (const [tab, text] of [['arcade', 'ARCADE'], ['console', 'CONSOLES']]
    ...
    function swapConsoleCover(canvas) {
