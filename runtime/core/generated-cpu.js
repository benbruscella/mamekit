...
export function generatedCpuFacts(type) {
...
class IrCpu {
    ...
    stateNames() {
        if (this.stateIndexNames)
            return this.stateIndexNames;
        const names = new Map();
    ...
    externalCalls() {
        ...
        return {
            ...Object.fromEntries(Object.entries(this.definition.callbacks ?? {}).map(([member, signal]) => [member, (state = 0) => Number(this.bus.signal?.(signal, state) ?? 0)])),
            ...Object.fromEntries(Object.keys(this.definition.callbacks ?? {}).map(member => [`${member}.isunset`, () => (this.bus.signal ? 0 : 1)])),
            READ: address => {
            ...
            READ32BE: address => {
                ...
                return ((this.readMemory(location) << 24) |
                    (this.readMemory(location + 1) << 16) |
                    (this.readMemory(location + 2) << 8) |
                    this.readMemory(location + 3)) >>> 0;
            },
            READ_VECTOR: ordinal => {
                this.set('cycles', this.get('cycles') + 1);
            ...
            'm_opcodes.read_byte': address => (this.bus.readOpcode?.(address & this.addressMask) ??
            ...
            standard_irq_callback: (...args) => this.bus.acknowledge?.(Number(args[0]) || 0) ?? this.acknowledgeIrq(),
    ...
    readMemory(address) {
        ...
        for (let index = 0; index < ports.length; index++) {
            ...
            if (index === handshake?.portIndex) {
                ...
                const data = (this.portHandshakeControl & handshake.latchEnableMask) ||
                    direction === 0xff
                    ? this.portData[index]
    ...
    writeMemory(address, value) {
        ...
        const handshake = this.definition.internal?.portHandshake;
        if (location === handshake?.controlAddress) {
            if (globalThis.__csrDbg < 10) {
                globalThis.__csrDbg = (globalThis.__csrDbg ?? 0) + 1;
    ...
    updatePortHandshakeInput(inputnum, state) {
        const handshake = this.definition.internal?.portHandshake;
        if (handshake && globalThis.__hsDbg < 20) {
            globalThis.__hsDbg = (globalThis.__hsDbg ?? 0) + 1;
            console.log('[hs] input line', inputnum, 'state', state, 'want', handshake.inputLine, 'csr', this.portHandshakeControl.toString(16), 'latched', this.portHandshakeLatched);
        }
        if (!handshake || inputnum !== handshake.inputLine)
            return;
        if (!this.portHandshakeInputState &&
            state !== 0 &&
    ...
    constructor() {
        this.B = new Proxy({}, {
            ...
            set: (_target, key, value) => {
                const index = Number(key);
                const slot = index >>> 1;
                const old = this.W[slot] ?? 0;
                this.W[slot] = index & 1 ? (old & 0xff00) | (Number(value) & 0xff)
                    : (old & 0xff) | ((Number(value) & 0xff) << 8);
                return true;
            },
        });
        this.L = new Proxy({}, {
