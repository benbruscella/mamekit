...
export async function runShell(cfg, preloaded, mounted = []) {
    ...
    if (cfg.kind !== 'console') {
        void loadArtwork(cfg.game).then(art => {
            if (art?.window)
                ui.setBezel(art.bmp, art.window, art.tints, art.backdrop);
        });
    }
 ...
 */
export function controlsHelp(cfg, pads, pointer) {
    const parts = [];
    // One entry per stick: a twin-stick panel (Battlezone's treads) names each.
    const sticks = new Map();
    const direction = (label) => ['_UP', '_DOWN', '_LEFT', '_RIGHT'].findIndex(suffix => label.endsWith(suffix));
    const seen = new Set();
    ...
    for (const b of cfg.bindings) {
        ...
        if (fn === 'move') {
            ...
            const stick = sticks.get(name) ?? { keys: new Map(), pads: new Set() };
            sticks.set(name, stick);
            for (const k of b.keys)
                if (!stick.keys.has(k))
                    stick.keys.set(k, direction(b.label));
            for (const pad of padNames(b))
                stick.pads.add(pad);
            continue;
        }
...
function buildDom(cfg) {
    ...
    return {
        ...
        setBezel: (bmp, win, tints, backdrop) => {
