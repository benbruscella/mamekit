...
export function nestedOperations(operation) {
    switch (operation.op) {
        ...
        case 'while':
        case 'do-while': return [operation.body];
...
export function operationExpressions(operation) {
    switch (operation.op) {
        ...
        case 'for':
        case 'while':
        case 'do-while': return [operation.condition];
        case 'switch':
...
export function childExpressions(expression) {
    switch (expression.kind) {
        case 'unary':
        case 'cast': return [expression.operand];
...
export function walkOperations(operations, visit) {
    for (const operation of operations) {
        visit(operation);
        for (const nested of nestedOperations(operation))
            walkOperations(nested, visit);
    }
}
/** Visit every expression a program evaluates, outermost first. */
export function walkExpressions(operations, visit) {
