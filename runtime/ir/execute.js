...
function preparedMachineCalls(machine, bindings, ownerClass) {
    ...
    if (cached && cached.seededFrom === bindings.referenceCalls)
        return cached;
...
function compileValueWrapper(valueType) {
    ...
    if (normalized === 'uint8_t' || normalized === 'u8')
...
function isFloatingExpression(expression) {
...
function wrapValue(valueType, value) {
    ...
    if (valueType === 'int32_t' || valueType === 's32')
        return number | 0;
    if (valueType === 'uint64_t' || valueType === 'u64' ||
