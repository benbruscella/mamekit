...
export class BoardIrError extends Error {
    diagnostics;
    constructor(subject, diagnostics) {
        super(`${subject}: board IR is invalid\n` +
            diagnostics
...
class Reader {
    ...
    optionalString(value, path, source) {
        if (value === undefined)
            return;
        this.string(value, path, source);
    }
    optionalNumber(value, path, source) {
        if (value === undefined)
            return;
 ...
 */
export function decodeBoardIr(value, subject = 'board') {
    const reader = new Reader();
    const root = reader.object(value, '');
    const schemaVersion = root.schemaVersion;
    if (schemaVersion !== BOARD_IR_SCHEMA_VERSION) {
        reader.fail('schemaVersion', `artifact declares ${describe(schemaVersion) === 'number' ? schemaVersion : describe(schemaVersion)}, ` +
...
function decodeStateMembers(reader, value) {
    if (value === undefined)
        return;
...
function decodeCallbacks(reader, value) {
    for (const [index, entry] of reader.array(value, 'callbacks').entries()) {
        ...
        if (callback.delivery !== undefined && !DELIVERIES.has(callback.delivery)) {
            reader.fail(`${path}.delivery`, `unknown delivery mode ${describe(callback.delivery)}`, source);
        }
        if (callback.transforms !== undefined) {
            for (const [position, transform] of reader.array(callback.transforms, `${path}.transforms`, source).entries()) {
                reader.string(transform, `${path}.transforms[${position}]`, source);
            }
        }
        if (callback.scanlines !== undefined) {
            for (const [position, scanline] of reader.array(callback.scanlines, `${path}.scanlines`, source).entries()) {
 ...
 */
function decodeConnections(reader, value) {
    for (const [index, entry] of reader.array(value, 'connections').entries()) {
        ...
        if (typeof kind !== 'string' || !EFFECT_KINDS.has(kind)) {
            reader.fail(`${path}.effect.kind`, `expected one of ${[...EFFECT_KINDS].join(', ')}, got ${describe(kind)}`, source);
        }
        else if (kind === 'cpu-line') {
            reader.string(effect.tag, `${path}.effect.tag`, source);
            if (!CPU_LINES.has(effect.line)) {
                reader.fail(`${path}.effect.line`, `unknown CPU line ${describe(effect.line)}`, source);
            }
            if (!DELIVERIES.has(effect.delivery)) {
                reader.fail(`${path}.effect.delivery`, `unknown delivery mode ${describe(effect.delivery)}`, source);
            }
        }
        ...
        for (const [position, raw] of reader.array(connection.transforms, `${path}.transforms`, source).entries()) {
            ...
            if (typeof transformKind !== 'string' || !TRANSFORM_KINDS.has(transformKind)) {
                reader.fail(`${transformPath}.kind`, `expected one of ${[...TRANSFORM_KINDS].join(', ')}, got ${describe(transformKind)}`, source);
                continue;
            }
            if (transformKind === 'mask')
                reader.number(transform.value, `${transformPath}.value`, source);
            if (transformKind === 'bit')
                reader.number(transform.bit, `${transformPath}.bit`, source);
...
function decodeVideo(reader, value) {
    if (value === undefined)
        return;
    ...
    if (video.ramPalette !== undefined) {
        ...
        if (palette.endianness !== undefined) {
            const endianness = reader.string(palette.endianness, 'video.ramPalette.endianness', source);
            if (endianness !== 'little' && endianness !== 'big') {
                throw new Error(`${source}: video.ramPalette.endianness must be "little" or "big"`);
            }
        }
        for (const [index, channel] of reader.array(palette.channels, 'video.ramPalette.channels', source).entries()) {
...
function decodeSound(reader, value) {
    if (value === undefined)
        return;
    ...
    for (const [index, entry] of reader.array(sound.routes ?? [], 'sound.routes').entries()) {
        ...
        for (const field of ['chip', 'channel', 'gain'])
            reader.number(route[field], `${path}.${field}`);
        reader.string(route.target, `${path}.target`);
    }
}
function decodeExecution(reader, value) {
    const execution = reader.object(value, 'execution');
    const cpus = reader.array(execution.cpus, 'execution.cpus');
    if (!cpus.length)
        reader.fail('execution.cpus', 'a board must declare at least one CPU');
    for (const [index, entry] of cpus.entries()) {
        ...
        if (cpu.space !== undefined)
            decodeAddressSpace(reader, cpu.space, `${path}.space`, source);
        if (cpu.ranges !== undefined)
            decodeRanges(reader, cpu.ranges, `${path}.ranges`, source);
        ...
        if (cpu.io !== undefined) {
            ...
            if (io.space !== undefined)
                decodeAddressSpace(reader, io.space, `${path}.io.space`, source);
        }
    }
    ...
    for (const field of ['startHandlers', 'resetHandlers']) {
        if (execution[field] !== undefined) {
            for (const [index, handler] of reader.array(execution[field], `execution.${field}`).entries()) {
    ...
    for (const [index, entry] of reader.array(execution.frameEvents, 'execution.frameEvents').entries()) {
    ...
    if (execution.accessTaps !== undefined) {
        for (const [index, entry] of reader.array(execution.accessTaps, 'execution.accessTaps').entries()) {
    ...
    if (execution.banks !== undefined) {
        for (const [index, entry] of reader.array(execution.banks, 'execution.banks').entries()) {
            ...
            if (bank.entryRegions !== undefined) {
                for (const [position, region] of reader.array(bank.entryRegions, `${path}.entryRegions`, source).entries()) {
                    if (region !== null)
                        reader.string(region, `${path}.entryRegions[${position}]`, source);
                }
            }
            reader.optionalNumber(bank.dynamicShift, `${path}.dynamicShift`, source);
            if (bank.entryMembers !== undefined) {
                for (const [position, member] of reader.array(bank.entryMembers, `${path}.entryMembers`, source).entries()) {
                    if (member !== null)
                        reader.string(member, `${path}.entryMembers[${position}]`, source);
                }
            }
...
function decodeAddressSpace(reader, value, path, source) {
...
function decodeRanges(reader, value, path, source) {
    ...
    for (const [index, entry] of reader.array(value, path, source).entries()) {
        ...
        if (!kinds.has(range.kind)) {
            reader.fail(`${rangePath}.kind`, `expected one of ${[...kinds].join(', ')}, got ${describe(range.kind)}`, source);
        }
...
function decodeDevices(reader, value) {
    if (value === undefined)
        return;
    for (const [index, entry] of reader.array(value, 'devices').entries()) {
        ...
        if (device.addressSpaces !== undefined) {
            for (const [position, raw] of reader.array(device.addressSpaces, `${path}.addressSpaces`, source).entries()) {
...
function decodeComposition(reader, value) {
    if (value === undefined)
        return;
...
function decodeHandlers(reader, value) {
    if (value === undefined)
        return;
...
function decodeMaps(reader, value) {
    if (value === undefined)
        return;
    for (const [index, entry] of reader.array(value, 'maps').entries()) {
        ...
        for (const [position, rangeEntry] of reader.array(map.ranges, `${path}.ranges`, source).entries()) {
