 ...
 */
export function isSchedulerCallback(callback) {
...
export function validateBoardIr(board) {
    ...
    for (const [index, cpu] of board.execution.cpus.entries()) {
        ...
        if (cpu.space && cpu.space.ownerTag !== cpu.tag) {
            fail(`execution.cpus[${index}].space.ownerTag`, `program space owner "${cpu.space.ownerTag}" does not match CPU "${cpu.tag}"`, cpu.source);
        }
    ...
    for (const [index, participant] of (board.execution.participants ?? []).entries()) {
        ...
        if (!deviceTags.has(participant.tag)) {
            fail(`execution.participants[${index}].tag`, `execute participant "${participant.tag}" has no matching device`);
        }
    ...
    for (const [index, callback] of board.callbacks.entries()) {
        ...
        if (isUnconnectedCallback(callback) || isSchedulerCallback(callback))
            continue;
    ...
    for (const [index, tap] of (board.execution.accessTaps ?? []).entries()) {
        if (!board.execution.cpus.some(cpu => cpu.tag === tap.cpu)) {
            fail(`execution.accessTaps[${index}].cpu`, `access tap watches unknown CPU "${tap.cpu}"`, tap.source);
        }
        if (!board.devices?.some(device => device.tag === tap.device)) {
            fail(`execution.accessTaps[${index}].device`, `access tap targets unknown device "${tap.device}"`, tap.source);
        }
        if (tap.bank !== undefined && !bankTags.has(tap.bank)) {
            fail(`execution.accessTaps[${index}].bank`, `access tap selects bank "${tap.bank}", which the board does not configure`, tap.source);
        }
    }
    for (const [index, cpu] of board.execution.cpus.entries()) {
    ...
    if (screenUpdate && !handlerKeys.has(screenUpdate.handler)) {
        ...
        if (!screenUpdate.deviceTag) {
            ...
            fail('execution.screenUpdate.deviceTag', `screen update belongs to device "${screenUpdate.deviceTag}", which the board does not compose`, screenUpdate.source);
        }
    }
    for (const [index, custom] of (board.execution.customs ?? []).entries()) {
        if (custom.source === 'device-line') {
            ...
            if (custom.deviceTag && deviceTags.has(custom.deviceTag))
                continue;
            fail(`execution.customs[${index}]`, `custom input port "${custom.port}" reads device "${custom.deviceTag}", ` +
